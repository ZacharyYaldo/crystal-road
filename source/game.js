...
function draw(){hits.length=0;PRESSED=[];tx.clearRect(0,0,tc.width,tc.height);px(0,0,W,H,'#0b0d16');
  ...
  if(G.toast&&!G.reveal&&!(G.toast.scope==='road'&&G.screen!=='road')){let a=clamp(Math.min(G.toast.t*4,((G.toast.dur||2.2)-G.toast.t)*3),0,1);if(G.toast.scope==='road'){const w=textW(G.toast.text,'xs')+12;ctx.globalAlpha=a*0.45;px(4,SCENE_Y+192,R(w),12,'#000');ctx.globalAlpha=1;text(10,SCENE_Y+194,G.toast.text,'xs',G.toast.col||C.cream,'left',a);}else if(G.toast.screen==='road'&&G.screen==='road'&&!G.sheet){const w=textW(G.toast.text,'s')+16,ty=SCENE_Y+186;ctx.globalAlpha=a*0.55;px(R(W/2-w/2),ty,R(w),16,'#000');ctx.globalAlpha=1;text(W/2,ty+4,G.toast.text,'s',G.toast.col||C.cream,'center',a);}
