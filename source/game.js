...
function startHordeFight(){if(G.delve){toast('Finish the catacombs first');return;}const C2=G.castle,S=hordeStrength(),Z=ZONES[ZONES[G.zone].endless?endSeg(G.prog[G.zone]):G.zone];const gar=garrisonHeroes().filter(h=>!G.active.includes(h));
  const avgL=G.active.length?G.active.reduce((a,h)=>a+h.lvl,0)/G.active.length:1;const hordeLv=R(Math.max(Z.lv+S*0.6,avgL*0.9+(C2.streak||0)*2));G.hordeFight={wave:0,saved:{active:G.active.slice(),zone:G.zone,mode:G.mode,prog:G.prog.slice()},lvl:hordeLv,pool:Z.pool,boss:Z.boss};
  G.active=G.active.concat(gar).slice(0,6);for(const h of G.active){h.dead=false;h.status={};if(h.hp<h.maxhp*0.5)h.hp=R(h.maxhp*0.5);}layout();{const D=defense(),p=C2.prep||{};const partyHp=G.active.reduce((a,h)=>a+h.maxhp,0);const ml=mercLv();const frac=Math.min(2,D.walls/100)+(ml?0.2+0.1*ml:0);G.hordeFight.gateMax=R(partyHp*frac);G.hordeFight.gate=G.hordeFight.gateMax;G.hordeFight.mercs=ml?Math.min(4,1+ml):0;G.hordeFight.mercLv=ml;}G.active.forEach(h=>{h.x=h.tx;});C2.hordeDue=false;G.enemies=[];G.projs=[];G.action=null;G.screen='road';closeSheet();sfx('cast');banner('The horde is at the gate','Wave 1 of 3',2.5);hordeWave();}
