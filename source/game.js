...
const now=()=>Date.now();
const ROLL={};function roll(key,target,tau){const r=ROLL[key]||(ROLL[key]={v:target});const d=target-r.v;const t=tau!=null?tau:(HOLD?0.10:0.045);if(Math.abs(d)<Math.max(0.5,Math.abs(target)*0.002)||Math.abs(d)>Math.abs(target)*1e6)r.v=target;else r.v+=d*(1-Math.exp(-rollDt/t));return r.v;} /* time-based easing (tau seconds), same speed on any frame rate; snaps once within half a unit or 0.2% */
let rollDt=1/60;
...
let PRESSED=[];function pressDy(x,y){for(const r of PRESSED)if(x>=r.x&&x<r.x+r.w&&y>=r.y&&y<r.y+r.h)return 1;return 0;}
function icon(name,x,y,pal=0,scale=1){ctx.drawImage(iconImg(name,pal,scale),x,y+pressDy(x+4,y+4));}
...
function button(x,y,w,h,gold=false,disabled=false){const pr=!disabled&&isPressed(x,y,w,h);if(pr){PRESSED.push({x,y,w,h});y+=1;}
...
function text(x,y,s,style='b',col=C.cream,align='left',alpha=1){y+=pressDy(x,y+3);
...
const TREE=[
  ...
  {branch:'Crystal',id:'b_dust',name:'Crystal Memory',desc:'+10% dust from every Shatter',max:20,base:8,cur:'dust'},
...
function tal(h,id){if(!h||!h.talents||!h.talents[id])return false;const i=(TALENTS[h.cls]||[]).findIndex(t=>t.id===id);return i<0||h.lvl>=talentReq(i).lvl;}
function buyTalent(h,i){h.talents=h.talents||{};const t=TALENTS[h.cls][i],q=talentReq(i);if(h.talents[t.id]){if(h.lvl<q.lvl)toast(t.name+' is learned, but sleeps until '+h.name+' reaches Lv '+q.lvl+' again');else toast(t.name+' is active. '+t.desc);return;}if(h.lvl<q.lvl){toast(t.name+' unlocks at Lv '+q.lvl+'. '+t.desc);return;}if(G.gold<q.gold){toast(t.name+' costs '+fmtNum(q.gold)+' gold. '+t.desc);return;}G.gold-=q.gold;h.talents[t.id]=true;buildChanged();sfx('level',true);toast(h.name+' learns '+t.name,C.goldL);}
const TIERPAL={
...
function promoteReq(h){const t=h.tier||0;return{lvl:30*(t+1),gold:R(5000*Math.pow(12,t)),dust:((h.tierMax||0)>t)?0:5*(t+1)};}
function promoteHero(h){const t=h.tier||0;if(t>=3){toast(h.name+' is already a '+heroTitle(h));return;}const q=promoteReq(h);if(h.lvl<q.lvl){toast(h.name+' can be promoted at Lv '+q.lvl);return;}if(G.gold<q.gold||G.dust<q.dust){toast('Needs '+fmtNum(q.gold)+' gold and '+q.dust+' dust');return;}G.gold-=q.gold;G.dust-=q.dust;h.tier=t+1;buildChanged();h.tierMax=Math.max(h.tierMax||0,h.tier);h.uid=heroUid(h);refreshStats(h);h.hp=h.maxhp;sfx('level',true);levelBurst(h);banner(h.name+' becomes a '+heroTitle(h),'+30% to every stat, ability power up',3,'any');}
...
// boss losses: first loss returns the party to the last checkpoint before the boss, consecutive losses go one checkpoint further back (capped at two); tapping the latest checkpoint or the boss dot resets the streak
...
function noteFight(win,wasBoss){const Z=ZONES[G.zone];if(!Z||Z.endless||G.delve||G.hordeFight)return;const T=G.train;
  if(T&&T.active){if(G.zone!==T.zone)return;T.fights++;const prog=partyProg()-T.startProg;if(prog>=(T.target||1)&&T.fights>=AT_MIN_FIGHTS){finishTraining(true);return;}T.r=(T.r||[]).concat([win?1:0]).slice(-AT_WINDOW);if(!win&&T.r.length>=AT_MIN&&T.r.filter(x=>!x).length>=AT_LOSSES&&T.zone>0){T.zone--;T.r=[];if(T.rec)T.rec.zone=ZONES[T.zone].name;goZone(T.zone);banner(ZONES[T.zone+1].name+' is too dangerous too','Training in '+ZONES[T.zone].name,3);}return;}
  if(wasBoss){if(!win&&AT_RESET_ON_BOSS_LOSS)G.danger=null;return;}
  if(T&&!T.active&&T.returnFights!=null&&T.after&&T.after.zone===G.zone)T.returnFights++;const prev=(T&&!T.active&&T.after&&T.after.zone===G.zone&&T.returnFights!=null)?{fights:T.returnFights,minutes:+((now()-(T.returnedAt||now()))/60000).toFixed(1)}:null;
  if(!atAllowed(G.zone)&&!(G.keepPushing&&G.keepPushing[G.zone]))return;
  ...
  if(G.train&&G.train.after!=null&&G.train.after.zone===G.zone){const a=G.train.after;a.r.push(win?1:0);if(a.r.length===AT_WINDOW)a.rec.winAfter=+(a.r.filter(x=>x).length/a.r.length).toFixed(2);if(a.r.length>=20){a.rec.winAfter20=+(a.r.filter(x=>x).length/a.r.length).toFixed(2);G.train=null;}}
  if(!win&&atAllowed(G.zone)&&D.r.length>=AT_MIN&&D.r.filter(x=>!x).length>=AT_LOSSES)startTraining(prev);}
function startTraining(prev){const ret=G.zone,zone=ret-1,S=atStats();const D=G.danger||{r:[]};prev=prev||null;const quick=!!(prev&&prev.fights>=1&&prev.fights<=AT_QUICK_FIGHTS);const target=quick?AT_RETRIGGER_TARGET:1;const Zr=ZONES[ret];const lp=G.lastLossProg!=null?G.lastLossProg:G.prog[ret];const rec={ret:ZONES[ret].name,zone:ZONES[zone].name,prog:G.prog[ret],lossProg:lp,enemyLv:roadLv(Zr,Zr.lv+lp*0.3),window:D.r.join(''),retrigger:prev,quick,target,before:partyState(),lvBefore:+partyLvAvg().toFixed(1),winBefore:+(D.r.filter(x=>x).length/Math.max(1,D.r.length)).toFixed(2),t0:now(),lvAfter:null,secs:null,fights:null,xp:null,winAfter:null};
  G.train={active:true,zone,ret,startProg:partyProg(),target,fights:0,t0:now(),gold0:G.stats.gold||0,items0:G.stats.items||0,rec};G.danger=null;S.triggers++;S.log.push(rec);if(S.log.length>60)S.log.shift();
...
function addHero(def,lvl=1){const h=mkHero(def);h.lvl=lvl;Object.assign(h,heroStats(h));h.hp=h.maxhp;G.roster.push(h);if(G.active.length<partyMax()){h.x=-30;G.active.push(h);layout();}return h;}
...
function dealDamage(src,tgt,mult=1,opts={}){
  ...
  if(!crit&&c&&rand()<critChance(src))crit=true;if(crit)dmg*=1.6;
  const preShield=dmg;if(tgt.status&&tgt.status.shield>0)dmg=Math.max(0,Math.min(dmg*(1-(tgt.status.shieldPct||0.3)),dmg-SHIELD_MIN_CUT)); /* a shield always takes at least SHIELD_MIN_CUT off the hit */if(tgt.status&&tgt.status.shield>0&&!tgt.enemy&&src.enemy&&sup(tgt,'sword')){const refl=R((preShield-dmg)*0.2);if(refl>0){src.hp=Math.max(1,src.hp-refl);float(src.x+src.dx,GROUND+(src.yoff||0)-36,fmtNum(refl),'#7fd4ff');}}if(tgt.status&&tgt.status.ward>0){const pre=dmg;dmg*=0.7;if(G.fs&&tgt.enemy)G.fs.wardReduced=(G.fs.wardReduced||0)+(pre-dmg);}if(tgt.status&&tgt.status.rage>0&&!(tgt.talents&&tal(tgt,'defiant')&&tgt.hp<tgt.maxhp*0.3)&&!sup(tgt,'axe'))dmg*=(tgt.enemy?1.5:1.3);if(tgt.status&&tgt.status.burn>0)dmg*=1.2;
  ...
  dmg=Math.max(1,R(Math.max(dmg,atk*mult*0.02)));if(!tgt.enemy&&tgt.talents&&tal(tgt,'unbroken')&&!tgt.unbrokenUsed&&tgt.hp-dmg<=0){dmg=tgt.hp-1;tgt.unbrokenUsed=true;float(tgt.x,GROUND-44,'Unbroken!',C.goldL,true);}if(!tgt.enemy)dmg=R(dmg*omenMult('hdmg'));if(!tgt.enemy&&G.hordeFight&&G.hordeFight.gate>0){const take=Math.min(G.hordeFight.gate,R(dmg*0.5*(G.hordeFight.mod==='siege'&&src.champ?2:1)));G.hordeFight.gate-=take;dmg-=take;if(G.hordeFight.gate<=0)float(W/2,GROUND-70,'The gate is breached!','#ff8a80',true);}tgt.hp-=dmg;if(!src.enemy)dmgBy(G.dmgSrc||'basic',dmg);if(!src.enemy&&src.status&&src.status.rage>0&&!src.dead)src.hp=Math.min(src.maxhp,src.hp+dmg*0.05);sfx(crit?'crit':'hit');if(!src.enemy&&G.fs){G.fs.dmg+=dmg;G.stats.hit=Math.max(G.stats.hit||0,dmg);}if(src.enemy&&G.fs){if(src.boss&&G.dmgSrcEnemy!=='charge')G.fs.bossHits=(G.fs.bossHits||0)+1;G.fs.taken=G.fs.taken||{};G.fs.taken[src.name]=(G.fs.taken[src.name]||0)+dmg;G.fs.takenT=G.fs.takenT||{};G.fs.takenT[src.range]=(G.fs.takenT[src.range]||0)+dmg;}
  if(src.enemy&&!tgt.dead&&tgt.talents&&tal(tgt,'riposte')&&tgt.status.shield>0&&rand()<0.2){src.hp-=R(tgt.atk*0.8);float(src.x+src.dx,GROUND-40,'riposte '+fmtNum(R(tgt.atk*0.8)),C.goldL);if(src.hp<=0){src.hp=0;src.dead=true;setAnim(src,'death',false,8);killReward(src);}}
  if(src.enemy&&!tgt.dead){const tr=src.traits||[];if(tr.includes('poison')&&rand()<0.35){tgt.status.poison=3;float(tgt.x,GROUND-46,'poisoned','#7ad07a');}if(tr.includes('stun')&&rand()<0.15&&!tgt.status.stun&&!(G.zone===0&&!(G.reforges>0)&&!G.delve&&!G.hordeFight&&G.prog[0]<ZONES[0].fights/3)){tgt.status.stun=1;float(tgt.x,GROUND-46,'stunned','#f4d35e');}if(src.mech&&src.mech.drain&&tgt.charge>0){tgt.charge=Math.max(0,tgt.charge-25);float(tgt.x,GROUND-46,'drained','#b06cff');}}
  if(!src.enemy&&tgt.enemy&&!tgt.dead){if(src.talents&&tal(src,'burn')&&(opts.spell)){tgt.status.burn=3;}if(src.talents&&tal(src,'mark')){tgt.marks=(tgt.marks||0)+1;}if(src.talents&&tal(src,'poison')&&crit){tgt.status.poison=3;}}
  if(tgt.enemy&&!tgt.dead)bossCheck(tgt);if(tgt.enemy&&!tgt.dead&&!tgt.enraged&&(tgt.traits||[]).includes('enrage')&&tgt.hp<tgt.maxhp*0.5)enrage(tgt);if(G.delve&&!src.enemy){const lr=G.delve.relics.find(r=>r.leech);if(lr){src.hp=Math.min(src.maxhp,src.hp+R(dmg*lr.leech));}}
  ...
  if(tgt.hp<=0){tgt.hp=0;tgt.dead=true;tgt.status={};setAnim(tgt,'death',false,8);if(tgt.enemy){killReward(tgt);if(!src.enemy&&src.talents){if(tal(src,'frenzy')&&src.status.rage>0)src.status.rage+=2;if(tal(src,'overflow'))src.charge=Math.min(100,src.charge+20);}}}
  else if(!(G.action&&G.action.u===tgt)&&!opts.noHurt)setAnim(tgt,'hurt',false,10);
  return dmg;}
function killReward(e){G.stats.kills++;if(e.boss)G.stats.bosses++;G.surge=Math.min(100,(G.surge||0)+2.5*(1+0.1*treeLv('b_surge'))*(built('workshop')?1.5:1));sfx('death');setTimeout(()=>sfx('coin'),120);const xp=R((6+e.lvl*3+(e.boss?60:0))*Math.pow(1.04,e.lvl)*bless('xp')),gold=R((3+e.lvl*2+(e.boss?80:0))*Math.pow(1.05,e.lvl)*(0.9+rand()*0.2)*goldMult()*zoneBonus());G.gold+=gold;G.run.gold+=gold;G.stats.gold+=gold;if(G.fs)G.fs.gold+=gold;float(e.x+e.dx,GROUND+(e.yoff||0)-40,'+'+fmtNum(gold)+' gold',C.goldL);
  ...
  if(rand()<ORE_DROP_CHANCE){const o=R((1+Math.floor(rand()*2))*(1+Math.floor(e.lvl/6))*oreScale(e.lvl)*oreMult()*zoneBonus());G.ore+=o;float(e.x+e.dx,GROUND+(e.yoff||0)-50,'+'+fmtNum(o)+' ore',C.muted);}}
...
function updateProjs(dt){for(const p of G.projs){p.t+=dt;if(p.magic&&rand()<0.7){const k=p.t/p.dur,txx=p.tgt.x+p.tgt.dx,x=p.x+(txx-p.x)*k,y=p.y+(GROUND+(p.tgt.yoff||0)-16-p.y)*k;G.parts.push({x,y:y+(rand()*4-2),vx:0,vy:-6,life:0.3,col:p.col});}if(p.t>=p.dur){p.hit=true;if(p.merc){if(!p.tgt.dead){const d=R(p.tgt.maxhp*p.frac);p.tgt.hp=Math.max(1,p.tgt.hp-d);p.tgt.flash=0.15;float(p.tgt.x+p.tgt.dx+(rand()*10-5),GROUND+(p.tgt.yoff||0)-34,fmtNum(d),'#9ad0ff');sfx('hit');}}else if(!p.tgt.dead){let m=p.mult||1;if(!p.src.enemy&&tal(p.src,'mark'))m*=1+0.05*Math.min(10,p.tgt.marks||0);G.dmgSrc=p.srcKind||null;dealDamage(p.src,p.tgt,m,{spell:!!p.magic});G.dmgSrc=null;}}}G.projs=G.projs.filter(p=>!p.hit);}
...
function tapDamage(){const avgAtk=G.active.reduce((s2,h)=>s2+h.atk,0)/Math.max(1,G.active.length);return Math.max(1,R(avgAtk*0.3*(1+0.2*treeLv('tap'))*(built('watchtower')?1.1:1)));}
...
function endBattle(win){if(G.hordeFight){if(!win){endHordeFight(false);return;}if(G.hordeFight.wave>=3){endHordeFight(true);return;}G.enemies=[];G.mode='delve';for(const h of G.active)if(!h.dead){const g2=R(h.maxhp*0.3);h.hp=Math.min(h.maxhp,h.hp+g2);float(h.x+h.dx,GROUND-30,'+'+fmtNum(g2),'#8ff0a0');}G.hordeFight.nextWaveAt=RT+0.7;return;}if(G.delve){if(win)delveWin();else endDelve(true);return;}const Z=ZONES[G.zone];
  if(win){G.wins++;G.losses=0;G.run.fights++;checkUnlocks();if(canReforge()){G.dustFrac=(G.dustFrac||0)+(1+0.25*G.zone+0.5*(G.reforges||0))/150;if(G.dustFrac>=1){const d=Math.floor(G.dustFrac);G.dustFrac-=d;G.dust+=d;}}if(G.bossFight){G.run.bosses++;if(canReforge()){G.dust+=2;}G.castle.bp=G.castle.bp||{};const BZ=bossZone();const sk=Object.keys(STRUCTS).find(k=>STRUCTS[k].boss===BZ.boss);if(sk&&!G.castle.bp[sk]){G.castle.bp[sk]=true;G.pending='Blueprint found|'+STRUCTS[sk].name+' - build it in Vael';G.vaelNew=true;}const bn=ENEMIES[BZ.boss].name,fs=G.fs||{dmg:0,taps:0,gold:0};G.card={title:bn,sub:'falls',lines:['Damage dealt '+fmtNum(fs.dmg)+'  ·  taps '+fs.taps,'Gold '+fmtNum(fs.gold)],t:0};if(Z.endless){const p0=G.prog[G.zone],m=p0/100;G.run.endless=Math.max(G.run.endless||0,p0);if(endlessMilestone(p0)&&m>(G.endMilestone||0)){G.endMilestone=m;for(const h of G.roster)refreshStats(h);const L=endlessLv(Z,p0),dust=3+2*m,gold=R((3+L*2)*Math.pow(1.05,L)*30*goldMult());G.dust+=dust;G.gold+=gold;let gear='',rvIt=null;const sk2=milestoneItem(m);if(sk2&&(SUPER_CAP[sk2]==null||superCount(sk2)<SUPER_CAP[sk2])){const it=makeSuper(sk2);giveItem(it);G.stats.items++;gear=itemName(it)+' (Mythic)';rvIt=it;}else if(G.pack.length<15){const it=makeItem(G.zone,'boss');G.pack.push(it);G.stats.items++;gear=itemName(it);}G.card={title:'Milestone '+m,sub:'Fight '+p0+' on the Endless Road',lines:['+'+dust+' crystal dust  ·  +'+fmtNum(gold)+' gold','+'+fmtNum(G.lastRenown||0)+' renown  ·  '+fmtNum(G.renown||0)+' total, +'+f1((renownStat()-1)*100)+'% stats',gear?'Found: '+gear:'Pack full - no chest'],t:0};banner('Milestone '+m,'the road goes on',3);if(rvIt){G.reveal={it:rvIt,t:0,card:G.card};G.card=null;sfx('level');}}}}
    ...
    const restHeal=restHealPct();for(const h of G.active){h.status={};if(h.dead){h.dead=false;h.hp=R(h.maxhp*0.3);}else h.hp=Math.min(h.maxhp,h.hp+R(h.maxhp*restHeal));}
    if(Z.endless){const p0=G.prog[G.zone],kind=endlessMilestone(p0)?'milestone':(p0>0&&p0%25===0)?'champion':'fight';const rg=renownGain(p0,kind);G.renown=(G.renown||0)+rg;G.lastRenown=rg;if(kind!=='fight'){float(W/2,GROUND-64,'+'+fmtNum(rg)+' renown','#8fdcff',true);}if(kind==='champion'&&!(G.omenMarks||[]).includes(p0)){G.omenMarks=(G.omenMarks||[]).concat([p0]);offerOmens();if(G.omenOffer)setTimeout(()=>{if(G.omenOffer&&!G.sheet&&!G.reveal)openSheet('omen');},1200);}if(kind==='milestone'&&(G.omens||[]).length){G.omens=[];toast('The omens fade with the milestone');}if(G.wins%10===0)for(const h of G.roster)refreshStats(h);}
    if(!G.cleared[G.zone]){G.prog[G.zone]++;if(Z.endless)G.run.endless=Math.max(G.run.endless||0,G.prog[G.zone]);if(Z.endless&&G.prog[G.zone]%30===0)banner(ZONES[endSeg(G.prog[G.zone])].name,'The road winds on',2.4);G.far=G.far||[];const farBefore=G.far[G.zone]||0;G.far[G.zone]=Math.max(farBefore,G.prog[G.zone]);{const cs=Z.fights/3,p2=G.prog[G.zone];if(p2%cs===0&&p2<Z.fights){G.cpHint=false;if(!Z.endless&&p2>farBefore){for(const h of G.active){h.dead=false;h.hp=h.maxhp;h.status={};}toast('New checkpoint - the party is fully healed',undefined,'road');}const cn=p2/cs,L2=Z.endless?endlessLv(Z,p2):R(Z.lv+p2*0.3),g2=R((3+L2*2)*Math.pow(1.05,L2)*(5+5*cn)*goldMult());G.gold+=g2;G.card={title:'Checkpoint '+cn+' of 3',sub:'The road behind you is safe',lines:['+'+fmtNum(g2)+' gold',cn===1?'Fall in battle and you return here':cn===2?'A champion blocks the way ahead':'The boss waits at the end of the road'],t:0};}}
      if(G.prog[G.zone]>=Z.fights){G.cleared[G.zone]=true;G.bossStreak=0;if(G.keepPushing)delete G.keepPushing[G.zone];if(G.train&&!G.train.active)G.train=null;grantShardVillagers();if(G.zone<ZONES.length-1)G.newZone=true;if(G.zone===3)G.reforgeNew=true;if(G.zone===0&&!G.vaelSeen)G.vaelNew=true;if(G.zone===0&&!(G.reforges>0)&&!G.firstBossHinted){G.firstBossHinted=true;G.firstBossHint=true;}banner('Crystal shard recovered!',G.zone<ZONES.length-1?ZONES[G.zone+1].name+' lies ahead':'The road is whole again',3.5);
    ...
    G.losses=(G.losses||0)+1;if(G.zone===0&&!G.gearHinted&&!(G.reforges>0)&&!G.roster.some(h=>['weapon','cape','charm'].some(sl=>h.eq[sl]))&&!Object.keys(G.tree||{}).some(k=>G.tree[k]>0)&&!G.pack.some(it=>it.lvl>0)&&!G.roster.some(h=>(h.abLvl||1)>1)){G.gearHinted=true;G.gearHint=true;}const cs2=ZONES[G.zone].fights/3,atCp=Math.floor(G.prog[G.zone]/cs2);if(G.losses>=3&&!G.cpHinted&&atCp>0){G.cpHinted=true;G.cpHint=true;tips.unshift('Tap an earlier checkpoint on the road to farm it');}
...
function onEndless(){const Z=ZONES[G.zone];return !!(Z&&Z.endless);}
function omenMult(k){if(!onEndless())return 1;let m=1;for(const id of (G.omens||[])){const o=OMENS[id];if(o&&o[k])m*=o[k];}return m;}
function omenCount(k){if(!onEndless())return 0;let n=0;for(const id of (G.omens||[])){const o=OMENS[id];if(o&&o[k])n+=o[k];}return n;}
function offerOmens(){const have=G.omens||[];const pool=Object.keys(OMENS).filter(k=>!have.includes(k));const pick=[];while(pick.length<3&&pool.length){const i=Math.floor(rand()*pool.length);pick.push(pool.splice(i,1)[0]);}G.omenOffer=pick.length?pick:null;}
...
function doReforge(){const gain=reforgeGain();G.dust+=gain;G.reforges=(G.reforges||0)+1;const startLv=1+3*treeLv('b_start');
  ...
  for(const h of G.active)setAnim(h,'walk');layout();G.screen='road';closeSheet();banner('The Crystal is shattered','+'+gain+' crystal dust · run '+(G.reforges+1),4,'any');saveGame();}
...
function questEvent(h){const L=h.lvl,ev=[];const gq=()=>R((30+L*8)*goldMult());
  ...
  ev.push(()=>{h.xp+=R(xpNeed(h.lvl)*0.25*xpMult());return 'Learned from the road: +25% of a level';});
  ev.push(()=>{h.hp=Math.max(1,R(h.maxhp*0.3));h.xp+=R(xpNeed(h.lvl)*0.15*xpMult());return 'Ambushed on the way back: wounded, +15% of a level';});
...
function collectQuest(q){const mult=(1+0.06*treeLv('quest'))*questBonus(q.hero,q.q);const L=q.hero.lvl;let msg='';
  if(q.q.reward==='gold'){const g=R((30+L*8)*mult*goldMult());G.gold+=g;msg='+'+fmtNum(g)+' gold';}
  else if(q.q.reward==='ore'){const o=R((8+L)*mult*oreMult());G.ore+=o;msg='+'+fmtNum(o)+' ore';if(rand()<0.4&&G.pack.length<15){G.pack.push(makeItem(G.zone));msg+=' and gear';}}
  ...
  else{const d=R(2*mult*refMult());G.dust+=d;msg='+'+d+' crystal dust';}
  q.hero.xp+=R(20*L*xpMult());G.quests=G.quests.filter(x=>x!==q);const ev=rand()<0.35?questEvent(q.hero):null;if(ev)G.card={title:q.hero.name+' returns',sub:msg,lines:[ev],t:0};toast(q.hero.name+' returns: '+msg+(ev?' · '+ev:''));}
...
// ============================================================ gear
function equipBest(only){AUTO_EQ=true;try{return equipBest0(only);}finally{AUTO_EQ=false;}}
function equipBest0(only){for(const h of (only?[only]:G.roster)){for(const slot of ['weapon','cape','charm']){const cands=G.pack.filter(it=>it.slot===slot&&(slot!=='weapon'||it.kind===CLASSES[h.cls].weapon));const cur=h.eq[slot];const score=it=>itemStat(it)+it.rank*0.01;const best=cands.sort((a,b)=>score(b)-score(a))[0];
...
function upgradeItem(it,h){let n=0,want=G.mult==='max'?1e9:G.mult;while(n<want){const cost=upgradeCost(it);if(G.ore<cost)break;G.ore-=cost;it.lvl++;n++;}if(!n){toast('Need '+fmtNum(upgradeCost(it))+' ore');return;}if(h)refreshStats(h);toast(itemName(it)+' Lv '+it.lvl+(n>1?' (+'+n+' levels)':''));}
function multLabel(){return G.mult==='max'?'Max':'×'+G.mult;}
function cycleMult(){G.mult=G.mult===1?5:G.mult===5?25:G.mult===25?'max':1;}

// ============================================================ main loop
let last=0;const hits=[];let HIT_DY=0;let HOLD=null,LAST_HOLD=null;
function endHold(){if(HOLD&&HOLD.r)LAST_HOLD={key:HOLD.r.x+','+HOLD.r.dy+','+HOLD.r.w,toastText:HOLD.toastText,end:RT};HOLD=null;}
...
function spendCall(fn,x,y,r){const g0=G.gold,o0=G.ore,d0=G.dust;G.toastKey=r?r.x+','+r.dy+','+r.w+','+r.h:null;try{fn();}finally{G.toastKey=null;}if(G.gold<g0||G.ore<o0||G.dust<d0){sfx('coin',true);if(r){const P=G.uparts||(G.uparts=[]),cx=x,cy=y;for(let k=0;k<12;k++){const a=-Math.PI/2+(rand()-0.5)*Math.PI*0.9,sp=60+rand()*70;P.push({x:cx+(rand()*8-4),y:cy+(rand()*4-2),vx:Math.cos(a)*sp,vy:Math.sin(a)*sp,life:0.5+rand()*0.4,col:[C.goldL,C.gold,'#fff','#ffb36b'][k%4],big:true});}}else uiSparks(x,y,8);}}
...
function update(dt,ui=true){if(G.mode==='battle')dt*=BATTLE_SPEED;else if(G.mode==='walk')dt*=MARCH_SPEED;G.t+=dt;if(G.shake>0)G.shake-=dt;
  ...
  for(const f of G.floats){f.life-=dt;f.y-=18*dt;}G.floats=G.floats.filter(f=>f.life>0);if(ui){for(const p of (G.uparts||[])){p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=60*dt;}if(G.uparts)G.uparts=G.uparts.filter(p=>p.life>0);}for(const p of G.parts){p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=40*dt;}G.parts=G.parts.filter(p=>p.life>0);
  ...
  if(!G.speedHinted&&!G.speedUsed&&G.campfireDone&&!(G.reforges>0)&&!G.cleared[0]&&!G.pending&&!G.banner&&G.tut==null&&G.mode==='walk'&&G.screen==='road'&&!G.sheet&&!G.title){G.speedHinted=true;saveGame();G.tut=TUT.length;TUT.push({t:'Speed up the road',s:'Tap the speed button to speed up marching and fighting.',font:'b',box:[W-36,SCENE_Y+4,32,18],once:true});}
  if(G.firstBossHint&&G.tut==null&&G.mode==='walk'&&G.screen==='road'&&!G.sheet&&!G.pending&&!G.banner&&!G.title){G.firstBossHint=false;saveGame();G.tut=TUT.length;TUT.push({t:'The next map is open',s:'Upgrade before you push on, or farm here to grow stronger first.',font:'b',box:[4,SCENE_Y+4,166,42],once:true});}
  if(G.gearHint&&G.tut==null&&G.mode==='walk'&&G.screen==='road'&&!G.sheet){G.gearHint=false;G.tut=TUT.length;TUT.push({t:'Strengthen the party',s:'Make sure to equip gear, rank up abilities, and grow the Tree.',font:'b',tabs:['heroes','gear','tree'],box:[0,TABY,W,42],once:true});}
  if(G.cpHint&&G.tut==null&&G.mode==='walk'&&G.screen==='road'&&!G.sheet){G.cpHint=false;G.tut=TUT.length;TUT.push({t:'Fall back to a checkpoint',s:'The road ahead is too strong for now. Tap an earlier dot to return to that checkpoint and farm it until the party is stronger.',s2:'A loss to the boss sends you to the last checkpoint. Lose to it again in a row and you fall one checkpoint further.',box:[4,SCENE_Y+4,166,42],once:true});}
  if(G.migMsg&&!G.title&&G.tut==null){toast(G.migMsg);G.migMsg=null;saveGame();}
  ...
  if(G.mode==='enter'){let all2=true;for(const e of G.enemies){if(e.x>e.slot){e.x=Math.max(e.slot,e.x-70*dt);all2=false;}else if(e.anim==='walk'&&!e.fly)setAnim(e,'idle');}if(all2){G.mode='battle';if(G.hordeFight&&G.hordeFight.volley){G.hordeFight.volley=false;G.hordeFight.volleyAt=RT+0.4;}G.enemies.forEach(e=>setAnim(e,'idle'));}}
    ...
    if(!G.action){for(const u of all)if(!u.dead)if(!(G.hordeFight&&G.banner))u.gauge+=u.spd*dt*7*(u.enemy?1:actionSpeedMult())*omenMult('speed');
...
function drawTabs(){px(0,TABY,W,42,C.out);px(0,TABY,W,1,C.gold);px(0,TABY+1,W,1,C.goldD);
  ...
  tabs.forEach(([ic,label,id],i)=>{const x=i*54,on=G.screen===id||(id==='road'&&G.screen==='map'),lk=tabLocked(id);
    ...
    if(id==='tree'&&G.treeNew&&!on&&G.tut==null){ctx.globalAlpha=0.5+0.5*Math.sin(RT*5);px(x+2,TABY+3,50,1,C.goldL);px(x+2,TABY+39,50,1,C.goldL);px(x+38,TABY+6,4,4,C.goldL);ctx.globalAlpha=1;}
    if(id==='heroes'&&!on&&G.tut==null&&heroesReady()){ctx.globalAlpha=0.5+0.5*Math.sin(RT*3);px(x+2,TABY+3,50,1,C.goldL);px(x+2,TABY+39,50,1,C.goldL);ctx.globalAlpha=1;}
    if(id==='vael'&&G.vaelNew&&!on&&G.tut==null){ctx.globalAlpha=0.5+0.5*Math.sin(RT*5);px(x+2,TABY+3,50,1,C.goldL);px(x+2,TABY+39,50,1,C.goldL);px(x+38,TABY+6,4,4,C.goldL);ctx.globalAlpha=1;}text(x+27,TABY+24,label,'sb',lk?C.dimt:(on?C.goldL:C.muted),'center');hit(x,TABY,54,42,()=>{if(lk){toast(lockHint(id));return;}G.screen=id;G.selItem=null;G.salvage=false;if(id==='tree'){G.treeNew=false;G.treeSeen=true;}
...
function drawScene(){const Z=ZONES[G.zone];const key=G.hordeFight?'door':G.delve?(G.delve.floor>=12?'door':'cave'):((G.bossFight&&Z.bossScene)?Z.bossScene:zoneScene(Z,G.prog[G.zone]));const layers=SCENES[key];
  ...
  ctx.restore();
  if(!G.sheet)for(const f of G.floats){if(G.banner&&(G.banner.scope==='any'||G.screen==='road')){const by=G.screen==='vael'?92:SCENE_Y+120;if(f.y>by-12&&f.y<by+40)continue;}ctx.globalAlpha=1;text(R(f.x),R(f.y),f.text,f.big?'h':'sb',f.color,'center',Math.min(1,f.life*2));}
  ...
  const sp=speedNow();{ctx.globalAlpha=0.75;button(W-36,SCENE_Y+4,32,18,sp>1);ctx.globalAlpha=1;text(W-20,SCENE_Y+8.5,sp+'×','sb',sp>1?C.goldL:C.cream,'center');hit(W-36,SCENE_Y+4,32,18,()=>{G.speedUsed=true;if(G.speed===1)G.speed=2;else if(G.speed===2){if(G.speedUntil>now())G.speed=4;else openSheet('speedad');}else G.speed=1;});}
  if(G.hordeFight){ctx.globalAlpha=0.8;frame(4,SCENE_Y+4,150,36,'rgba(44,22,26,0.8)');ctx.globalAlpha=1;text(9,SCENE_Y+7,'Horde at the gate','sb','#ff9696');text(9,SCENE_Y+17,'Wave '+G.hordeFight.wave+' of 3 · '+hordeTier(),'xs','#c88080');button(98,SCENE_Y+25,50,13,false);text(123,SCENE_Y+27.5,'Fall back','xs',C.cream,'center');hit(98,SCENE_Y+25,50,13,()=>{toast('The party falls back from the gate');endHordeFight(false);});const hf=G.hordeFight;if(hf.mercs>0){const A=ATLAS.soldier;for(let i=0;i<hf.mercs;i++){const mx=[104,166,90,180][i]||104,my=GROUND-44-(i>=2?10:0),fr=Math.floor(RT*4+i)%A.anims.idle.n;ctx.save();ctx.globalAlpha=0.95;drawSprite({uid:'soldier'},mx,my,fr,'idle',i%2===1,1);ctx.restore();}}if(hf.gateMax>0){const gx=176,gy=SCENE_Y+34;px(gx-2,gy-4,90,18,'rgba(0,0,0,0.55)');bar(gx,gy,86,6,hf.gate/hf.gateMax,hf.gate>0?'#9ad0ff':'#553');text(gx+43,gy+7,hf.gate>0?'Gate '+Math.round(100*hf.gate/hf.gateMax)+'%':'Gate breached','xs',hf.gate>0?'#cfe8ff':'#ff8a80','center');}}else if(G.delve){drawDelveOverlay();}else{
  // zone plate
  ...
  text(9,SCENE_Y+7,Z.endless?'Endless · '+ZONES[endSeg(G.prog[G.zone])].name:Z.name,'sb',C.goldL);{const hl=hordeLeft(),due=G.castle.hordeDue;text(9,SCENE_Y+17,due?'Horde at the gate - tap Vael to defend':(hl<900&&(G.castle.b.walls||G.castle.vil.total)?'Horde in '+fmtT(hl)+'  ·  Lv '+Z.lv:(Z.endless?'Fight '+(G.prog[G.zone]+1)+' · Lv '+endlessLv(Z,G.prog[G.zone])+' · best '+Math.max(G.prog[G.zone],(G.far||[])[G.zone]||0)+(G.renown?' · renown '+fmtNum(G.renown):''):(G.train&&G.train.active&&G.train.zone===G.zone?'Training for '+ZONES[G.train.ret].name+' · '+Math.min(99,R(100*Math.max(0,partyProg()-G.train.startProg)/(G.train.target||1)))+'% of '+((G.train.target||1)>1?(G.train.target)+' levels':'a level'):(G.keepPushing&&G.keepPushing[G.zone]?'Danger Lv '+Z.lv+'  ·  pushing on':'Danger Lv '+Z.lv+(G.cleared[G.zone]?'  ·  Shard recovered':''))))),'xs',due?'#ff8a80':(hl<900&&(G.castle.b.walls||G.castle.vil.total)?'#ffb080':C.muted));}hit(4,SCENE_Y+4,166,24,()=>{G.screen='map';G.newZone=false;G.reforgeNew=false;});if(G.train&&G.train.active&&G.train.zone===G.zone&&!Z.endless){button(172,SCENE_Y+26,60,16,true);text(202,SCENE_Y+30,'Keep pushing','xs',C.goldL,'center');hit(172,SCENE_Y+26,60,16,keepPushing);}
  {const lx=14,rx=156,ly=SCENE_Y+35;if(Z.endless){const p9=G.prog[G.zone],nx=25-(p9%25);const nm=100-(p9%100);text(14,ly-4,'Champion in '+nx+'  ·  milestone in '+nm+((G.omens||[]).length?'  ·  omens '+G.omens.length:''),'xs',C.muted);}else{px(lx,ly,rx-lx,1,'#3a4260');px(lx,ly,R((rx-lx)*far/Z.fights),1,'#5a6488');px(lx,ly,R((rx-lx)*p/Z.fights),1,G.cleared[G.zone]?'#7fd4ff':C.goldL);
    for(let k=0;k<=cpN;k++){const dx=R(lx+(rx-lx)*k/cpN),reached=far>=k*cpS,isBoss=k===cpN;
      ...
      const target=k<cpN?k*cpS:Z.fights-1;if(reached&&target!==p)hit(dx-15,ly-14,30,28,()=>{G.cpHint=false;if(k>=cpN-1)G.bossStreak=0;G.prog[G.zone]=target;G.enemies=[];G.projs=[];G.action=null;G.bossFight=false;G.mode='walk';G.enc=2;for(const h of G.active){h.dx=0;h.status={};setAnim(h,'walk');}toast(k<cpN?'Back to checkpoint '+k:'Onward to the boss');});}
...
function drawCard(){const c=G.card;if(!c||G.screen!=='road'||G.tut!=null||G.sheet)return;const al=clamp(Math.min(c.t*3,(4.2-c.t)*2),0,1);const y0=SCENE_Y+40,h=70+c.lines.length*11;ctx.globalAlpha=al;frame(20,y0,230,h,C.navy,C.goldL);ctx.globalAlpha=1;tx.clearRect(20*TS,y0*TS,230*TS,h*TS);
...
function drawCards(){px(0,16,W,80,'#0e111e');px(0,94,W,2,C.goldD);
  for(let i=0;i<4;i++){const x=4+i*66;const h=G.active[i];
    if(!h&&i>=partyMax()){frame(x,18,62,74,'#10131f',C.goldD,false);ctx.globalAlpha=0.25;icon('lock',x+25,32,0);ctx.globalAlpha=1;text(x+31,52,'Sealed by','xs',C.dimt,'center');text(x+31,61,'the Oath','xs',C.dimt,'center');continue;}
...
function drawGear(){drawPanelScreen();const h=G.roster[G.gearHero]||G.roster[0];
  ...
  ['weapon','cape','charm'].forEach((slot,i)=>{const x=4+i*88,it=h.eq[slot];frame(x,SY,86,SLOTH,C.navy,it&&it.rank>0?RANKHEX[it.rank]:C.gold);
    ...
    if(it){drawItemIcon(it,x+31,SY+14,2);const nm=itemName(it);text(x+43,SY+43,nm,textW(nm,'sb')>78?'xsb':'sb',it.rank>0?RANKHEX[it.rank]:C.cream,'center');text(x+43,SY+53,(it.rank>=5?'MYTHIC':'★'.repeat(it.rank+1)+'☆'.repeat(4-it.rank))+(it.lvl?'  Lv '+Math.round(roll('it_'+it.id,it.lvl,0.03)):''),'xs',C.goldL,'center');text(x+43,SY+61,itemStatLabel(it).replace(fmtNum(itemStat(it)),fmtNum(roll('is_'+it.id,itemStat(it),0.03))),'xs',C.green,'center');
      const cost=upgradeCost(it),bk=bulkCost(l=>R(3*Math.pow(2.2,it.rank)*Math.pow(1.14,l)*(built('forge')?0.85:1)),it.lvl,G.ore),can=G.ore>=cost;const ub=BIG?22:16,uy=SY+SLOTH-ub-4;button(x+6,uy,74,ub,can);icon('ore',x+10,uy+ub/2-6,0);textA(x+49,x+23,x+78,uy+ub/2-4,G.mult==='max'?fmtNum(bk.total)+' ('+bk.n+')':'Upgrade '+fmtNum(bk.total),G.mult==='max'?'0.00K (00)':'Upgrade 000','xs',can?C.goldL:C.muted,fmtNum(bk.total));hit(x+6,uy,74,ub,()=>upgradeItem(it,h),true);
      if(it.rank<4){const locked=it.rank>=ascendCap(),ac2=ascendCost(it),acan=!locked&&G.ore>=ac2,ay=uy-ub-3;button(x+6,ay,74,ub,acan,locked);if(locked)icon('lock',x+10,ay+ub/2-6,0);text(x+43+(locked?6:0),ay+ub/2-4,locked?'Shatter to ascend':'Ascend '+fmtNum(ac2),'xs',locked?C.dimt:(acan?RANKHEX[it.rank+1]:C.muted),'center');hit(x+6,ay,74,ub,()=>ascendItem(it,h));}
  ...
  for(let k=0;k<15;k++){const cx=k%5,cy=Math.floor(k/5),x=gx+cx*step,y=gy+cy*step,it=G.pack[k];
    ...
    if(G.salvage&&on)icon('check',x+CELL-14,y+2,4);else if(it.isNew)px(x+CELL-9,y+4,5,5,'#ff5050');if(it.lvl)text(x+CELL-3,y+3,'Lv'+it.lvl,'xs',C.goldL,'right');if(usable)text(x+3,y+CELL-10,itemStatLabel(it),'xs',C.green);
...
function drawHeroes(){drawPanelScreen();text(8,26,'Heroes','title',C.goldL);text(262,30,G.roster.length+' recruited','xs',C.muted,'right');beginScroll('heroes',44);
  let y=46;const HR=BIG?110:98;for(const h of G.roster){frame(4,y,262,HR);{const fx0=tierFx(h);if(fx0.aura){const g=ctx.createRadialGradient(28,y+26,2,28,y+26,20);g.addColorStop(0,'rgba(255,120,60,0.35)');g.addColorStop(1,'rgba(255,120,60,0)');ctx.fillStyle=g;ctx.fillRect(8,y+6,40,40);}if(fx0.ghost)ctx.globalAlpha=0.75+0.2*Math.sin(RT*7);drawSprite(h,26,y+38,0,'idle',false);ctx.globalAlpha=1;}{const fx=tierFx(h);ctx.save();ctx.beginPath();ctx.rect(5,y+1,60,HR-2);ctx.clip();drawTierExtras(h,26,y+38,1,Object.assign({},fx,{wglow:null}),false);if(fx.halo){const top=y+38-(ATLAS[h.uid].oy-headRow(h));ctx.strokeStyle='rgba(255,240,180,'+(0.6+0.3*Math.sin(RT*3))+')';ctx.lineWidth=1.2;ctx.beginPath();ctx.ellipse(28,top-3,6,2,0,0,Math.PI*2);ctx.stroke();}ctx.restore();}const st=heroStatus(h);
    ...
    const ab=CLASSES[h.cls].ab;icon(ab.icon,46,y+27,2);text(62,y+28,ab.name,'xsb',C.goldL);text(62+textW(ab.name,'xsb')+6,y+28,'rank '+h.abLvl,'xs',C.muted);{const d1=abilityDesc(h),parts=wrap2(d1,'xs',128);text(62,y+37,parts[0],'xs',C.muted);if(parts[1])text(62,y+46,parts[1],'xs',C.muted);drawAbilityNums(h,62,y+37,parts);}if((h.tier||0)<3){const q=promoteReq(h);text(62,y+56,fitText('Promote at Lv '+q.lvl+': '+fmtNum(q.gold)+' gold'+(q.dust?', '+q.dust+' dust':', no dust'),'xs',128),'xs',C.dimt);hit(62,y+53,128,12,()=>{const why=h.lvl<q.lvl?h.name+' can be promoted at Lv '+q.lvl:G.gold<q.gold?'Need '+fmtNum(q.gold)+' gold':G.dust<q.dust?'Need '+q.dust+' dust':'Ready to promote';toast(why+(q.dust?'':' · dust for this tier was already paid'));});}
    const cost=rankCost(h),can=G.gold>=cost;const HB=Math.floor((HR-34)/2);button(194,y+4,66,HB,can);icon('coin',198,y+4+HB/2-6,4);text(228,y+4+HB/2-9,'Rank up','xs',can?C.goldL:C.muted,'center');text(228,y+4+HB/2+1,fitText(fmtNum(cost),'xs',46),'xs',can?C.goldL:C.muted,'center');hit(194,y+4,66,HB,()=>{const c=rankCost(h);if(G.gold<c){toast('Need '+fmtNum(c)+' gold');return;}G.gold-=c;h.abLvl++;toast(ab.name+' rank '+h.abLvl);},true);
    {h.talents=h.talents||{};TALENTS[h.cls].forEach((t,i)=>{const own=!!h.talents[t.id],q=talentReq(i),x=12+i*62,ty=y+HR-24,lk=h.lvl<q.lvl,can=!own&&!lk&&G.gold>=q.gold;button(x,ty,58,20,own||can,lk);const lbl=own?t.name:(lk?'Lv '+q.lvl:t.name),two=(own&&lk)||(!own&&!lk);text(x+29,ty+(two?3:6),fitText(lbl,'xs',54),'xs',own?C.goldL:(lk?C.dimt:C.cream),'center');if(!own&&!lk)text(x+29,ty+11,(can?'buy ':'')+fmtNum(q.gold),'xs',can?C.goldL:C.dimt,'center');else if(own&&lk)text(x+29,ty+11,'wakes at Lv '+q.lvl,'xs',C.dimt,'center');hit(x,ty,58,20,()=>buyTalent(h,i));});}
    y+=HR+4;}
  if(G.roster.length<HEROES.length)text(W/2,y+6,'More will join along the road','xs',C.dimt,'center');endScroll('heroes',y+20);}
function drawMap(){drawPanelScreen();ctx.strokeStyle=C.goldL;ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(16,33,9,0,Math.PI*2);ctx.stroke();icon('back',10,27,4);text(32,26,'The Crystal Road','title',C.goldL);hit(0,16,110,34,()=>{G.screen='road';});const rb=BIG?26:18;const ok0=canReforge();button(170,22,92,rb,ok0);if(ok0){const ring=(c)=>{px(171,23,90,1,c);px(171,22+rb-2,90,1,c);px(171,23,1,rb-2,c);px(260,23,1,rb-2,c);};ring(C.goldD);ctx.globalAlpha=0.35+0.65*(0.5+0.5*Math.sin(RT*4));ring(C.goldL);ctx.globalAlpha=1;}icon('crystal',174,22+rb/2-6,2);text(220,22+rb/2-4,ok0?'Shatter +'+fmtNum(reforgeGain()):'Beat the Warlord','xsb',ok0?C.goldL:C.dimt,'center');if(ok0)hit(170,22,92,rb,()=>openSheet('shatter'));
  beginScroll('map',48);
  ZONES.forEach((z,i)=>{const y=52+i*72,ok=zoneUnlocked(i),cur=i===G.zone;frame(4,y,262,66,C.navy,cur?C.goldL:(ok?C.gold:C.goldD));
    ...
    text(108,y+30,ok?(z.endless?'Fight '+(G.prog[i]+1)+' · Lv '+endlessLv(z,G.prog[i])+' · best '+Math.max(G.prog[i],(G.far||[])[i]||0):G.cleared[i]?'Shard recovered':'Checkpoint '+Math.floor(Math.max(G.prog[i],(G.far||[])[i]||0)/(z.fights/3))+'/3 · boss: '+ENEMIES[z.boss].name):((G.reforges||0)<reqReforge(i)?'Needs '+reqReforge(i)+' Shatter'+(reqReforge(i)>1?'s':'')+(G.cleared[i-1]?'':' · beat the previous boss'):'Defeat the previous boss'),'xs',ok?C.goldL:C.dimt);
    if(ok&&!cur){button(180,y+42,76,16,true);text(218,y+45,'Travel','sb',C.goldL,'center');hit(180,y+42,76,16,()=>{if(G.train&&G.train.active)finishTraining(false);G.train=null;G.danger=null;if(G.keepPushing)delete G.keepPushing[G.zone];goZone(i);G.screen='road';banner(z.name,'Danger Lv '+z.lv);});}
    else if(cur)text(218,y+46,'You are here','xs',C.goldL,'center');});
  const ey=52+ZONES.length*72;text(W/2,ey,'Shatter keeps your heroes, talents, gear, gold, Vael,','xs',C.dimt,'center');text(W/2,ey+10,'the tree and blessings, and pays out crystal dust.','xs',C.dimt,'center');endScroll('map',ey+30);}
function drawTree(){drawPanelScreen();text(8,26,'Upgrade Tree','title',C.goldL);
  ...
  const top=42+(BIG?22:16)+4+22;SCROLLTOP.tree=top;const nRows=TREE.filter(n=>n.branch===G.treeTab).length+(G.treeTab==='Crystal'?1:0);const TRH=Math.max(22,Math.min(BIG?44:32,Math.floor((TABY-8-TABTOP-top)/nRows)-3));beginScroll('tree',top);let y=top+2;if(G.treeTab==='Crystal'){text(8,y,'Blessings are paid in crystal dust and kept through every Shatter.','xs','#bbdefb');y+=14;}for(const n of TREE){if(n.branch!==G.treeTab)continue;
    ...
    icon('nodes',10,y+RH/2-6,lv?4:0);const dl=Math.round(roll('tr_'+n.id,lv));const tight=RH<32;text(28,y+RH/2-(tight?9:12),n.name+(dl?'  rank '+dl:''),tight?'sb':'sb',C.cream);{const dl2=fitText(n.desc+nodeTotal(n,dl),'xs',162),dy=y+RH/2+(tight?0:-1);text(28,dy,dl2,'xs',C.muted);boldNums(dl2,28,dy);}
    const bk=max?null:bulkCost(l=>nodeCost(n,l),lv,isD?G.dust:G.gold);button(192,y+(RH-TB)/2,66,TB,can,max);if(!max)icon(isD?'crystal':'coin',197,y+RH/2-6,isD?2:4);if(max)text(225,y+RH/2-4,'Maxed','xsb',C.dimt,'center');else textA(231,211,256,y+RH/2-4,fmtNum(bk.total)+(G.mult==='max'&&bk.n>1?' ('+bk.n+')':''),G.mult==='max'?'0.00K (00)':'0.00K','xsb',can?C.goldL:C.muted);
    if(!max)hit(192,y+(RH-TB)/2,66,TB,()=>{let k=0,want=G.mult==='max'?1e9:G.mult;while(k<want&&buyTreeRank(n))k++;if(!k){if(treeLv(n.id)>=n.max)toast(n.name+' is maxed');else toast('Need '+fmtNum(nodeCost(n,treeLv(n.id)))+(isD?' crystal dust':' gold'));return;}for(const h of G.roster)refreshStats(h);toast(n.name+' rank '+treeLv(n.id));},true);y+=RH+3;}endScroll('tree',y);}
...
function mercLv(){const p=G.castle.prep||{};return p.mercLv||(p.merc?1:0);}
function mercDef(lv){return lv?R((8+castleLv()*2)*Math.pow(1.25,lv-1)):0;}
function mercVolley(lv){if(!lv)return 0;const base=Math.min(0.2,0.05+0.004*castleLv());return 0.75-(0.75-base)*Math.pow(0.96,lv-1);}
function defense(){const p=G.castle.prep||{};const w=(G.castle.b.walls*5+(built('watchtower')?15:0))*(p.walls?1.25:1);const g=garrisonHeroes().reduce((s,h)=>s+3+h.lvl*0.5,0)+mercDef(mercLv());return{walls:R(w),garrison:R(g),total:R(w+g)};}
const HORDE_MODS={armored:{name:'Armored',desc:'+25% enemy defense'},bloodthirsty:{name:'Bloodthirsty',desc:'+20% enemy attack'},shaman:{name:'Shaman',desc:'a shaman wards each wave'},swarming:{name:'Swarming',desc:'one more enemy per wave'},siege:{name:'Siege',desc:'the champion strikes fast and batters the gate'}};
...
function textA(cx,minX,maxR,y,str,tmpl,style,col,alt){let ax=Math.max(minX,R(cx-textW(tmpl,style)/2));let sw=textW(str,style);if(ax+sw>maxR)ax=Math.max(minX,R(maxR-sw));if(ax+sw>maxR&&alt!=null){str=alt;sw=textW(str,style);ax=Math.max(minX,R(cx-sw/2));}text(ax,y,fitText(str,style,maxR-ax),style,col);}
...
function hordeStrength(){const C=G.castle,n=C.hordeCount||0,streak=C.streak||0,rf=G.reforges||0;const base=n===0?4:8+n*4+Math.pow(n,1.25)*1.5;return R(base*(1+G.zone*0.04)*(1+rf*0.08)*(1+Math.min(0.3,streak*0.05))*(C.hordePressure||1));}
function castleTick(){const C2=G.castle,t=now();let dt=(t-C2.last)/HOUR*tscale();C2.last=t;if(dt<=0)return;dt=Math.min(dt,8);
  ...
  for(const h of garrisonHeroes()){h.xp+=garrisonXp(h)*xpMult()*dt;while(h.xp>=xpNeed(h.lvl)){h.xp-=xpNeed(h.lvl);h.lvl++;refreshStats(h);h.hp=h.maxhp;}}
  if(t>=C2.hordeAt&&!G.hordeFight){if(document.hidden||(t-C2.hordeAt)>180000)resolveHorde();else C2.hordeDue=true;}}
...
function hordeAfter(win){const C=G.castle;C.hordeCount=(C.hordeCount||0)+1;if(win)C.hordePressure=Math.min(1,(C.hordePressure||1)+0.03);else C.hordePressure=Math.max(0.8,(C.hordePressure||1)-0.1);C.prep={};C.hordeAt=now()+8*HOUR/tscale();genHorde();}
function resolveHorde(fought){const C2=G.castle,d=defense().total,s=hordeThreat();C2.hordeDue=false;if(fought!=null){if(fought){C2.repelled++;C2.streak=(C2.streak||0)+1;G.stats.hordes++;const HL=(G.hordeFight&&G.hordeFight.lvl)||(C2.lastHordeLv||1);const g=R((3+HL*2)*Math.pow(1.05,HL)*30*goldMult()),du=2+Math.floor(HL/40),v=2+Math.floor(rand()*2);G.gold+=g;G.dust+=du;C2.vil.total+=v;if(G.pack.length<15){const it=makeItem(G.zone,'boss');G.pack.push(it);G.stats.items++;}banner('The gate holds','+'+fmtNum(g)+' gold, +'+du+' dust, '+v+' villagers, a chest',3.5,'any');}else{C2.lost++;C2.streak=0;const lost=Math.min(2,C2.vil.total);C2.vil.total-=lost;banner('The gate falls',lost+' villagers lost',3.5,'any');}hordeAfter(!!fought);return;}
  if(d>=s){C2.repelled++;C2.streak=(C2.streak||0)+1;G.stats.hordes++;const HL=hordeCombatLevel();const g=R((3+HL*2)*Math.pow(1.05,HL)*15*goldMult()),v=1+Math.floor(rand()*2);G.gold+=g;G.dust+=1;C2.vil.total+=v;banner('The horde breaks on the walls','+'+fmtNum(g)+' gold, +1 dust, '+v+' villagers rally',3.5,'any');hordeAfter(true);}
    ...
    const lost=Math.min(2,C2.vil.total);C2.vil.total-=lost;C2.vil.mine=Math.min(C2.vil.mine,C2.vil.total);C2.vil.market=Math.min(C2.vil.market,Math.max(0,C2.vil.total-C2.vil.mine));banner('The horde overruns Vael',(msg?msg+' · ':'')+lost+' villagers lost',3.5,'any');hordeAfter(false);}
  }
function collectCastle(){const C2=G.castle,g=Math.floor(C2.stored.gold),o=Math.floor(C2.stored.ore);if(!g&&!o){toast('Nothing to collect yet');return;}G.gold+=g;G.ore+=o;C2.stored.gold-=g;C2.stored.ore-=o;toast('Collected '+fmtNum(g)+' gold and '+fmtNum(o)+' ore from Vael');}
...
function postHero(h){const C2=G.castle;if(heroStatus(h)!=='camp'){toast(h.name+' is '+(heroStatus(h)==='marching'?'on the road':heroStatus(h)==='quest'?'away on a quest':'already posted'));return;}if(C2.garrison.length>=garrisonSlots()){toast('No free posts - raise the walls');return;}C2.garrison.push(h.id);h.postedAt=now();toast(h.name+' takes a post on the walls for at least an hour');}
function postLeft(h){return Math.max(0,((h.postedAt||0)+HOUR/tscale()-now())/1000);}
function recallHero(h){if(postLeft(h)>0){toast(h.name+' holds the post for '+fmtT(postLeft(h))+' more');return;}G.castle.garrison=G.castle.garrison.filter(id=>id!==h.id);toast(h.name+' stands down');}
...
const VAEL_Y=176;
function drawVael(){drawPanelScreen();castleTick();const C2=G.castle,D=defense(),S=hordeThreat();if(!C2.currentHorde)genHorde();
  ...
  const safe=D.total>=S;const pw=80,px1=98,px2=184,py=18,ph=35,sy=py+20,sh=12;const plate=(x,fill,line)=>{px(x,py,pw,ph,C.out);px(x+1,py+1,pw-2,ph-2,line);px(x+2,py+2,pw-4,ph-4,fill);};plate(px1,safe?'#14321e':'#3c1e1e',safe?'#5ab46e':'#c85050');text(px1+pw/2,py+3,'Defense '+fmtNum(D.total),'sb',safe?'#96e6aa':'#ff9696','center');{const wa=fmtNum(D.walls),ga=fmtNum(D.garrison);const t2=(D.walls>=1000||D.garrison>=1000)?'W '+wa+' · G '+ga:'walls '+wa+' · garrison '+ga;text(px1+pw/2,py+12,t2,'xs',safe?'#78aa82':'#c88080','center');}button(px1+4,sy,pw-8,sh,false);text(px1+pw/2,sy+sh/2-4,'Garrison','xs',C.cream,'center');hit(px1,py,pw,ph,()=>openSheet('garrison'));
  ...
  for(const cd of cards){frame(4,y0,262,VCH);px(10,y0+7,32,32,C.out);px(11,y0+8,30,30,'#222844');cd.ic();
    ...
    else{const cost=buildCost(cd.k),can=G.gold>=cost;button(190,y0+6,70,VB,can);icon('coin',193,y0+6+VB/2-6,4);textA(233,207,258,y0+6+VB/2-4,(lv?'Upgrade ':'Build ')+fmtNum(cost),(lv?'Upgrade ':'Build ')+'0.00K','xs',can?C.goldL:C.muted,fmtNum(cost));
      hit(190,y0+6,70,VB,()=>{if(G.gold<cost){toast('Need '+fmtNum(cost)+' gold');return;}G.gold-=cost;C2.b[cd.k]++;toast(BUILD[cd.k].name+(C2.b[cd.k]===1?' built':' raised to Lv '+C2.b[cd.k]));},true);
...
function hordeWindow(){const C2=G.castle,t=now(),span=8*HOUR/tscale();C2.hordeLog=(C2.hordeLog||[]).filter(x=>t-x<span);return{used:C2.hordeLog.length,next:C2.hordeLog.length?Math.max(0,(C2.hordeLog[0]+span-t)/1000):0};}
function startHordeFight(){if(G.delve){toast('Finish the catacombs first');return;}{const hw=hordeWindow();if(hw.used>=2){toast('The horde needs time to gather - next in '+fmtT(hw.next));return;}G.castle.hordeLog.push(now());}const C2=G.castle,S=hordeStrength(),Z=ZONES[ZONES[G.zone].endless?endSeg(G.prog[G.zone]):G.zone];const gar=garrisonHeroes().filter(h=>!G.active.includes(h));
  if(!C2.currentHorde)genHorde();const hordeLv=hordeCombatLevel();G.hordeFight={wave:0,saved:{active:G.active.slice(),zone:G.zone,mode:G.mode,prog:G.prog.slice()},lvl:hordeLv,pool:Z.pool,boss:Z.boss,waves:hordeWaves(),tier:hordeTier(),mod:(C2.currentHorde||{}).mod||null};
  G.active=G.active.concat(gar).slice(0,6);for(const h of G.active){h.dead=false;h.status={};if(h.hp<h.maxhp*0.5)h.hp=R(h.maxhp*0.5);}layout();{const D=defense(),p=C2.prep||{};const partyHp=G.active.reduce((a,h)=>a+h.maxhp,0);const ml=mercLv();const frac=Math.min(3,D.total/120);G.hordeFight.gateMax=R(partyHp*frac);G.hordeFight.gate=G.hordeFight.gateMax;G.hordeFight.mercs=ml?Math.min(4,1+ml):0;G.hordeFight.mercLv=ml;}G.active.forEach(h=>{h.x=h.tx;});C2.hordeDue=false;G.enemies=[];G.projs=[];G.action=null;G.screen='road';closeSheet();sfx('cast');banner('The horde is at the gate','Wave 1 of 3',2.5);hordeWave();}
function hordeWave(){const hf=G.hordeFight;hf.wave++;const wv=hf.waves||[2,2,1];const n=Math.min(3,hf.wave===3?1+wv[2]:wv[hf.wave-1]);G.enemies=[];const tierRank=['Rabble','Raiders','Warband','Horde','Legion'].indexOf(hf.tier||'Rabble');
  for(let i=0;i<n;i++){const boss=hf.wave===3&&i===0;const id=boss?hf.boss:hf.pool[Math.floor(rand()*hf.pool.length)];const E=ENEMIES[id];const e={uid:E.uid||id,id,name:(boss?'Champion ':'')+E.name,lvl:hf.lvl,x:W+40+i*36,slot:boss?210:ENEMY_X[i],dx:0,yoff:boss?0:ENEMY_Y[i],flip:true,fly:!!E.fly,range:E.range,boss:false,scale:boss?Math.min(E.scale||2,2.5):1,native:!!E.native,nscale:E.nscale||1,hue:E.hue||0,tier:hf.wave-1,gauge:rand()*40,dead:false,enemy:true,status:{},flash:0,traits:(E.traits||[]).slice(),acts:0};Object.assign(e,enemyStats(id,hf.lvl));if(boss){e.maxhp=R(e.maxhp*2);e.traits.push('charge');e.champ=true;if(hf.mod==='siege')e.spd=R(e.spd*1.3);}
    if(hf.mod==='armored')e.def=R(e.def*1.25);if(hf.mod==='bloodthirsty')e.atk=R(e.atk*1.2);if(hf.mod==='shaman'&&!boss&&i===n-1&&!e.traits.includes('shieldAllies')){e.traits.push('shieldAllies');e.name='Shaman '+E.name;}
...
function delveEnemy(kind){const d=G.delve,sc=delveScale();const Z=ZONES[Math.min(ZONES.length-1,Math.floor(d.floor/4))];const pool=Z.pool;const id=kind==='boss'?lordFor(d.floor):kind==='elite'?ZONES[Math.min(ZONES.length-1,Math.floor(d.floor/4))].boss:pool[Math.floor(rand()*pool.length)];
  ...
  Object.assign(e,enemyStats(id,sc.lvl));if(kind==='elite'){e.maxhp=R(e.maxhp*1.6);e.atk=R(e.atk*1.2);}if(kind==='boss'){e.name=E.name;e.boss=true;e.traits=(E.traits||[]).slice();e.mech=E.mech?JSON.parse(JSON.stringify(E.mech)):null;if(e.mech&&e.mech.shieldAllies)e.traits.push('shieldAllies');e.acts=0;e.maxhp=R(e.maxhp*1.6);e.slot=E.native?226:200+8*(E.scale||1);}const idol=d.relics.find(r=>r.ehp);if(idol)e.maxhp=R(e.maxhp*idol.ehp);e.hp=e.maxhp;e.scale=kind==='elite'?2:kind==='boss'?Math.min(E.scale||2.5,2.5):1;setAnim(e,'walk');return e;}
function chooseDoor(kind){const d=G.delve;if(!d||!d.choose)return;d.choose=false;const h=d.hero,sc=delveScale();
  ...
  if(kind==='chest'){if(rand()<0.6){const it=makeItem(Math.min(ZONES.length-1,Math.floor(d.floor/4)),'boss');d.bag.items.push(it);toast('Chest: '+itemName(it),RANKHEX[it.rank]);}else{const o=R((6+d.floor*3)*sc.mult*oreMult());d.bag.ore+=o;toast('Chest: +'+fmtNum(o)+' ore');}}
  ...
  nextFloor();}
function delveWin(){const d=G.delve,sc=delveScale();const gold=R((20+d.floor*10)*sc.mult*(d.relics.find(r=>r.gold)?1.5:1)*goldMult());d.bag.gold+=gold;float(W/2,SCENE_Y+40,'+'+fmtNum(gold)+' gold',C.goldL,true);
  ...
  if(G.delveFight==='boss'){for(let k=0;k<2;k++){const it=makeItem(Math.min(ZONES.length-1,2+Math.floor(d.floor/4)),'boss');d.bag.items.push(it);}const others=RELICS.filter(x=>!d.relics.includes(x));if(others.length&&d.relics.length<3){const nr=others[Math.floor(rand()*others.length)];d.relics.push(nr);refreshDelveStats();}const lb=G.enemies.find(x=>x.boss);toast((lb?lb.name:'The Catacomb Lord')+' falls - two items and a relic',C.goldL);}
  G.enemies=[];G.mode='delve';G.delve.nextFloorAt=RT+0.6;}
function endDelve(died,fled){const d=G.delve;if(!d)return;const h=d.hero;const keep=died?0.6:fled?0.8:1;const gold=R(d.bag.gold*keep),ore=R(d.bag.ore*keep);G.gold+=gold;G.ore+=ore;let kept=0;for(const it of d.bag.items){if(!died||rand()<0.6){if(G.pack.length<15){G.pack.push(it);kept++;}else{G.ore+=salvageBase(it.rank);}}}
...
function grabTreasure(){const t=G.treasure;if(!t)return;const tx0=treasureX()+14;G.treasure=null;const Z=ZONES[G.zone],L=R(Z.lv+Math.min(G.prog[G.zone],Z.fights)*0.3);sfx('coin');uiSparks(tx0,t.y-6,16);
  ...
  else if(t.kind==='ore'){const o=R(CHEST_ORE_MULT*(2+L*0.4)*(1+Math.floor(L/6))*oreScale(L)*oreMult()*zoneBonus('chest'));G.ore+=o;float(tx0,t.y-16,'+'+fmtNum(o)+' ore',C.muted,true);}
  else{const g=R((5+L*3)*Math.pow(1.05,L)*goldMult()*zoneBonus('chest'));G.gold+=g;float(tx0,t.y-16,'+'+fmtNum(g)+' gold',C.goldL,true);}}
...
function offlineGains(hours,eff){const fights=Math.floor(hours*3600/22*eff);const Z=ZONES[G.zone],L=Z.endless?endlessLv(Z,G.prog[G.zone]):roadLv(Z,Z.lv+Math.min(G.prog[G.zone],Z.fights-1)*0.45);const per=1.8;
  return{fights,gold:R(fights*per*(3+2*L)*Math.pow(1.05,L)*goldMult()*zoneBonus()),xp:R(fights*per*(6+3*L)*Math.pow(1.04,L)*bless('xp')),ore:R(fights*ORE_DROP_CHANCE*1.5*(1+Math.floor(L/6))*oreScale(L)*oreMult()*zoneBonus())};}
function offlineReport(lastT){const elapsed=Math.max(0,(now()-lastT)/1000);if(elapsed<120)return;
  const hours=Math.min(8+0.5*treeLv('offline')+(built('shrine')?2:0),elapsed/3600),eff=(()=>{const bh=Math.max(0,Math.min(hours,(G.autoUntil-lastT)/3600000));return hours>0?(0.65*bh+0.5*(hours-bh))/hours:0.5;})();const g=offlineGains(hours,eff),fights=g.fights;if(fights<1)return;
  ...
  const items=[];const nItems=Math.min(3,Math.floor(fights*0.04));for(let i=0;i<nItems;i++){const it=makeItem(G.zone);if(G.autoSalvage&&it.rank<G.autoSalvage){G.ore+=salvageBase(it.rank);}else if(G.pack.length<15){G.pack.push(it);items.push(it);}else G.ore+=salvageBase(it.rank);}
...
function drawSheet(){const s=G.sheet;if(!s)return;tx.clearRect(0,0,tc.width,tc.height);dimRect(0,0,W,H,0.75);hit(0,0,W,H,()=>{if(s.kind!=='welcome')closeSheet();});
  ...
  if(s.kind==='swap'){text(16,y0+8,'Party',"h",C.goldL);text(16,y0+20,'Tap a marcher, then a hero at camp to trade places.','xs',C.muted);text(16,y0+29,'Two marchers to reorder. Left slot is the front.','xs',C.muted);
  ...
  else if(s.kind==='horde'){const C2=G.castle,p=C2.prep||(C2.prep={}),D=defense(),S=hordeThreat(),due=C2.hordeDue;text(16,y0+8,due?'The horde is at the gate':'The next horde','h',due?'#ff8a80':C.goldL);{const ch=C2.currentHorde||genHorde();text(16,y0+22,'#'+ch.number+' '+ch.tier+' · strength '+fmtNum(ch.threat)+(ch.mod?' · '+HORDE_MODS[ch.mod].name:'')+'   Defense '+fmtNum(D.total)+(D.total>=ch.threat?' - holds':' - falls'),'xs',D.total>=ch.threat?'#96e6aa':'#ff9696');}
    let y=y0+36;const rows=[['Shore up the walls','+25% wall defense, stronger gate',p.walls?null:prepCost('walls'),()=>{p.walls=true;}],[mercLv()?'Drill the mercenaries  ·  Lv '+mercLv():'Hire mercenaries',mercLv()?'volley '+f1(mercVolley(mercLv())*100)+'% → '+f1(mercVolley(mercLv()+1)*100)+'%, defense +'+fmtNum(mercDef(mercLv()))+' → +'+fmtNum(mercDef(mercLv()+1)):'+'+fmtNum(mercDef(1))+' defense, volley '+f1(mercVolley(1)*100)+'% each wave',R(prepCost('merc')*Math.pow(1.9,mercLv())),()=>{p.merc=true;p.mercLv=mercLv()+1;}]];
    for(const[t,sub,cost,fn]of rows){px(14,y,242,26,C.band);text(20,y+4,t,'sb',cost==null?C.goldL:C.cream);text(20,y+14,cost==null?'Prepared':sub,'xs',C.muted);if(cost!=null){const can=G.gold>=cost;button(196,y+5,54,16,can);text(223,y+8,fmtNum(cost),'xs',can?C.goldL:C.muted,'center');hit(196,y+5,54,16,()=>{if(G.gold<cost){toast('Need '+fmtNum(cost)+' gold');return;}G.gold-=cost;fn();},true);}y+=30;}
    {const hw=hordeWindow(),ok=hw.used<2;px(14,y,242,26,C.band);text(20,y+4,due?'Defend the gate':'Sound the horns','sb',ok?C.goldL:C.dimt);text(20,y+14,ok?(due?'Fight it now with your party and garrison for full rewards':'Call the horde early  ·  '+(2-hw.used)+' of 2 left this 8 hours'):'Two hordes fought already  ·  next in '+fmtT(hw.next),'xs',C.muted);button(196,y+5,54,16,ok,!ok);text(223,y+8,'Fight','xs',ok?C.goldL:C.dimt,'center');if(ok)hit(196,y+5,54,16,()=>{closeSheet();startHordeFight();});y+=30;}
  ...
  else if(s.kind==='omen'){const offer=G.omenOffer||[];text(16,y0+8,'A Road Omen','h',C.goldL);text(16,y0+22,'Choose one. It lasts until the next milestone.','xs',C.muted);let y=y0+34;if((G.omens||[]).length){text(16,y,fitText('Active: '+G.omens.map(k=>OMENS[k].name).join(', '),'xs',236),'xs','#bbdefb');y+=12;}
    for(const id of offer){const o=OMENS[id];px(14,y,242,26,C.band);text(20,y+4,o.name,'sb',C.cream);text(20,y+14,fitText(o.desc,'xs',172),'xs',C.muted);button(196,y+5,54,16,true);text(223,y+8,'Take','xs',C.goldL,'center');hit(196,y+5,54,16,()=>{G.omens=(G.omens||[]).concat([id]);G.omenOffer=null;sfx('cast');toast(o.name+' settles over the road');closeSheet();});y+=30;}
    button(16,y+2,110,18,false);text(71,y+7,'Walk on without one','xs',C.cream,'center');hit(16,y+2,110,18,()=>{G.omenOffer=null;closeSheet();});}
    ...
    for(const h of G.roster){if(y>H-90)break;const st=heroStatus(h);if(st==='marching'||st==='quest')continue;const posted=st==='garrison';px(14,y,242,26,C.band);drawSprite(h,28,y+22,0,'idle',false);text(46,y+4,h.name+' · '+CLASSES[h.cls].name+' Lv '+h.lvl,'s',C.cream);text(46,y+14,posted?'On the walls · +'+fmtNum(garrisonXp(h))+' XP per hour':'At camp','xs',posted?C.goldL:C.muted);
      const lk2=posted&&postLeft(h)>0;button(196,y+5,54,16,!posted,lk2);text(223,y+8,posted?(lk2?fmtT(postLeft(h)):'Recall'):'Post','xs',lk2?C.dimt:(posted?C.cream:C.goldL),'center');hit(14,y,242,26,()=>{if(posted)recallHero(h);else postHero(h);});y+=30;}
    ...
    button(16,y0+56,110,20,true);icon('play',22,y0+60,4);text(71,y0+61,'Watch ad (placeholder)','xs',C.goldL,'center');hit(16,y0+56,110,20,()=>{buySpeed4();closeSheet();});
    button(140,y0+56,110,20,false);text(195,y0+61,'Back to 1×','xs',C.cream,'center');hit(140,y0+56,110,20,()=>{G.speed=1;closeSheet();});}
  else if(s.kind==='shatter'){const r=G.run,best=Math.max(...G.roster.map(h=>h.lvl)),hrs=(now()-r.start)/HOUR;text(16,y0+8,'Run '+((G.reforges||0)+1)+' - the summary','h',C.goldL);
    const rows=[['Time on the road',hrs>=1?hrs.toFixed(1)+' h':Math.round(hrs*60)+' min'],['Fights won',fmtNum(r.fights)],['Bosses beaten',String(r.bosses)],['Gold earned',fmtNum(r.gold)],['Deepest catacomb floor',String(r.bestFloor||0)],['Endless Road this run','Fight '+(r.endless||0)],['Oath this run',G.oath&&OATHS[G.oath]?OATHS[G.oath].name+'  (tap to break)':'none'],['Highest hero level',String(best)]];
    rows.forEach(([k,v],i)=>{const y=y0+24+i*11;text(20,y,k,'xs',C.muted);text(250,y,v,'xsb',C.cream,'right');if(k==='Oath this run'&&G.oath)hit(140,y-2,116,11,()=>{G.oath=null;for(const h of G.roster)refreshStats(h);toast('The oath is broken. No bonus this run.');});});let ty=y0+24+rows.length*11+6;text(16,ty,'Dust for this run','sb',C.goldL);ty+=11;const parts=reforgeParts(),bm=dustBlessMult();parts.forEach(([k,v])=>{text(20,ty,k,'xs',C.muted);text(250,ty,'+'+v,'xsb','#bbdefb','right');ty+=10;});if(bm>1){text(20,ty,'Dust blessing','xs',C.muted);text(250,ty,'x'+bm.toFixed(2),'xsb','#bbdefb','right');ty+=10;}if(G.oath&&OATHS[G.oath]){text(20,ty,OATHS[G.oath].name,'xs',C.muted);text(250,ty,'x'+OATHS[G.oath].dust,'xsb','#bbdefb','right');ty+=10;}const e2=(G.run&&G.run.endless)||0,nxt=Math.floor((e2+100)/25)+2*Math.floor((e2+100)/100)-(Math.floor(e2/25)+2*Math.floor(e2/100));text(20,ty,'Next 100 Endless fights would add','xs',C.dimt);text(250,ty,'+'+nxt,'xsb',C.dimt,'right');ty+=14;
    text(16,ty,'Crystal Oath for the next run (optional)','sb',C.goldL);ty+=12;Object.keys(OATHS).forEach((k,i)=>{const bx=16+(i%2)*120,by=ty+Math.floor(i/2)*21,on=G.oathPick===k;button(bx,by,114,18,on);text(bx+57,by+5,OATHS[k].name,'xs',on?C.goldL:C.cream,'center');hit(bx,by,114,18,()=>{G.oathPick=on?null:k;});});ty+=44;{const ok=G.oathPick?OATHS[G.oathPick]:null;text(16,ty,ok?ok.desc+'  ·  dust x'+ok.dust+(ok.renown?', renown x'+ok.renown:''):'No oath: a plain run.','xs',ok?'#bbdefb':C.dimt);ty+=13;}
    text(16,ty,'Shatter the Crystal for +'+reforgeGain()+' dust. Its shards scatter along the road','xs','#bbdefb');text(16,ty+9,'again, and you walk it from Greenhollow.','xs','#bbdefb');text(16,ty+18,'You keep your heroes, talents, gear and its upgrades, gold, ore, Vael,','xs','#bbdefb');text(16,ty+27,'the tree and blessings.','xs','#bbdefb');
    button(16,ty+42,110,22,true);text(71,ty+49,'Begin run '+((G.reforges||0)+2),'sb',C.goldL,'center');hit(16,ty+42,110,22,doReforge);button(140,ty+42,110,22,false);text(195,ty+49,'Not yet','xs',C.cream,'center');hit(140,ty+42,110,22,closeSheet);}
  else if(s.kind==='stats'){const st=G.stats;const rows=[['Enemies defeated',fmtNum(st.kills)],['Bosses defeated',String(st.bosses)],['Endless Road best','Fight '+Math.max(G.prog[ZONES.length-1]||0,(G.far||[])[ZONES.length-1]||0)],['Renown',fmtNum(G.renown||0)+' (+'+f1((renownStat()-1)*100)+'% stats, +'+f1((renownGold()-1)*100)+'% gold)'],['Highest hit',fmtNum(st.hit)],['Gold earned',fmtNum(st.gold)],['Items found',String(st.items)],['Sunforged found',String(st.legendary)],['Taps',fmtNum(st.taps)],['Hordes repelled',String(st.hordes)],['Offline fights',fmtNum(st.offline)],['Catacomb runs',String(st.delveRuns)],['Deepest floor',String(G.delveBest||0)],['Shatters',String(G.reforges||0)],['Defeats',String(st.defeats||0)],['Road time',fmtT(G.simT||0)]];
    ...
    const offers=[['Auto-cast · 4h',G.autoUntil>now()?fmtT((G.autoUntil-now())/1000)+' left':'Abilities cast on their own (offline too)',()=>{G.autoUntil=Math.max(now(),G.autoUntil)+4*3600*1000;G.autoCast=true;toast('Auto-cast active for four hours');}],
      ['4× road speed · 2h',G.speed===4&&G.speedUntil>now()?fmtT((G.speedUntil-now())/1000)+' left':'The road runs four times faster',buySpeed4],
...
function trackFor(){const Z=ZONES[G.zone]||ZONES[0];const M=ASSETS.music||{};if(Z.endless&&M.endless)return 'endless';const zi=Z.endless?endSeg(G.prog[G.zone]||0):G.zone;const k=['greenhollow','lagoon','thornwood','ironvein','emberwaste','amberfall','ashen','keep','foundry'][zi]||'greenhollow';return M[k]?k:'greenhollow';}
...
const TUT=[
  ...
  {t:'The Tree turns gold into permanent ranks',s:'HP, attack, tap damage, camp perks - and blessings after a Shatter.',screen:'tree',box:()=>{const top=42+(BIG?22:16)+26,n=TREE.filter(x=>x.branch===G.treeTab).length+(G.treeTab==='Crystal'?1:0),rh=Math.max(22,Math.min(BIG?44:32,Math.floor((TABY-8-TABTOP-top)/n)-3));return[4,top,262,Math.min(TABY-14-top,n*(rh+3)+1)];}},
];
function drawTutorial(){if(G.tut==null)return;const st=TUT[G.tut];hits.length=0;let[bx,by,bw,bh]=typeof st.box==='function'?st.box():st.box;bx=Math.max(2,bx);by=Math.max(18,by);bw=Math.min(bw,W-4-bx);bh=Math.min(bh,(st.tabs?H:TABY)-2-by);
  const dimAround=(x,y,w,h)=>{dimRect(0,0,W,y,0.6);dimRect(0,y+h,W,H-y-h,0.6);dimRect(0,y,x,h,0.6);dimRect(x+w,y,W-x-w,h,0.6);tx.fillStyle='rgba(6,8,16,0.6)';tx.fillRect(0,0,tc.width,y*TS);tx.fillRect(0,(y+h)*TS,tc.width,(H-y-h)*TS);tx.fillRect(0,y*TS,x*TS,h*TS);tx.fillRect((x+w)*TS,y*TS,(W-x-w)*TS,h*TS);};dimAround(bx,by,bw,bh);drawTabs();hits.length=0;const TABIDS=['heroes','gear','road','vael','tree'];if(st.screen){const ti=TABIDS.indexOf(st.screen);if(ti>=0)px(ti*54+2,TABY+3,50,1,C.goldL);}if(st.tabs)TABIDS.forEach((id,ti)=>{if(!st.tabs.includes(id)){dimRect(ti*54,TABY+2,54,40,0.6);tx.fillStyle='rgba(6,8,16,0.6)';tx.fillRect(ti*54*TS,(TABY+2)*TS,54*TS,40*TS);}else px(ti*54+2,TABY+3,50,1,C.goldL);});const bord=(x,y,w,h)=>{px(x-2,y-2,w+4,2,C.goldL);px(x-2,y+h,w+4,2,C.goldL);px(x-2,y-2,2,h+4,C.goldL);px(x+w,y-2,2,h+4,C.goldL);};if(!st.tabs){ctx.globalAlpha=0.7+0.3*Math.sin(RT*4);bord(bx,by,bw,bh);ctx.globalAlpha=1;}
  const ww=st.icon?178:228,tx0=st.icon?70:20;const fs=st.font||'s',lh=fs==='s'?11:13;const lines=[].concat(wrapN(st.s,fs,ww),st.s2?wrapN(st.s2,fs,ww):[],st.s3?wrapN(st.s3,fs,ww):[]);const extra=Math.max((lines.length-1)*lh,st.icon?22:0),ch=72+extra;const cands=[by+bh+10,by-ch-10,40,TABY-ch-10];let py=cands.find(y=>y>=20&&y+ch<=TABY-4&&!(y<by+bh&&y+ch>by));if(py==null)py=by>H/2?40:TABY-ch-10;frame(12,py,246,ch,C.navy,C.goldL);tx.clearRect(12*TS,py*TS,246*TS,ch*TS);text(20,py+8,st.t,'h',C.goldL);if(st.icon&&IMG.superGear){const ii=(ASSETS.superOrder||[]).indexOf(st.icon);if(ii>=0){const g=ctx.createRadialGradient(42,py+44,2,42,py+44,26);g.addColorStop(0,'rgba(120,220,255,0.5)');g.addColorStop(1,'rgba(120,220,255,0)');ctx.fillStyle=g;ctx.fillRect(14,py+16,56,56);ctx.drawImage(IMG.superGear,ii*32,0,32,32,20,py+22,44,44);}}lines.forEach((l,i)=>text(tx0,py+23+i*lh,l,fs,C.cream));if(!st.once)text(20,py+46+extra,(G.tut+1)+' / '+TUT.length,'xs',C.dimt);hit(0,0,W,H,()=>{});button(170,py+42+extra,80,22,true);text(210,py+49+extra,st.once?'Got it':(G.tut<TUT.length-1?'Next':'Begin'),'sb',C.goldL,'center');hit(170,py+42+extra,80,22,()=>{const once=TUT[G.tut]&&TUT[G.tut].once;G.tut++;if(G.tut===1&&G.mode==='walk'&&!G.enemies.length){const E=ENEMIES.slime,e={uid:'slime',id:'slime',name:'Slime',lvl:1,x:224,slot:224,dx:0,yoff:0,flip:true,fly:false,range:'melee',boss:false,scale:1,native:false,nscale:1,hue:0,tier:0,gauge:0,dead:false,enemy:true,status:{},flash:0,traits:[],acts:0};Object.assign(e,enemyStats('slime',1));e.hp=e.maxhp;setAnim(e,'idle');G.enemies=[e];G.fs={dmg:0,taps:0,gold:0};G.mode='battle';G.active.forEach(h=>setAnim(h,'idle'));}if(once){if(TUT[G.tut]&&TUT[G.tut].once){return;}while(TUT.length&&TUT[TUT.length-1].once)TUT.pop();G.tut=null;saveGame();return;}if(G.tut>=TUT.length){G.tut=null;G.tutDone=true;G.screen='road';saveGame();}else G.screen=TUT[G.tut].screen||'road';});}
function draw(){hits.length=0;PRESSED=[];tx.clearRect(0,0,tc.width,tc.height);px(0,0,W,H,'#0b0d16');
  ...
  drawBanner();drawCard();drawTabs();drawTop();drawSheet();drawTutorial();for(const p of (G.uparts||[])){ctx.globalAlpha=clamp(p.life*2,0,1);const sz=p.big?(p.life>0.4?3:2):(p.life>0.4?2:1);px(R(p.x),R(p.y),sz,sz,p.col);}ctx.globalAlpha=1;
  ...
  if(G.toast&&!G.reveal&&G.tut==null&&!(G.toast.scope==='road'&&(G.screen!=='road'||G.sheet))){let a=clamp(Math.min(G.toast.t*4,((G.toast.dur||2.2)-G.toast.t)*3),0,1);if(G.toast.scope==='road'){const w=textW(G.toast.text,'xs')+12;ctx.globalAlpha=a*0.45;px(4,SCENE_Y+192,R(w),12,'#000');ctx.globalAlpha=1;text(10,SCENE_Y+194,G.toast.text,'xs',G.toast.col||C.cream,'left',a);}else if(G.toast.screen==='road'&&G.screen==='road'&&!G.sheet){const w=textW(G.toast.text,'s')+16,ty=SCENE_Y+186;ctx.globalAlpha=a*0.55;px(R(W/2-w/2),ty,R(w),16,'#000');ctx.globalAlpha=1;text(W/2,ty+4,G.toast.text,'s',G.toast.col||C.cream,'center',a);}
  else{const lines=wrap2(G.toast.text,'sb',W-44),w=W-12,hh=12+lines.length*10,onRoad=false,dur=G.toast.dur||2.2,k=onRoad?1:clamp(Math.min(G.toast.t*4,(dur-G.toast.t)*4),0,1),ease=1-Math.pow(1-k,3),base=G.screen==='gear'&&!G.sheet?50:20,ty=onRoad?SCENE_Y+200-hh:R(base-(hh+base+2)*(1-ease)),tx0=6;if(!onRoad)a=1;G.toastRect={x:tx0,y:ty,w:R(w),h:hh};tx.clearRect(tx0*TS,Math.max(0,ty)*TS,R(w)*TS,hh*TS);ctx.globalAlpha=a;px(tx0,ty,R(w),hh,'#0b0d16');px(tx0,ty,R(w),1,C.goldD);px(tx0,ty+hh-1,R(w),1,C.goldD);px(tx0,ty,1,hh,C.goldD);px(tx0+R(w)-1,ty,1,hh,C.goldD);ctx.globalAlpha=1;lines.forEach((l,i)=>text(W/2,ty+6+i*10,l,'sb',G.toast.col||C.cream,'center',a));}}}
function buySpeed4(){G.speed=4;G.speedUntil=Math.max(now(),G.speedUntil||0)+2*3600*1000;toast('4× speed for two hours');} /* the 4x boost: two real hours from now, or from the current end when one is still running; speedNow() falls back to 2x when it lapses */
...
let RT=0;
function loop(ts){const dt=Math.min(0.05,(ts-last)/1000||0);last=ts;RT+=dt;rollDt=dt;if(G.booted&&!G.delve&&!G.hordeFight){const mk=trackFor();if(mk!==music.cur)playMusic(mk);const nk=nextTrackKey();if(nk&&nk!==music.cur&&!music.bufs[nk]&&!music.pending[nk])ensureTrack(nk);}if(HOLD&&RT>=HOLD.next){const r0=HOLD.r,fresh=hits.find(h=>h.hold&&h.x===r0.x&&h.dy===r0.dy&&h.w===r0.w&&h.h===r0.h);if(fresh){HOLD.r=fresh;HOLD.fn=fresh.fn;}const r=HOLD.r;if(fresh&&HOLD.px>=r.x&&HOLD.px<r.x+r.w&&HOLD.py>=r.y&&HOLD.py<r.y+r.h){spendCall(HOLD.fn,HOLD.px,HOLD.py,HOLD.r);HOLD.next=RT+0.12;}else endHold();}const n=G.tut!=null?1:speedNow();for(let k=0;k<n;k++)update(dt,k===0);draw();requestAnimationFrame(loop);}
...
function endDrag(e){const d=G.drag;if(!d)return;G.drag=null;const[x,y]=evPos(e);
  ...
  if(G.active[j]){[G.active[d.i],G.active[j]]=[G.active[j],G.active[d.i]];}else{G.active.splice(j>G.active.length?G.active.length:j,0,G.active.splice(d.i,1)[0]);}layout();buildChanged();toast(G.active[0].name+' takes the front');}
cv.addEventListener('pointerup',endDrag);cv.addEventListener('pointercancel',()=>{G.drag=null;endHold();});window.addEventListener('pointerup',()=>{endHold();toastTouch=null;});window.addEventListener('pointercancel',()=>{endHold();});window.addEventListener('blur',()=>{endHold();});cv.addEventListener('pointerleave',()=>{endHold();});
function dispatch(x,y){for(let i=hits.length-1;i>=0;i--){const h=hits[i];if(x>=h.x&&x<h.x+h.w&&y>=h.y&&y<h.y+h.h){G.lastHit={x:h.x,y:h.dy,w:h.w,h:h.h,until:RT+0.12};spendCall(h.fn,x,y,h);if(h.hold)draw();if(h.hold){const key=h.x+','+h.dy+','+h.w,again=LAST_HOLD&&LAST_HOLD.key===key&&RT-LAST_HOLD.end<0.35;HOLD={fn:h.fn,next:RT+0.45,r:h,px:x,py:y,toastText:again?LAST_HOLD.toastText:((G.toast&&G.toast.t===0)?G.toast.text:null)};}if(G.drag&&!G.drag.x0){G.drag.x0=x;G.drag.y0=y;G.drag.x=x;G.drag.y=y;}return true;}}return false;}
let scrollTouch=null,toastTouch=null;
function dismissToast(){const T=G.toast;if(!T)return;const d=T.dur||2.2;if(T.t<d-0.25)T.t=d-0.25;}
cv.addEventListener('pointermove',e=>{if(toastTouch){const[,my]=evPos(e);if(toastTouch.y0-my>8){dismissToast();toastTouch=null;}}if(HOLD){const[hx,hy]=evPos(e);HOLD.px=hx;HOLD.py=hy;}if(!scrollTouch)return;const[x,y]=evPos(e);if(!scrollTouch.moved){const rc=scrollTouch.rect;const out=scrollTouch.held?(y<rc.y||y>=rc.y+rc.h):Math.abs(y-scrollTouch.y0)>5;if(out){scrollTouch.moved=true;if(scrollTouch.held){HOLD=null;scrollTouch.y0=y;}}}if(scrollTouch.moved){SCROLL[scrollTouch.key]=clamp(scrollTouch.s0-(y-scrollTouch.y0),0,scrollMax(scrollTouch.key));}});
...
cv.addEventListener('pointerdown',e=>{if(!G.title)startMusic();if(!G.title&&G.music&&!music.ok){if(music.buf&&!music.node)startNode();else if(music.el&&music.el.paused)tryPlay();}const[x,y]=evPos(e);toastTouch=null;{const T=G.toast,rc=G.toastRect;if(T&&rc&&T.screen!=='road'&&T.t<(T.dur||2.2)&&x>=rc.x&&x<rc.x+rc.w&&y>=rc.y&&y<rc.y+rc.h)toastTouch={y0:y};}
  if(!G.sheet&&(G.screen==='tree'||G.screen==='heroes'||G.screen==='map')&&y>(SCROLLTOP[G.screen]||44)&&y<TABY){let top=null;for(let i=hits.length-1;i>=0;i--){const h=hits[i];if(x>=h.x&&x<h.x+h.w&&y>=h.y&&y<h.y+h.h){top=h;break;}}const held=!!(top&&top.hold);if(held)dispatch(x,y);scrollTouch={key:G.screen,x0:x,y0:y,s0:SCROLL[G.screen]||0,moved:false,held,rect:held?top:null};return;}
