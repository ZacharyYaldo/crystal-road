...
function dealDamage(src,tgt,mult=1,opts={}){
  ...
  dmg=Math.max(1,R(Math.max(dmg,atk*mult*0.02)));if(!tgt.enemy&&tgt.talents&&tgt.talents.unbroken&&!tgt.unbrokenUsed&&tgt.hp-dmg<=0){dmg=tgt.hp-1;tgt.unbrokenUsed=true;float(tgt.x,GROUND-44,'Unbroken!',C.goldL,true);}if(!tgt.enemy)dmg=R(dmg*omenMult('hdmg'));if(!tgt.enemy&&G.hordeFight&&G.hordeFight.gate>0){const take=Math.min(G.hordeFight.gate,R(dmg*0.5*(G.hordeFight.mod==='siege'&&src.champ?2:1)));G.hordeFight.gate-=take;dmg-=take;if(G.hordeFight.gate<=0)float(W/2,GROUND-70,'The gate is breached!','#ff8a80',true);}tgt.hp-=dmg;if(!src.enemy&&src.status&&src.status.rage>0&&!src.dead)src.hp=Math.min(src.maxhp,src.hp+dmg*0.05);sfx(crit?'crit':'hit');if(!src.enemy&&G.fs){G.fs.dmg+=dmg;G.stats.hit=Math.max(G.stats.hit||0,dmg);}if(src.enemy&&G.fs){G.fs.taken=G.fs.taken||{};G.fs.taken[src.name]=(G.fs.taken[src.name]||0)+dmg;G.fs.takenT=G.fs.takenT||{};G.fs.takenT[src.range]=(G.fs.takenT[src.range]||0)+dmg;}
...
function doReforge(){const gain=reforgeGain();G.dust+=gain;G.reforges=(G.reforges||0)+1;const startLv=1+5*treeLv('b_start');
  ...
  G.run={start:now(),fights:0,bosses:0,gold:0,bestFloor:0,endless:0};G.endMilestone=0;G.oath=G.oathPick||null;G.oathPick=null;G.castle.currentHorde=null;G.omens=[];G.omenOffer=null;G.omenMarks=[];if(G.active.length>partyMax())G.active=G.active.slice(0,partyMax());G.ore+=R(refund);G.prog=ZONES.map(()=>0);G.far=ZONES.map(()=>0);G.cleared=ZONES.map(()=>false);G.zone=0;G.enemies=[];G.projs=[];G.action=null;G.mode='walk';G.enc=4;G.wins=Math.max(G.wins,3);G.newZone=false;
...
function drawSheet(){const s=G.sheet;if(!s)return;tx.clearRect(0,0,tc.width,tc.height);dimRect(0,0,W,H,0.75);hit(0,0,W,H,()=>{if(s.kind!=='welcome')closeSheet();});
  ...
  if(s.kind==='swap'){text(16,y0+8,'Party',"h",C.goldL);text(16,y0+20,'Tap a marcher, then a hero at camp to trade places.','xs',C.muted);text(16,y0+29,'Two marchers to reorder. Left slot is the front.','xs',C.muted);
    ...
    text(16,ty,'Shatter the Crystal for +'+reforgeGain()+' dust. Its shards scatter along the road','xs','#bbdefb');text(16,ty+9,'again, and you walk it from Greenhollow.','xs','#bbdefb');text(16,ty+18,'You keep your heroes, talents, gear, gold, Vael, the tree and blessings.','xs','#bbdefb');text(16,ty+27,'Gear upgrades pay back half their ore.','xs','#bbdefb');
