...
function drawReveal(){const rv=G.reveal;if(!rv||G.screen!=='road')return;if(!rv.at)rv.at=performance.now();const el=(performance.now()-rv.at)/1000,k=Math.min(1,el*3);hits.length=0;dimRect(0,0,W,H,0.78*k);tx.fillStyle='rgba(6,8,16,'+(0.78*k)+')';tx.fillRect(0,0,tc.width,tc.height);
  ...
  text(cx,py0+8,'A Crystal treasure','xs',C.muted,'center');text(cx,cy+52,itemName(rv.it),'big','#8fdcff','center');const d=SUPER[rv.it.super];text(cx,cy+72,d&&d.fx?d.fx:itemStatLabel(rv.it),'s',C.cream,'center');text(cx,cy+88,'Mythic  ·  '+itemStatLabel(rv.it),'xs','#8fdcff','center');text(cx,py0+ph-14,'tap to continue','xs',C.dimt,'center',0.5+0.5*Math.sin(RT*3));
...
function draw(){hits.length=0;tx.clearRect(0,0,tc.width,tc.height);px(0,0,W,H,'#0b0d16');
  ({road:drawRoad,gear:drawGear,heroes:drawHeroes,vael:drawVael,map:drawMap,tree:drawTree})[G.screen]();drawDragGhost();drawReveal();if(G.updateReady&&!G.sheet&&G.tut==null){const uw=112,ux=W/2-uw/2,uy=TABY-30;ctx.globalAlpha=0.6+0.4*Math.sin(RT*3);button(ux,uy,uw,20,true);ctx.globalAlpha=1;text(W/2,uy+6,'Update ready - tap to reload','xs',C.goldL,'center');hit(ux,uy,uw,20,applyUpdate);}
