...
function drawGear(){drawPanelScreen();const h=G.roster[G.gearHero]||G.roster[0];
  ...
  ['weapon','cape','charm'].forEach((slot,i)=>{const x=4+i*88,it=h.eq[slot];frame(x,SY,86,SLOTH,C.navy,it&&it.rank>0?RANKHEX[it.rank]:C.gold);
    ...
    if(it){drawItemIcon(it,x+31,SY+14,2);const nm=itemName(it);text(x+43,SY+43,nm,textW(nm,'sb')>78?'xsb':'sb',it.rank>0?RANKHEX[it.rank]:C.cream,'center');text(x+43,SY+53,(it.rank>=5?'MYTHIC':'★'.repeat(it.rank+1)+'☆'.repeat(4-it.rank))+(it.lvl?'  Lv '+it.lvl:''),'xs',C.goldL,'center');text(x+43,SY+61,itemStatLabel(it),'xs',C.green,'center');
