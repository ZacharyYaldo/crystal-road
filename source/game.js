...
function toast(t,col,scope){const dur=Math.min(4.5,2.2+t.length*0.03);if(HOLD){if(HOLD.toastText===t)return;HOLD.toastText=t;}const cur=G.toast;if(cur&&cur.t<(cur.dur||2.2)&&cur.scope===scope){const out=(cur.dur||2.2)-cur.t;cur.text=t;cur.col=col;cur.dur=dur;if(out<0.25)cur.t=0.25-out;else if(cur.t>0.3)cur.t=0.3;return;}G.toast={text:t,t:0,col,scope,dur};}
...
function draw(){hits.length=0;PRESSED=[];tx.clearRect(0,0,tc.width,tc.height);px(0,0,W,H,'#0b0d16');
  ...
  if(G.toast&&!G.reveal&&!(G.toast.scope==='road'&&G.screen!=='road')){let a=clamp(Math.min(G.toast.t*4,((G.toast.dur||2.2)-G.toast.t)*3),0,1);if(G.toast.scope==='road'){const w=textW(G.toast.text,'xs')+12;ctx.globalAlpha=a*0.45;px(4,SCENE_Y+192,R(w),12,'#000');ctx.globalAlpha=1;text(10,SCENE_Y+194,G.toast.text,'xs',G.toast.col||C.cream,'left',a);}else{const lines=wrap2(G.toast.text,'sb',W-44),w=W-12,hh=12+lines.length*10,onRoad=G.screen==='road'&&!G.sheet,dur=G.toast.dur||2.2,k=onRoad?1:clamp(Math.min(G.toast.t*4,(dur-G.toast.t)*4),0,1),ease=1-Math.pow(1-k,3),ty=onRoad?SCENE_Y+200-hh:R(20-(hh+22)*(1-ease)),tx0=6;if(!onRoad)a=1;tx.clearRect(tx0*TS,Math.max(0,ty)*TS,R(w)*TS,hh*TS);ctx.globalAlpha=a;px(tx0,ty,R(w),hh,'#0b0d16');px(tx0,ty,R(w),1,C.goldD);px(tx0,ty+hh-1,R(w),1,C.goldD);px(tx0,ty,1,hh,C.goldD);px(tx0+R(w)-1,ty,1,hh,C.goldD);ctx.globalAlpha=1;lines.forEach((l,i)=>text(W/2,ty+6+i*10,l,'sb',G.toast.col||C.cream,'center',a));}}}
