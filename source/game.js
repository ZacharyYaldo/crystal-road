...
function update(dt,ui=true){G.t+=dt;if(G.shake>0)G.shake-=dt;
  ...
  else if(G.mode==='battle'){if(G.hordeFight&&G.hordeFight.volleyAt&&RT>=G.hordeFight.volleyAt&&!G.banner){const hf=G.hordeFight;hf.volleyAt=0;const fr=Math.min(0.4,0.08+0.02*castleLv())/Math.max(1,hf.mercs);const src={enemy:false,status:{},x:0,dx:0};for(let i=0;i<hf.mercs;i++){const mx=[104,166][i]||104;for(const e of G.enemies)if(!e.dead)G.projs.push({x:mx,y:GROUND-58,tgt:e,src,t:0,dur:0.45,flip:false,merc:true,frac:fr});}float(W/2,GROUND-70,'Mercenary volley!','#9ad0ff',true);sfx('cast');}for(const e of G.enemies){if(!e.dead&&e.x>e.slot){e.x=Math.max(e.slot,e.x-90*dt);if(e.x<=e.slot&&e.anim==='walk'&&!e.fly)setAnim(e,'idle');}}}
...
function drawScene(){const Z=ZONES[G.zone];const key=G.hordeFight?'door':G.delve?(G.delve.floor>=12?'door':'cave'):((G.bossFight&&Z.bossScene)?Z.bossScene:zoneScene(Z,G.prog[G.zone]));const layers=SCENES[key];
  ...
  if(G.hordeFight){ctx.globalAlpha=0.8;frame(4,SCENE_Y+4,150,36,'rgba(44,22,26,0.8)');ctx.globalAlpha=1;text(9,SCENE_Y+7,'Horde at the gate','sb','#ff9696');text(9,SCENE_Y+17,'Wave '+G.hordeFight.wave+' of 3 · '+hordeTier(),'xs','#c88080');const hf=G.hordeFight;if(hf.mercs>0){const A=ATLAS.soldier;for(let i=0;i<hf.mercs;i++){const mx=[104,166][i]||104,fr=Math.floor(RT*4+i)%A.anims.idle.n;ctx.save();ctx.globalAlpha=0.95;drawSprite({uid:'soldier'},mx,GROUND-44,fr,'idle',i===1,1);ctx.restore();}}if(hf.gateMax>0){const gx=176,gy=SCENE_Y+34;px(gx-2,gy-4,90,18,'rgba(0,0,0,0.55)');bar(gx,gy,86,6,hf.gate/hf.gateMax,hf.gate>0?'#9ad0ff':'#553');text(gx+43,gy+7,hf.gate>0?'Gate '+Math.round(100*hf.gate/hf.gateMax)+'%':'Gate breached','xs',hf.gate>0?'#cfe8ff':'#ff8a80','center');}}else if(G.delve){drawDelveOverlay();}else{
  // zone plate
...
function startHordeFight(){if(G.delve){toast('Finish the catacombs first');return;}const C2=G.castle,S=hordeStrength(),Z=ZONES[ZONES[G.zone].endless?endSeg(G.prog[G.zone]):G.zone];const gar=garrisonHeroes().filter(h=>!G.active.includes(h));
  ...
  G.active=G.active.concat(gar).slice(0,6);for(const h of G.active){h.dead=false;h.status={};if(h.hp<h.maxhp*0.5)h.hp=R(h.maxhp*0.5);}layout();{const D=defense(),p=C2.prep||{};const partyHp=G.active.reduce((a,h)=>a+h.maxhp,0);const frac=Math.min(2,D.walls/100)+(p.merc?0.3:0);G.hordeFight.gateMax=R(partyHp*frac);G.hordeFight.gate=G.hordeFight.gateMax;G.hordeFight.mercs=p.merc?2:0;}G.active.forEach(h=>{h.x=h.tx;});C2.hordeDue=false;G.enemies=[];G.projs=[];G.action=null;G.screen='road';closeSheet();sfx('cast');banner('The horde is at the gate','Wave 1 of 3',2.5);hordeWave();}
...
function drawSheet(){const s=G.sheet;if(!s)return;tx.clearRect(0,0,tc.width,tc.height);dimRect(0,0,W,H,0.75);hit(0,0,W,H,()=>{if(s.kind!=='welcome')closeSheet();});
  ...
  if(s.kind==='swap'){text(16,y0+8,'Party',"h",C.goldL);text(16,y0+20,'Tap a marcher, then a hero at camp to trade places.','xs',C.muted);text(16,y0+29,'Two marchers to reorder. Left slot is the front.','xs',C.muted);
    ...
    let y=y0+36;const rows=[['Shore up the walls','+25% wall defense, stronger gate',p.walls?null:prepCost('walls'),()=>{p.walls=true;}],['Hire mercenaries','+'+(8+castleLv()*2)+' defense, a volley each wave',p.merc?null:prepCost('merc'),()=>{p.merc=true;}]];
