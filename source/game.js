...
function loadAll(){const ps=[];const cap=p=>Promise.race([p,new Promise(r=>setTimeout(r,8000))]);for(const k of ['logo','roadIcon','tab_heroes','tab_gear','tab_vael','tab_tree','superGear'])if(ASSETS[k]){const im=new Image();ps.push(cap(new Promise(res=>{im.onload=res;im.onerror=res;})));im.src=ASSETS[k];IMG[k]=im;}
...
const SUPER_ORDER=['sword','staff','dagger','bow','axe','cape','amulet','ring'];
...
function drawItemIcon(it,x,y,scale){if(it.super&&IMG.superGear&&IMG.superGear.width){const i=(ASSETS.superOrder||[]).indexOf(it.super);if(i>=0){const sz=16*scale;ctx.drawImage(IMG.superGear,i*32,0,32,32,x-(sz-12*scale)/2,y-(sz-12*scale)/2,sz,sz);return;}}icon(itemIcon(it),x,y,it.rank,scale);}
