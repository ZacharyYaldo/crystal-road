...
function headBtn(x,y,w,h,label,can,dis,cost,need){button(x,y,w,h,can,dis);const hh=10;px(x+2,y+2,w-4,hh,dis?'#10131f':(can?'#2a2410':'#161b30'));px(x+2,y+2+hh,w-4,1,dis?C.dimt:(can?C.goldD:'#3a4470'));text(x+w/2,y+3,label,'xs',dis?C.dimt:(can?C.goldL:C.cream),'center');const by=y+2+hh,bh=h-4-hh;if(need){text(x+w/2,by+R(bh/2)-3,need,'xs',C.dimt,'center');}else{icon('coin',x+3,by+R(bh/2)-6,4);textA(x+39,x+17,x+w-3,by+R(bh/2)-4,cost,'0.00K','xsb',can?C.goldL:C.muted);}} /* slim header + Tree-style cost body; outer size identical to button() */
...
function drawHeroes(){drawPanelScreen();text(8,26,'Heroes','title',C.goldL);text(262,30,G.roster.length+' recruited','xs',C.muted,'right');beginScroll('heroes',44);
  let y=46;const HR=BIG?110:98;for(const h of G.roster){frame(4,y,262,HR);{const fx0=tierFx(h);if(fx0.aura){const g=ctx.createRadialGradient(28,y+26,2,28,y+26,20);g.addColorStop(0,'rgba(255,120,60,0.35)');g.addColorStop(1,'rgba(255,120,60,0)');ctx.fillStyle=g;ctx.fillRect(8,y+6,40,40);}if(fx0.ghost)ctx.globalAlpha=0.75+0.2*Math.sin(RT*7);drawSprite(h,26,y+38,0,'idle',false);ctx.globalAlpha=1;}{const fx=tierFx(h);ctx.save();ctx.beginPath();ctx.rect(5,y+1,60,HR-2);ctx.clip();drawTierExtras(h,26,y+38,1,Object.assign({},fx,{wglow:null}),false);if(fx.halo){const top=y+38-(ATLAS[h.uid].oy-headRow(h));ctx.strokeStyle='rgba(255,240,180,'+(0.6+0.3*Math.sin(RT*3))+')';ctx.lineWidth=1.2;ctx.beginPath();ctx.ellipse(28,top-3,6,2,0,0,Math.PI*2);ctx.stroke();}ctx.restore();}const st=heroStatus(h);
    ...
    {const q=promoteReq(h),can=(h.tier||0)<3&&h.lvl>=q.lvl&&G.gold>=q.gold&&G.dust>=q.dust,PB2=Math.floor((HR-34)/2),py2=y+HR-26-PB2;if((h.tier||0)<3){headBtn(194,py2,66,PB2,'Promote',can,h.lvl<q.lvl,fmtNum(q.gold),h.lvl<q.lvl?'Lv '+q.lvl+' required':null);hit(194,py2,66,PB2,()=>promoteHero(h));}}
    ...
    const cost=rankCost(h),can=G.gold>=cost;const HB=Math.floor((HR-34)/2);headBtn(194,y+4,66,HB,'Rank up',can,false,fmtNum(cost),null);hit(194,y+4,66,HB,()=>{const c=rankCost(h);if(G.gold<c){toast('Need '+fmtNum(c)+' gold');return;}G.gold-=c;h.abLvl++;toast(ab.name+' rank '+h.abLvl);},true);
