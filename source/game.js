...
function renownGain(p,kind){return R(5*Math.pow(1.02,p)*(kind==='milestone'?100:kind==='champion'?10:1)*(G.oath==='solitude'?1.5:1));}
function oreScale(L){return Math.pow(1.035,L||0);}
function refMult(){return (1+0.25*(G.reforges||0))*renownGold();}
const OATHS={iron:{name:'Oath of Iron',desc:'Enemies have +50% HP and attack',dust:1.5},solitude:{name:'Oath of Solitude',desc:'Only three heroes may march',dust:1.35,renown:1.5},poverty:{name:'Oath of Poverty',desc:'Gold from the road is cut by 40%',dust:1.4},silence:{name:'Oath of Silence',desc:'Abilities charge 30% slower',dust:1.3}};
function partyMax(){return G.oath==='solitude'?3:4;}
