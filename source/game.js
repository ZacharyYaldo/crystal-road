...
function dealDamage(src,tgt,mult=1,opts={}){
  ...
  dmg=Math.max(1,R(Math.max(dmg,atk*mult*0.02)));if(!tgt.enemy&&tgt.talents&&tal(tgt,'unbroken')&&!tgt.unbrokenUsed&&tgt.hp-dmg<=0){dmg=tgt.hp-1;tgt.unbrokenUsed=true;float(tgt.x,GROUND-44,'Unbroken!',C.goldL,true);}if(!tgt.enemy)dmg=R(dmg*omenMult('hdmg'));if(!tgt.enemy&&G.hordeFight&&G.hordeFight.gate>0){const take=Math.min(G.hordeFight.gate,R(dmg*0.5*(G.hordeFight.mod==='siege'&&src.champ?2:1)));G.hordeFight.gate-=take;dmg-=take;if(G.hordeFight.gate<=0)float(W/2,GROUND-70,'The gate is breached!','#ff8a80',true);}tgt.hp-=dmg;if(!src.enemy&&src.status&&src.status.rage>0&&!src.dead)src.hp=Math.min(src.maxhp,src.hp+dmg*0.05);sfx(crit?'crit':'hit');if(!src.enemy&&G.fs){G.fs.dmg+=dmg;G.stats.hit=Math.max(G.stats.hit||0,dmg);}if(src.enemy&&G.fs){G.fs.taken=G.fs.taken||{};G.fs.taken[src.name]=(G.fs.taken[src.name]||0)+dmg;G.fs.takenT=G.fs.takenT||{};G.fs.takenT[src.range]=(G.fs.takenT[src.range]||0)+dmg;}
...
function updateAction(dt){const a=G.action,u=a.u;
  ...
  if(a.kind==='melee'||a.kind==='chargeM'||(a.kind==='ability'&&a.ab==='shadowstep')){
    ...
    else if(a.phase==='hit'){if(!a.hitDone&&u.frame>=a.hit){a.hitDone=true;if(!tgt.dead){if(a.kind==='ability'){a.hits++;let m2=0.9*a.pow;if(u.talents&&tal(u,'executioner')&&tgt.hp<tgt.maxhp*0.3)m2*=1.5;dealDamage(u,tgt,m2,{crit:a.hits===3,noHurt:a.hits<3});if(a.hits===3&&!tgt.dead&&sup(u,'dagger'))tgt.status.bleed=Math.max(tgt.status.bleed||0,3);}else if(a.kind==='chargeM'){dealDamage(u,tgt,a.parried?1.0:2.5);G.shake=0.3;}else{let m3=1;if(!u.enemy&&u.talents&&tal(u,'mark'))m3*=1+0.05*Math.min(10,tgt.marks||0);dealDamage(u,tgt,m3);}}}
