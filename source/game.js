...
function insets(){const cs=getComputedStyle(_probe);const et=parseFloat(cs.paddingTop)||0,eb=parseFloat(cs.paddingBottom)||0;const portrait=innerHeight>=innerWidth;const sh=portrait?screen.height:screen.width;const gap=Math.max(0,sh-innerHeight);const sa=navigator.standalone===true&&/iPhone|iPad|iPod/.test(navigator.userAgent);const forced=sa&&gap>4;let top=et,bot=forced?eb:(gap>=et+eb-2?0:eb);if(sa&&gap<4&&et===0){top=sh>=812?50:20;bot=sh>=812?34:0;}return{top,bot,et,eb,gap,sh,forced};}
