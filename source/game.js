...
function draw(){hits.length=0;PRESSED=[];tx.clearRect(0,0,tc.width,tc.height);px(0,0,W,H,'#0b0d16');
  ...
  if(G.toast&&!G.reveal&&!(G.toast.scope==='road'&&G.screen!=='road')){const a=clamp(Math.min(G.toast.t*4,((G.toast.dur||2.2)-G.toast.t)*3),0,1);if(G.toast.scope==='road'){const w=textW(G.toast.text,'xs')+12;ctx.globalAlpha=a*0.45;px(4,SCENE_Y+192,R(w),12,'#000');ctx.globalAlpha=1;text(10,SCENE_Y+194,G.toast.text,'xs',G.toast.col||C.cream,'left',a);}else{const lines=wrap2(G.toast.text,'sb',W-44),w=W-12,hh=12+lines.length*10,ty=G.screen==='road'&&!G.sheet?SCENE_Y+200-hh:20,tx0=6;tx.clearRect(tx0*TS,ty*TS,R(w)*TS,hh*TS);ctx.globalAlpha=a;px(tx0,ty,R(w),hh,'#0b0d16');px(tx0,ty,R(w),1,C.goldD);px(tx0,ty+hh-1,R(w),1,C.goldD);px(tx0,ty,1,hh,C.goldD);px(tx0+R(w)-1,ty,1,hh,C.goldD);ctx.globalAlpha=1;lines.forEach((l,i)=>text(W/2,ty+6+i*10,l,'sb',G.toast.col||C.cream,'center',a));}}}
