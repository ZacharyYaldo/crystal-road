...
function updateAction(dt){const a=G.action,u=a.u;
  ...
  if(a.kind==='melee'||a.kind==='chargeM'||(a.kind==='ability'&&a.ab==='shadowstep')){
    ...
    else if(a.phase==='hit'){if(!a.hitDone&&u.frame>=a.hit){a.hitDone=true;if(!tgt.dead){if(a.kind==='ability'){a.hits++;let m2=0.9*a.pow;if(u.talents&&tal(u,'executioner')&&tgt.hp<tgt.maxhp*0.3)m2*=1.5;G.dmgSrc='ability';dealDamage(u,tgt,m2,{crit:a.hits===3,noHurt:a.hits<3});G.dmgSrc=null;if(a.hits===3&&!tgt.dead&&sup(u,'dagger'))tgt.status.bleed=Math.max(tgt.status.bleed||0,3);}else if(a.kind==='chargeM'){dealDamage(u,tgt,a.parried?1.0:2.5);G.shake=0.3;}else{let m3=1;if(!u.enemy&&u.talents&&tal(u,'mark'))m3*=1+0.05*Math.min(10,tgt.marks||0);dealDamage(u,tgt,m3);}}}
