...
function update(dt,ui=true){G.t+=dt;if(G.shake>0)G.shake-=dt;
  ...
  if(G.booted&&!G.endlessSeen&&ZONES[G.zone]&&ZONES[G.zone].endless&&G.tut==null&&G.mode==='walk'&&G.screen==='road'&&!G.sheet&&!G.title){G.endlessSeen=true;saveGame();G.tut=TUT.length;TUT.push({t:'The Endless Road',s:'No shard waits at the end of this one. The road winds back through every land, and the enemies keep growing.',s2:'A champion blocks every 25th fight. Every 100th, the boss of the land you are crossing guards a milestone, with rewards the first time you beat it. Fall, and you return to the last champion.',box:[4,SCENE_Y+4,166,42],once:true});}
