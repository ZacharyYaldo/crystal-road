...
function nodeTotal(n,rank){if(!rank)return '';const d=n.desc;let m=d.match(/([+\-]?\d+(?:\.\d+)?)(%?)(?=[^\d%]*per rank)/);if(!m)m=d.match(/([+\-]?\d+(?:\.\d+)?)(%?)/);if(!m)return '';const per=parseFloat(m[1]),pct=m[2],tot=Math.round(per*rank*100)/100;const unit=/min/.test(d.slice(m.index,m.index+12))?' min':'';return ' · '+(tot>0?'+':'')+tot+pct+unit+' total';} /* cumulative effect of the ranks bought: per-rank number x rank */
