...
function upgradeItem(it,h){let n=0,want=G.mult==='max'?1e9:G.mult;while(n<want){const cost=upgradeCost(it);if(G.ore<cost)break;G.ore-=cost;it.lvl++;n++;}if(!n){toast('Need '+fmtNum(upgradeCost(it))+' ore');return;}if(h)refreshStats(h);toast(itemName(it)+' +'+it.lvl+(n>1?' ('+n+' ranks)':''));}
