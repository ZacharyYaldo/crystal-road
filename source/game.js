...
const TREE=[
  ...
  {branch:'Party',id:'spd',name:'Quickstep',desc:'+2% action speed',max:25,base:900,cur:'gold'},
  {branch:'Party',id:'crit',name:'Keen Edge',desc:'+1% critical chance',max:40,base:250,cur:'gold'},
  {branch:'Party',id:'abil',name:'Focus',desc:'+2% ability power',max:30,base:150,cur:'gold'},
  {branch:'Party',id:'front',name:'Vanguard',desc:'+5% front hero defense',max:999,base:160,cur:'gold'},
  ...
  {branch:'Camp',id:'offline',name:'Night Watch',desc:'+30 min offline cap',max:8,base:800,cur:'gold'},
  ...
  {branch:'Crystal',id:'b_dust',name:'Crystal Memory',desc:'+5% dust from every Shatter',max:20,base:8,cur:'dust'},
...
function reforgeGain(){const sum=reforgeParts().reduce((a,p)=>a+p[1],0);return R(sum*(1+0.05*treeLv('b_dust'))*(G.oath&&OATHS[G.oath]?OATHS[G.oath].dust:1));}
...
function collectQuest(q){const mult=(1+0.06*treeLv('quest'))*questBonus(q.hero,q.q)*refMult();const L=q.hero.lvl;let msg='';
  if(q.q.reward==='gold'){const g=R((30+L*8)*mult);G.gold+=g;msg='+'+fmtNum(g)+' gold';}
  else if(q.q.reward==='ore'){const o=R((8+L)*mult);G.ore+=o;msg='+'+fmtNum(o)+' ore';if(rand()<0.4&&G.pack.length<15){G.pack.push(makeItem(G.zone));msg+=' and gear';}}
...
function orePerHour(){return R(G.castle.b.mine*G.castle.vil.mine*VAEL_ORE_BASE*refMult());}
function goldPerHour(){return R(G.castle.b.market*G.castle.vil.market*VAEL_GOLD_BASE*refMult());}
...
function resolveHorde(fought){const C2=G.castle,d=defense().total,s=hordeThreat();C2.hordeDue=false;if(fought!=null){if(fought){C2.repelled++;C2.streak=(C2.streak||0)+1;G.stats.hordes++;const HL=(G.hordeFight&&G.hordeFight.lvl)||(C2.lastHordeLv||1);const g=R((3+HL*2)*Math.pow(1.05,HL)*30*bless('gold')*refMult()),du=2+Math.floor(HL/40),v=2+Math.floor(rand()*2);G.gold+=g;G.dust+=du;C2.vil.total+=v;if(G.pack.length<15){const it=makeItem(G.zone,'boss');G.pack.push(it);G.stats.items++;}banner('The gate holds','+'+fmtNum(g)+' gold, +'+du+' dust, '+v+' villagers, a chest',3.5,'any');}else{C2.lost++;C2.streak=0;const lost=Math.min(2,C2.vil.total);C2.vil.total-=lost;banner('The gate falls',lost+' villagers lost',3.5,'any');}hordeAfter(!!fought);return;}
  if(d>=s){C2.repelled++;C2.streak=(C2.streak||0)+1;G.stats.hordes++;const HL=hordeCombatLevel();const g=R((3+HL*2)*Math.pow(1.05,HL)*15*bless('gold')*refMult()),v=1+Math.floor(rand()*2);G.gold+=g;G.dust+=1;C2.vil.total+=v;banner('The horde breaks on the walls','+'+fmtNum(g)+' gold, +1 dust, '+v+' villagers rally',3.5,'any');hordeAfter(true);}
...
function drawVael(){drawPanelScreen();castleTick();const C2=G.castle,D=defense(),S=hordeThreat();if(!C2.currentHorde)genHorde();
  ...
  // cards
  const cards=[
    {k:'mine',ic:()=>pixGrid(CASTLE_PIX.mine.slice(2).map(r=>r.slice(4,18)),19,y0+19),sub:C2.b.mine?C2.vil.mine+' villagers · +'+fmtNum(orePerHour())+' ore/h · '+fmtNum(C2.stored.ore)+' stored':'Not built · '+BUILD.mine.desc(0),vil:true},
...
function chooseDoor(kind){const d=G.delve;if(!d||!d.choose)return;d.choose=false;const h=d.hero,sc=delveScale();
  ...
  if(kind==='chest'){if(rand()<0.6){const it=makeItem(Math.min(ZONES.length-1,Math.floor(d.floor/4)),'boss');d.bag.items.push(it);toast('Chest: '+itemName(it),RANKHEX[it.rank]);}else{const o=R((6+d.floor*3)*sc.mult);d.bag.ore+=o;toast('Chest: +'+fmtNum(o)+' ore');}}
