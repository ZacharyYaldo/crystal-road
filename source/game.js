...
function insets(){const cs=getComputedStyle(_probe);const et=parseFloat(cs.paddingTop)||0,eb=parseFloat(cs.paddingBottom)||0;const portrait=innerHeight>=innerWidth;const sh=portrait?screen.height:screen.width;const gap=Math.max(0,sh-innerHeight);let top=et,bot=gap>=et+eb-2?0:eb;const sa=navigator.standalone===true&&/iPhone|iPad|iPod/.test(navigator.userAgent);if(sa&&gap<4&&et===0){top=sh>=812?50:20;bot=sh>=812?34:0;}return{top,bot,et,eb,gap,sh};}
function padsApply(){const p=insets();const w=document.getElementById('wrap');if(!w)return p;const t=p.top+'px',b=p.bot+'px';if(w.style.paddingTop!==t)w.style.paddingTop=t;if(w.style.paddingBottom!==b)w.style.paddingBottom=b;return p;}
const cv=document.getElementById('c'),tc=document.getElementById('t'),stage=document.getElementById('stage');
cv.width=W;const ctx=cv.getContext('2d');ctx.imageSmoothingEnabled=false;
const tx=tc.getContext('2d');let SC=1,TS=1;
const wrapEl=document.getElementById('wrap');function availH(){if(!wrapEl)return innerHeight;const cs=getComputedStyle(wrapEl);const vv=window.visualViewport?window.visualViewport.height:wrapEl.clientHeight;return Math.min(vv,wrapEl.clientHeight)-(parseFloat(cs.paddingTop)||0)-(parseFloat(cs.paddingBottom)||0);}
function wantH(){padsApply();const a=availH();return Math.max(480,Math.min(600,Math.round(W*a/(innerWidth||270))));}
function fit(){const want=wantH();if(want!==H)applyH(want);const sc=Math.min(innerWidth/W,availH()/H);const dpr=Math.min(3,window.devicePixelRatio||1);const ts=Math.max(3,sc*dpr);
...
const WEAPON_ADJ=['Iron','Jade','Mithril','Amethyst','Sunforged'],WEAPON_NOUN={sword:'Blade',staff:'Staff',dagger:'Daggers',bow:'Bow',axe:'Axe'};
const CAPE_NAMES=['Wool Cape','Verdant Cape','Mariner Cape','Royal Cape','Sunforged Cape'],CHARM_NAMES=['Iron Band','Jade Ring','Moon Amulet','Amethyst Ring','Sun Amulet'];
...
function heroStats(h){const c=CLASSES[h.cls],L=h.lvl-1,tm=Math.pow(1.3,h.tier||0);let hp=(c.hp+c.ghp*L)*tm,atk=(c.atk+c.gatk*L)*tm,def=(c.def+c.gdef*L)*tm;
  ...
  hp*=(1+0.04*treeLv('hp'))*(1+0.1*treeLv('b_hp'));atk*=1+0.04*treeLv('atk');def*=1+0.04*treeLv('def');return{maxhp:R(hp),atk:R(atk),def:R(def),spd:c.spd};}
...
function ascendItem(it,h){if(it.rank>=4){toast('Already Sunforged');return;}if(it.rank>=ascendCap()){toast(RANKS[it.rank+1]+' needs '+(it.rank+1-(G.reforges||0))+' more Reforge'+(it.rank-(G.reforges||0)?'s':''));return;}const c=ascendCost(it);if(G.ore<c){toast('Need '+fmtNum(c)+' ore');return;}G.ore-=c;it.rank++;if(h)refreshStats(h);sfx('level',true);toast(itemName(it)+' - ascended to '+RANKS[it.rank],RANKHEX[it.rank]);}
...
function loadAll(){const ps=[];const cap=p=>Promise.race([p,new Promise(r=>setTimeout(r,8000))]);for(const k of ['logo','roadIcon','tab_heroes','tab_gear','tab_vael','tab_tree'])if(ASSETS[k]){const im=new Image();ps.push(cap(new Promise(res=>{im.onload=res;im.onerror=res;})));im.src=ASSETS[k];IMG[k]=im;}
...
const WTINT=[null,[130,0.55,1.0],[210,0.6,1.0],[285,0.55,1.0],[45,0.85,1.0]];
...
function spawnEncounter(){G.fs={dmg:0,taps:0,gold:0};
  const Z=ZONES[G.zone],p=G.prog[G.zone];const bossFight=!!(Z.boss&&p===Z.fights-1&&!G.cleared[G.zone]);const cpS=Z.fights/3,elite=!bossFight&&!G.cleared[G.zone]&&(p===2*cpS||(Z.endless&&p>0&&p%25===0));
  const n=(bossFight||elite)?1:1+Math.floor(rand()*Math.min(3,1+p/3));G.enemies=[];
  const pool=Z.endless?ZONES[endSeg(p)].pool:Z.pool,etier=Math.min(2,Z.endless?Math.floor(p/50):Math.floor(p/(Z.fights/3)));for(let i=0;i<n;i++){const id=bossFight?Z.boss:pool[Math.floor(rand()*pool.length)];const L=Z.endless?endlessLv(Z,p):R(Z.lv+p*0.3+(G.cleared[G.zone]?3:0));
  ...
  G.mode='enter';G.bossFight=bossFight;G.active.forEach(h=>setAnim(h,'idle'));if(bossFight)banner(ENEMIES[Z.boss].name,'holds '+(3+G.zone)+' prisoners',2.5);else if(elite)banner(G.enemies[0].name,'blocks the road',2.2);
}
...
function dealDamage(src,tgt,mult=1,opts={}){
  ...
  if(tgt.status&&tgt.status.shield>0)dmg*=(1-(tgt.status.shieldPct||0.3));if(tgt.status&&tgt.status.ward>0)dmg*=0.7;if(tgt.status&&tgt.status.rage>0&&!(tgt.talents&&tgt.talents.defiant&&tgt.hp<tgt.maxhp*0.3))dmg*=(tgt.enemy?1.5:1.3);if(tgt.status&&tgt.status.burn>0)dmg*=1.2;
...
function updateAction(dt){const a=G.action,u=a.u;
  ...
  if(a.kind==='melee'||a.kind==='chargeM'||(a.kind==='ability'&&a.ab==='shadowstep')){
    ...
    else if(a.phase==='hit'){if(!a.hitDone&&u.frame>=a.hit){a.hitDone=true;if(!tgt.dead){if(a.kind==='ability'){a.hits++;let m2=0.9*a.pow;if(u.talents&&u.talents.executioner&&tgt.hp<tgt.maxhp*0.3)m2*=1.5;dealDamage(u,tgt,m2,{crit:a.hits===3,noHurt:a.hits<3});}else if(a.kind==='chargeM'){dealDamage(u,tgt,2.5);G.shake=0.3;}else{let m3=1;if(!u.enemy&&u.talents&&u.talents.mark)m3*=1+0.05*Math.min(10,tgt.marks||0);dealDamage(u,tgt,m3);}}}
...
function endBattle(win){if(G.hordeFight){if(!win){endHordeFight(false);return;}if(G.hordeFight.wave>=3){endHordeFight(true);return;}G.enemies=[];G.mode='delve';setTimeout(()=>{if(G.hordeFight)hordeWave();},700);return;}if(G.delve){if(win)delveWin();else endDelve(true);return;}const Z=ZONES[G.zone];
  if(win){G.wins++;G.losses=0;G.run.fights++;checkUnlocks();if(canReforge()){G.dustFrac=(G.dustFrac||0)+(1+0.25*G.zone+0.5*(G.reforges||0))/150;if(G.dustFrac>=1){const d=Math.floor(G.dustFrac);G.dustFrac-=d;G.dust+=d;}}if(G.bossFight){G.run.bosses++;if(canReforge()){G.dust+=2;}G.castle.bp=G.castle.bp||{};const sk=Object.keys(STRUCTS).find(k=>STRUCTS[k].boss===Z.boss);if(sk&&!G.castle.bp[sk]){G.castle.bp[sk]=true;G.pending='Blueprint found|'+STRUCTS[sk].name+' - build it in Vael';G.vaelNew=true;}const bn=ENEMIES[Z.boss].name,fs=G.fs||{dmg:0,taps:0,gold:0};G.card={title:bn,sub:'falls',lines:['Damage dealt '+fmtNum(fs.dmg)+'  ·  taps '+fs.taps,'Gold '+fmtNum(fs.gold)],t:0};}
    ...
    if(!G.cleared[G.zone]){G.prog[G.zone]++;if(Z.endless&&G.prog[G.zone]%30===0)banner(ZONES[endSeg(G.prog[G.zone])].name,'The road winds on',2.4);G.far=G.far||[];G.far[G.zone]=Math.max(G.far[G.zone]||0,G.prog[G.zone]);{const cs=Z.fights/3,p2=G.prog[G.zone];if(p2%cs===0&&p2<Z.fights){G.cpHint=false;const cn=p2/cs,g2=R((40+80*cn)*(1+G.zone)*bless('gold'));G.gold+=g2;G.card={title:'Checkpoint '+cn+' of 3',sub:'The road behind you is safe',lines:['+'+fmtNum(g2)+' gold',cn===1?'Fall in battle and you return here':cn===2?'A champion blocks the way ahead':'The boss waits at the end of the road'],t:0};}}
...
function doReforge(){const gain=reforgeGain();G.dust+=gain;G.reforges=(G.reforges||0)+1;const startLv=1+5*treeLv('b_start');
  ...
  G.run={start:now(),fights:0,bosses:0,gold:0,bestFloor:0};G.gold=0;G.ore=R(refund);G.prog=ZONES.map(()=>0);G.far=ZONES.map(()=>0);G.cleared=ZONES.map(()=>false);G.zone=0;G.enemies=[];G.projs=[];G.action=null;G.mode='walk';G.enc=4;G.wins=Math.max(G.wins,3);G.newZone=false;
...
// ============================================================ gear
function equipBest(only){for(const h of (only?[only]:G.roster)){for(const slot of ['weapon','cape','charm']){const cands=G.pack.filter(it=>it.slot===slot&&(slot!=='weapon'||it.kind===CLASSES[h.cls].weapon));const cur=h.eq[slot];const score=it=>it.rank*10000+itemStat(it);const best=cands.sort((a,b)=>score(b)-score(a))[0];
...
function update(dt,ui=true){G.t+=dt;if(G.shake>0)G.shake-=dt;
  for(const h of G.active){if(h.tx!=null&&Math.abs(h.x-h.tx)>0.2)h.x+=(h.tx-h.x)*Math.min(1,dt*6);else if(h.tx!=null)h.x=h.tx;h.hop=Math.max(0,h.hop-dt*2.5);if(G.mode==='battle'&&!h.dead&&h.charge<100){h.charge=Math.min(100,h.charge+7*dt*(G.delve&&G.delve.relics.find(r=>r.charge)?2:1));if(h.charge>=100)h.readyT=0;}if(h.charge>=100&&!h.dead)h.readyT+=dt;}
...
function drawTabs(){px(0,TABY,W,42,C.out);px(0,TABY,W,1,C.gold);px(0,TABY+1,W,1,C.goldD);
  ...
  tabs.forEach(([ic,label,id],i)=>{const x=i*54,on=G.screen===id||(id==='road'&&G.screen==='map'),lk=tabLocked(id);
    ...
    if(id==='vael'&&!lk&&!on&&keysNow()>=keyMax()&&!G.keysSeen){ctx.globalAlpha=0.4+0.4*Math.sin(RT*2);px(x+2,TABY+3,50,1,C.goldL);px(x+2,TABY+39,50,1,C.goldL);ctx.globalAlpha=1;}
...
function drawScene(){const Z=ZONES[G.zone];const key=G.hordeFight?'door':G.delve?(G.delve.floor>=12?'door':'cave'):((G.bossFight&&Z.bossScene)?Z.bossScene:zoneScene(Z,G.prog[G.zone]));const layers=SCENES[key];
  ...
  text(9,SCENE_Y+7,Z.endless?'Endless · '+ZONES[endSeg(G.prog[G.zone])].name:Z.name,'sb',C.goldL);{const hl=hordeLeft(),due=G.castle.hordeDue;text(9,SCENE_Y+17,due?'Horde at the gate - tap Vael to defend':(hl<900&&(G.castle.b.walls||G.castle.vil.total)?'Horde in '+fmtT(hl)+'  ·  Lv '+Z.lv:(Z.endless?'Fight '+(G.prog[G.zone]+1)+' · Lv '+endlessLv(Z,G.prog[G.zone])+' · best '+Math.max(G.prog[G.zone],(G.far||[])[G.zone]||0):'Danger Lv '+Z.lv+(G.cleared[G.zone]?'  ·  Shard recovered':''))),'xs',due?'#ff8a80':(hl<900&&(G.castle.b.walls||G.castle.vil.total)?'#ffb080':C.muted));}hit(4,SCENE_Y+4,166,24,()=>{G.screen='map';G.newZone=false;G.reforgeNew=false;});
  {const lx=14,rx=156,ly=SCENE_Y+35;if(Z.endless){const p9=G.prog[G.zone],nx=25-(p9%25);text(14,ly-4,'The road goes on  ·  champion in '+nx+(nx===1?' fight':' fights'),'xs',C.muted);}else{px(lx,ly,rx-lx,1,'#3a4260');px(lx,ly,R((rx-lx)*far/Z.fights),1,'#5a6488');px(lx,ly,R((rx-lx)*p/Z.fights),1,G.cleared[G.zone]?'#7fd4ff':C.goldL);
...
function drawBanner(){if(G.tut!=null)return;if(G.banner&&(G.banner.scope==='any'||G.screen==='road')){const b=G.banner,al=clamp(Math.min(b.t*3,(b.dur-b.t)*2),0,1),by=G.screen==='vael'?92:SCENE_Y+120;ctx.globalAlpha=al*0.6;px(0,by,W,34,'#000');ctx.globalAlpha=1;text(W/2,by+6,b.text,'big',C.goldL,'center',al);if(b.sub)text(W/2,by+21,b.sub,'s',C.cream,'center',al);}}
...
function drawGear(){drawPanelScreen();const h=G.roster[G.gearHero]||G.roster[0];
  ...
  ['weapon','cape','charm'].forEach((slot,i)=>{const x=4+i*88,it=h.eq[slot];frame(x,SY,86,SLOTH,C.navy,it&&it.rank>0?RANKHEX[it.rank]:C.gold);
    ...
    if(it){icon(itemIcon(it),x+31,SY+14,it.rank,2);const nm=itemName(it);text(x+43,SY+43,nm,textW(nm,'sb')>78?'xsb':'sb',it.rank>0?RANKHEX[it.rank]:C.cream,'center');text(x+43,SY+53,'★'.repeat(it.rank+1)+'☆'.repeat(4-it.rank)+(it.lvl?'  ↑'+Math.round(roll('it_'+it.id,it.lvl)):''),'xs',C.goldL,'center');text(x+43,SY+61,itemStatLabel(it).replace(fmtNum(itemStat(it)),fmtNum(roll('is_'+it.id,itemStat(it)))),'xs',C.green,'center');
  ...
  if(G.salvage){button(146,PY+6,54,PB,sel.length>0,!sel.length);text(173,PY+6+PB/2-4,sel.length?'Salvage '+sel.length:'Select…','xs',sel.length?C.goldL:C.dimt,'center');
    if(sel.length)hit(146,PY+6,54,PB,()=>{for(const it of sel)G.pack=G.pack.filter(i=>i!==it);G.ore+=ore;toast('Salvaged '+sel.length+' for '+ore+' ore');G.sel=[];G.salvage=false;});
  ...
  for(let k=0;k<15;k++){const cx=k%5,cy=Math.floor(k/5),x=gx+cx*step,y=gy+cy*step,it=G.pack[k];
    ...
    const col=RANKHEX[it.rank];const on=G.salvage?sel.includes(it):G.selItem===it,usable=it.slot!=='weapon'||it.kind===CLASSES[h.cls].weapon;px(x,y,CELL,CELL,C.out);px(x+1,y+1,CELL-2,CELL-2,on?C.goldL:col);px(x+2,y+2,CELL-4,CELL-4,C.out);px(x+3,y+3,CELL-6,CELL-6,on?'#3a3420':'#1e223a');ctx.globalAlpha=usable?1:0.35;icon(itemIcon(it),x+CELL/2-12,y+CELL/2-14,it.rank,2);ctx.globalAlpha=1;if(!usable)text(x+CELL/2,y+CELL-13,wielders(it),'xs',C.dimt,'center');
...
function drawHeroes(){drawPanelScreen();text(8,26,'Heroes','title',C.goldL);text(262,30,G.roster.length+' recruited','xs',C.muted,'right');beginScroll('heroes',44);
  let y=46;const HR=BIG?110:98;for(const h of G.roster){frame(4,y,262,HR);drawSprite(h,26,y+38,0,'idle',false);const st=heroStatus(h);
...
function saveGame(){if(G.title||G.noSave||G.delve||G.hordeFight)return;storeSet(serialize());}
...
function drawWelcome(s,y0){const o=G.offline;text(16,y0+8,'While you were away','h',C.goldL);text(16,y0+22,fmtT(o.hours*3600)+' on the road · '+fmtNum(o.fights)+' battles in '+ZONES[G.zone].name,'xs',C.muted);
  let y=y0+36;icon('coin',20,y,4);text(36,y+1,'+'+fmtNum(o.gold)+' gold','sb',C.goldL);icon('ore',150,y,0);text(166,y+1,'+'+fmtNum(o.ore)+' ore','sb',C.cream);y+=16;
  ...
  if(!o.doubled){button(16,y,110,18,true);icon('play',22,y+4,4);text(75,y+5,'Double it (ad)','sb',C.goldL,'center');hit(16,y,110,18,()=>{G.gold+=o.gold;G.ore+=o.ore;o.doubled=true;toast('Rewards doubled');});}
...
function drawSheet(){const s=G.sheet;if(!s)return;tx.clearRect(0,0,tc.width,tc.height);dimRect(0,0,W,H,0.75);hit(0,0,W,H,()=>{if(s.kind!=='welcome')closeSheet();});
  ...
  const hh=s.kind==='horde'?150:s.kind==='welcome'?(160+Math.ceil(((G.offline&&G.offline.levels)||[]).length/2)*11+((G.offline&&G.offline.items&&G.offline.items.length)?11:0)):s.kind==='blueprints'?36+30*Object.keys(STRUCTS).length:s.kind==='stats'?190:s.kind==='reforge'?150:s.kind==='delve'?54+30*G.roster.filter(h=>heroStatus(h)!=='quest').length:s.kind==='ad'?196:s.kind==='reforge'?96:s.kind==='speedad'?92:s.kind==='autoad'?92:s.kind==='welcome'?122:s.kind==='garrison'?46+30*Math.max(1,G.roster.filter(h=>['camp','garrison'].includes(heroStatus(h))).length):s.kind==='swap'?118+30*Math.max(1,camp.length):s.kind==='send'?(s.hero?40+38*QUESTS.length:40+30*Math.max(1,free.length)):s.kind==='settings'?352:96;
  ...
  if(s.kind==='swap'){text(16,y0+8,'Party',"h",C.goldL);text(16,y0+20,'Tap a marcher, then a hero at camp to trade places.','xs',C.muted);text(16,y0+29,'Two marchers to reorder. Left slot is the front.','xs',C.muted);
    ...
    const rows=[['Time on the road',hrs>=1?hrs.toFixed(1)+' h':Math.round(hrs*60)+' min'],['Fights won',fmtNum(r.fights)],['Bosses beaten',String(r.bosses)],['Gold earned',fmtNum(r.gold)],['Deepest catacomb floor',String(r.bestFloor||0)],['Highest hero level',String(best)]];
    rows.forEach(([k,v],i)=>{const y=y0+24+i*11;text(20,y,k,'xs',C.muted);text(250,y,v,'xsb',C.cream,'right');});
    text(16,y0+94,'Reforge for +'+reforgeGain()+' dust. You keep your heroes, talents, gear,','xs','#bbdefb');text(16,y0+103,'Vael, the tree and blessings. Gear upgrades pay back half their ore.','xs','#bbdefb');
    button(16,y0+118,110,22,true);text(71,y0+125,'Begin run '+((G.reforges||0)+2),'sb',C.goldL,'center');hit(16,y0+118,110,22,doReforge);button(140,y0+118,110,22,false);text(195,y0+125,'Not yet','xs',C.cream,'center');hit(140,y0+118,110,22,closeSheet);}
  else if(s.kind==='stats'){const st=G.stats;const rows=[['Enemies defeated',fmtNum(st.kills)],['Bosses defeated',String(st.bosses)],['Highest hit',fmtNum(st.hit)],['Gold earned',fmtNum(st.gold)],['Items found',String(st.items)],['Sunforged found',String(st.legendary)],['Taps',fmtNum(st.taps)],['Hordes repelled',String(st.hordes)],['Offline fights',fmtNum(st.offline)],['Catacomb runs',String(st.delveRuns)],['Deepest floor',String(G.delveBest||0)],['Reforges',String(G.reforges||0)],['Defeats',String(st.defeats||0)],['Road time',fmtT(G.simT||0)]];
    ...
    text(16,y0+292,storeSet('__probe')?'Progress saves automatically on this device.':'This preview cannot save - download the file to keep progress.','xs',C.dimt);}
...
function trackFor(){const Z=ZONES[G.zone]||ZONES[0];const zi=Z.endless?endSeg(G.prog[G.zone]||0):G.zone;const M=ASSETS.music||{};const k=['greenhollow','lagoon','thornwood','ironvein','emberwaste','amberfall'][zi]||'greenhollow';return M[k]?k:'greenhollow';}
...
function playMusic(key){if(!ASSETS.music||!ASSETS.music[key])return;if(music.cur===key)return;music.cur=key;
  if(music.bufs[key]){swapTo(music.bufs[key]);return;}
  const a=ac();if(a&&window.fetch){music.decoding=true;fetch(ASSETS.music[key]).then(r=>r.arrayBuffer()).then(ab=>a.decodeAudioData(ab)).then(buf=>{buf=loopify(a,buf,key);music.bufs[key]=buf;music.decoding=false;if(music.cur===key)swapTo(buf);}).catch(()=>{music.decoding=false;elementFallback(key);});}
  else elementFallback(key);}
...
function checkUpdate(){try{fetch('version.json?t='+Date.now(),{cache:'no-store'}).then(r=>r.json()).then(j=>{if(j&&j.build&&typeof BUILD_ID==='string'&&j.build!==BUILD_ID){G.updateReady=j.build;}}).catch(()=>{});}catch(e){}}
...
function drawTutorial(){if(G.tut==null)return;const st=TUT[G.tut];hits.length=0;let[bx,by,bw,bh]=typeof st.box==='function'?st.box():st.box;bx=Math.max(2,bx);by=Math.max(18,by);bw=Math.min(bw,W-4-bx);bh=Math.min(bh,TABY-2-by);
  ...
  const lines=[].concat(wrapN(st.s,'s',228),st.s2?wrapN(st.s2,'s',228):[],st.s3?wrapN(st.s3,'s',228):[]);const extra=(lines.length-1)*11,ch=72+extra;const cands=[by+bh+10,by-ch-10,40,TABY-ch-10];let py=cands.find(y=>y>=20&&y+ch<=TABY-4&&!(y<by+bh&&y+ch>by));if(py==null)py=by>H/2?40:TABY-ch-10;frame(12,py,246,ch,C.navy,C.goldL);tx.clearRect(12*TS,py*TS,246*TS,ch*TS);text(20,py+8,st.t,'h',C.goldL);lines.forEach((l,i)=>text(20,py+23+i*11,l,'s',C.cream));if(!st.once)text(20,py+46+extra,(G.tut+1)+' / '+TUT.length,'xs',C.dimt);hit(0,0,W,H,()=>{});button(170,py+42+extra,80,22,true);text(210,py+49+extra,st.once?'Got it':(G.tut<TUT.length-1?'Next':'Begin'),'sb',C.goldL,'center');hit(170,py+42+extra,80,22,()=>{const once=TUT[G.tut]&&TUT[G.tut].once;G.tut++;if(G.tut===1&&G.mode==='walk'&&!G.enemies.length){const E=ENEMIES.slime,e={uid:'slime',id:'slime',name:'Slime',lvl:1,x:224,slot:224,dx:0,yoff:0,flip:true,fly:false,range:'melee',boss:false,scale:1,native:false,nscale:1,hue:0,tier:0,gauge:0,dead:false,enemy:true,status:{},flash:0,traits:[],acts:0};Object.assign(e,enemyStats('slime',1));e.hp=e.maxhp;setAnim(e,'idle');G.enemies=[e];G.fs={dmg:0,taps:0,gold:0};G.mode='battle';G.active.forEach(h=>setAnim(h,'idle'));}if(once){if(TUT[G.tut]&&TUT[G.tut].once){return;}while(TUT.length&&TUT[TUT.length-1].once)TUT.pop();G.tut=null;saveGame();return;}if(G.tut>=TUT.length){G.tut=null;G.tutDone=true;G.screen='road';saveGame();}else G.screen=TUT[G.tut].screen||'road';});}
function draw(){hits.length=0;tx.clearRect(0,0,tc.width,tc.height);px(0,0,W,H,'#0b0d16');
  ({road:drawRoad,gear:drawGear,heroes:drawHeroes,vael:drawVael,map:drawMap,tree:drawTree})[G.screen]();drawDragGhost();if(G.updateReady&&!G.sheet&&G.tut==null){const uw=112,ux=W/2-uw/2,uy=TABY-30;ctx.globalAlpha=0.6+0.4*Math.sin(RT*3);button(ux,uy,uw,20,true);ctx.globalAlpha=1;text(W/2,uy+6,'Update ready - tap to reload','xs',C.goldL,'center');hit(ux,uy,uw,20,applyUpdate);}
  ...
  if(G.title){hits.length=0;tx.clearRect(0,0,tc.width,tc.height);px(0,0,W,H,'#0b0d16');const L=SCENES.hills;ctx.save();ctx.beginPath();ctx.rect(0,140,W,208);ctx.clip();for(const l of L){const w=l.img.width||368;let off=-(Math.floor(RT*10*l.p)%w);for(let x=off;x<W+w;x+=w)ctx.drawImage(l.img,x,140);}ctx.restore();
    ...
    const a=0.55+0.45*Math.sin(RT*3);text(W/2,388,'Tap to begin','h',C.goldL,'center',a);text(W/2,404,'Music by you · art by Zerie & ansimuz','xs',C.dimt,'center');if(typeof BUILD_ID==='string')text(W/2,H-12,'build '+BUILD_ID,'xs',C.dimt,'center');hit(0,0,W,H,()=>{G.title=false;startMusic();banner(ZONES[G.zone].name,G.wins>0?'Onward':'The first shard lies ahead',3);if(!G.tutDone&&G.wins===0){G.tut=0;G.banner=null;G.pending='Greenhollow Fields';}});}
  if(G.toast&&!(G.toast.scope==='road'&&G.screen!=='road')){const a=clamp(Math.min(G.toast.t*4,(2.2-G.toast.t)*3),0,1);if(G.toast.scope==='road'){const w=textW(G.toast.text,'xs')+12;ctx.globalAlpha=a*0.45;px(4,SCENE_Y+192,R(w),12,'#000');ctx.globalAlpha=1;text(10,SCENE_Y+194,G.toast.text,'xs',G.toast.col||C.cream,'left',a);}else{const w=textW(G.toast.text,'s')+16,ty=G.screen==='road'&&!G.sheet?SCENE_Y+186:H-64;ctx.globalAlpha=a*0.55;px(R(W/2-w/2),ty,R(w),16,'#000');ctx.globalAlpha=1;text(W/2,ty+4,G.toast.text,'s',G.toast.col||C.cream,'center',a);}}}
