...
function draw(){hits.length=0;PRESSED=[];tx.clearRect(0,0,tc.width,tc.height);px(0,0,W,H,'#0b0d16');
  ...
  else{const lines=wrap2(G.toast.text,'sb',W-44),w=W-12,hh=12+lines.length*10,onRoad=false,dur=G.toast.dur||2.2,k=onRoad?1:clamp(Math.min(G.toast.t*4,(dur-G.toast.t)*4),0,1),ease=1-Math.pow(1-k,3),ty=onRoad?SCENE_Y+200-hh:R(20-(hh+22)*(1-ease)),tx0=6;if(!onRoad)a=1;tx.clearRect(tx0*TS,Math.max(0,ty)*TS,R(w)*TS,hh*TS);ctx.globalAlpha=a;px(tx0,ty,R(w),hh,'#0b0d16');px(tx0,ty,R(w),1,C.goldD);px(tx0,ty+hh-1,R(w),1,C.goldD);px(tx0,ty,1,hh,C.goldD);px(tx0+R(w)-1,ty,1,hh,C.goldD);ctx.globalAlpha=1;lines.forEach((l,i)=>text(W/2,ty+6+i*10,l,'sb',G.toast.col||C.cream,'center',a));}}}
