...
function startTraining(){const ret=G.zone,zone=ret-1,S=atStats();const D=G.danger||{r:[]};const prev=(G.train&&!G.train.active&&G.train.after&&G.train.after.zone===ret)?{fights:G.train.returnFights||0,minutes:+((now()-(G.train.returnedAt||now()))/60000).toFixed(1)}:null;const Zr=ZONES[ret];const rec={ret:ZONES[ret].name,zone:ZONES[zone].name,prog:G.prog[ret],enemyLv:roadLv(Zr,Zr.lv+G.prog[ret]*0.3),window:D.r.join(''),retrigger:prev,before:partyState(),lvBefore:+partyLvAvg().toFixed(1),winBefore:+(D.r.filter(x=>x).length/Math.max(1,D.r.length)).toFixed(2),t0:now(),lvAfter:null,secs:null,fights:null,xp:null,winAfter:null};
