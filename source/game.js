...
function promoteHero(h){const t=h.tier||0;if(t>=3){toast(h.name+' is already a '+heroTitle(h));return;}const q=promoteReq(h);if(h.lvl<q.lvl){toast('Reach Lv '+q.lvl+' first');return;}if(G.gold<q.gold||G.dust<q.dust){toast('Needs '+fmtNum(q.gold)+' gold and '+q.dust+' dust');return;}G.gold-=q.gold;G.dust-=q.dust;h.tier=t+1;h.tierMax=Math.max(h.tierMax||0,h.tier);h.uid=heroUid(h);refreshStats(h);h.hp=h.maxhp;sfx('level',true);levelBurst(h);banner(h.name+' becomes a '+heroTitle(h),'+30% to every stat, ability power up',3,'any');}
