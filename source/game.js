...
function update(dt,ui=true){if(G.mode==='battle')dt*=BATTLE_SPEED;G.t+=dt;if(G.shake>0)G.shake-=dt;
  ...
  if(G.gearHint&&G.tut==null&&G.mode==='walk'&&G.screen==='road'&&!G.sheet){G.gearHint=false;G.tut=TUT.length;TUT.push({t:'Strengthen the party',s:'Equip gear on every hero.',s2:'Rank up abilities in Heroes.',s3:'Spend gold in the Tree.'+(G.wins<3?' It opens after three wins.':''),font:'b',tabs:['heroes','gear','tree'],box:[0,TABY,W,42],once:true});}
