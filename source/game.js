...
const TREE=[
  ...
  {branch:'Party',id:'rest',name:'Warm Fire',desc:'+2% to the 15% fight heal',total:r=>'heals '+Math.min(100,15+2*r)+'%',max:40,base:90,cur:'gold'},
...
function dealDamage(src,tgt,mult=1,opts={}){
  ...
  if(!crit&&c&&rand()<Math.min(CRIT_CAP,(c.crit||0)+0.01*treeLv('crit')+0.02*treeLv('b_crit')+(built('range')?0.05:0)))crit=true;if(crit)dmg*=1.6;
...
function reforgeGain(){const sum=reforgeParts().reduce((a,p)=>a+p[1],0);return R(sum*(1+0.10*treeLv('b_dust'))*(G.oath&&OATHS[G.oath]?OATHS[G.oath].dust:1));}
