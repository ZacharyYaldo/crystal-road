...
function nextTrackKey(){const Z=ZONES[G.zone];if(!Z)return null;const p=G.prog[G.zone]||0;const M=['greenhollow','lagoon','thornwood','ironvein','emberwaste','amberfall','ashen'];let zi=null;if(Z.endless){if(p%30>=26)zi=endSeg(p+4);}else if(!G.cleared[G.zone]&&p>=Z.fights-4&&G.zone<ZONES.length-1)zi=G.zone+1;if(zi==null)return null;const k=M[zi]||'greenhollow';return (ASSETS.music&&ASSETS.music[k])?k:null;}
