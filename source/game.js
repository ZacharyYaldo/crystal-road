...
function dispatch(x,y){for(let i=hits.length-1;i>=0;i--){const h=hits[i];if(x>=h.x&&x<h.x+h.w&&y>=h.y&&y<h.y+h.h){G.lastHit={x:h.x,y:h.dy,w:h.w,h:h.h,until:RT+0.12};spendCall(h.fn,x,y,h);if(h.hold){const key=h.x+','+h.dy+','+h.w,again=LAST_HOLD&&LAST_HOLD.key===key&&RT-LAST_HOLD.end<0.35;HOLD={fn:h.fn,next:RT+0.45,r:h,px:x,py:y,toastText:again?LAST_HOLD.toastText:((G.toast&&G.toast.t===0)?G.toast.text:null)};}if(G.drag&&!G.drag.x0){G.drag.x0=x;G.drag.y0=y;G.drag.x=x;G.drag.y=y;}return true;}}return false;}
