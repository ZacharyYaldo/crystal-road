...
function update(dt,ui=true){if(G.mode==='battle')dt*=BATTLE_SPEED;else if(G.mode==='walk')dt*=MARCH_SPEED;G.t+=dt;if(G.shake>0)G.shake-=dt;
  ...
  if(!G.speedHinted&&!G.speedUsed&&G.campfireDone&&!(G.reforges>0)&&!G.cleared[0]&&!G.pending&&!G.banner&&G.tut==null&&G.mode==='walk'&&G.screen==='road'&&!G.sheet&&!G.title){G.speedHinted=true;saveGame();G.tut=TUT.length;TUT.push({t:'Fights can run faster',s:'Tap the speed button to march and fight at 2x.',font:'b',box:[W-36,SCENE_Y+4,32,18],once:true});}
