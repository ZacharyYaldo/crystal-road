...
function update(dt,ui=true){G.t+=dt;if(G.shake>0)G.shake-=dt;
  ...
  if(G.booted&&!G.endlessSeen&&ZONES[G.zone]&&ZONES[G.zone].endless&&G.tut==null&&G.mode==='walk'&&G.screen==='road'&&!G.sheet&&!G.title){G.endlessSeen=true;saveGame();G.tut=TUT.length;TUT.push({t:'The Endless Road',s:'No shard at the end. The road winds through every land, and the enemies keep growing.',box:[4,SCENE_Y+4,166,42],once:true},{t:'Champions and milestones',s:'A champion every 25 fights. A boss every 100. Fall, and you return to the last champion.',box:[4,SCENE_Y+4,166,42],once:true},{t:'Crystal gear',s:'Beat a milestone boss once and a piece of Crystal gear is yours. The strongest in the game.',icon:'sword',box:[4,SCENE_Y+4,166,42],once:true});}
