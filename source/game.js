...
function dealDamage(src,tgt,mult=1,opts={}){
  ...
  dmg=Math.max(1,R(dmg));if(!tgt.enemy&&tgt.talents&&tgt.talents.unbroken&&!tgt.unbrokenUsed&&tgt.hp-dmg<=0){dmg=tgt.hp-1;tgt.unbrokenUsed=true;float(tgt.x,GROUND-44,'Unbroken!',C.goldL,true);}if(!tgt.enemy)dmg=R(dmg*omenMult('hdmg'));if(!tgt.enemy&&G.hordeFight&&G.hordeFight.gate>0){const take=Math.min(G.hordeFight.gate,R(dmg*0.5));G.hordeFight.gate-=take;dmg-=take;if(G.hordeFight.gate<=0)float(W/2,GROUND-70,'The gate is breached!','#ff8a80',true);}tgt.hp-=dmg;if(!src.enemy&&src.status&&src.status.rage>0&&!src.dead)src.hp=Math.min(src.maxhp,src.hp+dmg*0.05);sfx(crit?'crit':'hit');if(!src.enemy&&G.fs){G.fs.dmg+=dmg;G.stats.hit=Math.max(G.stats.hit||0,dmg);}if(src.enemy&&G.fs){G.fs.taken=G.fs.taken||{};G.fs.taken[src.name]=(G.fs.taken[src.name]||0)+dmg;G.fs.takenT=G.fs.takenT||{};G.fs.takenT[src.range]=(G.fs.takenT[src.range]||0)+dmg;}
...
function doReforge(){const gain=reforgeGain();G.dust+=gain;G.reforges=(G.reforges||0)+1;const startLv=1+5*treeLv('b_start');
  ...
  G.run={start:now(),fights:0,bosses:0,gold:0,bestFloor:0,endless:0};G.endMilestone=0;G.oath=G.oathPick||null;G.oathPick=null;G.omens=[];G.omenOffer=null;G.omenMarks=[];if(G.active.length>partyMax())G.active=G.active.slice(0,partyMax());G.gold=0;G.ore=R(refund);G.prog=ZONES.map(()=>0);G.far=ZONES.map(()=>0);G.cleared=ZONES.map(()=>false);G.zone=0;G.enemies=[];G.projs=[];G.action=null;G.mode='walk';G.enc=4;G.wins=Math.max(G.wins,3);G.newZone=false;
...
function update(dt,ui=true){G.t+=dt;if(G.shake>0)G.shake-=dt;
  ...
  else if(G.mode==='battle'){if(G.hordeFight&&G.hordeFight.volleyAt&&RT>=G.hordeFight.volleyAt&&!G.banner){const hf=G.hordeFight;hf.volleyAt=0;const fr=mercVolley(hf.mercLv||1)/Math.max(1,hf.mercs);const src={enemy:false,status:{},x:0,dx:0};for(let i=0;i<hf.mercs;i++){const mx=[104,166,90,180][i]||104;for(const e of G.enemies)if(!e.dead)G.projs.push({x:mx,y:GROUND-58,tgt:e,src,t:0,dur:0.45,flip:false,merc:true,frac:fr});}float(W/2,GROUND-70,'Mercenary volley!','#9ad0ff',true);sfx('cast');}for(const e of G.enemies){if(!e.dead&&e.x>e.slot){e.x=Math.max(e.slot,e.x-90*dt);if(e.x<=e.slot&&e.anim==='walk'&&!e.fly)setAnim(e,'idle');}}}
...
function startHordeFight(){if(G.delve){toast('Finish the catacombs first');return;}{const hw=hordeWindow();if(hw.used>=2){toast('The horde needs time to gather - next in '+fmtT(hw.next));return;}G.castle.hordeLog.push(now());}const C2=G.castle,S=hordeStrength(),Z=ZONES[ZONES[G.zone].endless?endSeg(G.prog[G.zone]):G.zone];const gar=garrisonHeroes().filter(h=>!G.active.includes(h));
  const avgL=G.active.length?G.active.reduce((a,h)=>a+h.lvl,0)/G.active.length:1;const hordeLv=R(Math.max(Z.lv+S*0.6,avgL*0.85+(C2.streak||0)*2));G.hordeFight={wave:0,saved:{active:G.active.slice(),zone:G.zone,mode:G.mode,prog:G.prog.slice()},lvl:hordeLv,pool:Z.pool,boss:Z.boss};
...
function drawSheet(){const s=G.sheet;if(!s)return;tx.clearRect(0,0,tc.width,tc.height);dimRect(0,0,W,H,0.75);hit(0,0,W,H,()=>{if(s.kind!=='welcome')closeSheet();});
  ...
  else if(s.kind==='horde'){const C2=G.castle,p=C2.prep||(C2.prep={}),D=defense(),S=hordeStrength(),due=C2.hordeDue;text(16,y0+8,due?'The horde is at the gate':'The next horde','h',due?'#ff8a80':C.goldL);text(16,y0+22,'Horde · '+hordeTier()+'   Your defense '+fmtNum(D.total)+(D.total>=S?' - holds':' - falls'),'xs',D.total>=S?'#96e6aa':'#ff9696');
