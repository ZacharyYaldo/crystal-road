...
function dealDamage(src,tgt,mult=1,opts={}){
  ...
  dmg=Math.max(1,R(Math.max(dmg,atk*mult*0.02)));if(!tgt.enemy&&tgt.talents&&tgt.talents.unbroken&&!tgt.unbrokenUsed&&tgt.hp-dmg<=0){dmg=tgt.hp-1;tgt.unbrokenUsed=true;float(tgt.x,GROUND-44,'Unbroken!',C.goldL,true);}if(!tgt.enemy)dmg=R(dmg*omenMult('hdmg'));if(!tgt.enemy&&G.hordeFight&&G.hordeFight.gate>0){const take=Math.min(G.hordeFight.gate,R(dmg*0.5));G.hordeFight.gate-=take;dmg-=take;if(G.hordeFight.gate<=0)float(W/2,GROUND-70,'The gate is breached!','#ff8a80',true);}tgt.hp-=dmg;if(!src.enemy&&src.status&&src.status.rage>0&&!src.dead)src.hp=Math.min(src.maxhp,src.hp+dmg*0.05);sfx(crit?'crit':'hit');if(!src.enemy&&G.fs){G.fs.dmg+=dmg;G.stats.hit=Math.max(G.stats.hit||0,dmg);}if(src.enemy&&G.fs){G.fs.taken=G.fs.taken||{};G.fs.taken[src.name]=(G.fs.taken[src.name]||0)+dmg;G.fs.takenT=G.fs.takenT||{};G.fs.takenT[src.range]=(G.fs.takenT[src.range]||0)+dmg;}
...
function startHordeFight(){if(G.delve){toast('Finish the catacombs first');return;}{const hw=hordeWindow();if(hw.used>=2){toast('The horde needs time to gather - next in '+fmtT(hw.next));return;}G.castle.hordeLog.push(now());}const C2=G.castle,S=hordeStrength(),Z=ZONES[ZONES[G.zone].endless?endSeg(G.prog[G.zone]):G.zone];const gar=garrisonHeroes().filter(h=>!G.active.includes(h));
  const avgL=G.active.length?G.active.reduce((a,h)=>a+h.lvl,0)/G.active.length:1;const hordeLv=R(Math.max(3,avgL*0.85+(C2.streak||0)*2+castleLv()*0.1));G.hordeFight={wave:0,saved:{active:G.active.slice(),zone:G.zone,mode:G.mode,prog:G.prog.slice()},lvl:hordeLv,pool:Z.pool,boss:Z.boss};
