...
function update(dt,ui=true){G.t+=dt;if(G.shake>0)G.shake-=dt;
  ...
  else if(G.mode==='battle'){if(G.hordeFight&&G.hordeFight.volleyAt&&RT>=G.hordeFight.volleyAt&&!G.banner){const hf=G.hordeFight;hf.volleyAt=0;const fr=Math.min(0.5,(0.08+0.02*castleLv())*(1+0.25*((hf.mercLv||1)-1)))/Math.max(1,hf.mercs);const src={enemy:false,status:{},x:0,dx:0};for(let i=0;i<hf.mercs;i++){const mx=[104,166,90,180][i]||104;for(const e of G.enemies)if(!e.dead)G.projs.push({x:mx,y:GROUND-58,tgt:e,src,t:0,dur:0.45,flip:false,merc:true,frac:fr});}float(W/2,GROUND-70,'Mercenary volley!','#9ad0ff',true);sfx('cast');}for(const e of G.enemies){if(!e.dead&&e.x>e.slot){e.x=Math.max(e.slot,e.x-90*dt);if(e.x<=e.slot&&e.anim==='walk'&&!e.fly)setAnim(e,'idle');}}}
...
function defense(){const p=G.castle.prep||{};const w=(G.castle.b.walls*5+(built('watchtower')?15:0))*(p.walls?1.25:1);const g=garrisonHeroes().reduce((s,h)=>s+3+h.lvl*0.5,0)+(mercLv()?(8+castleLv()*2)*(1+0.5*(mercLv()-1)):0);return{walls:R(w),garrison:R(g),total:R(w+g)};}
...
function drawSheet(){const s=G.sheet;if(!s)return;tx.clearRect(0,0,tc.width,tc.height);dimRect(0,0,W,H,0.75);hit(0,0,W,H,()=>{if(s.kind!=='welcome')closeSheet();});
  ...
  if(s.kind==='swap'){text(16,y0+8,'Party',"h",C.goldL);text(16,y0+20,'Tap a marcher, then a hero at camp to trade places.','xs',C.muted);text(16,y0+29,'Two marchers to reorder. Left slot is the front.','xs',C.muted);
    ...
    let y=y0+36;const rows=[['Shore up the walls','+25% wall defense, stronger gate',p.walls?null:prepCost('walls'),()=>{p.walls=true;}],[mercLv()?'Drill the mercenaries  ·  Lv '+mercLv():'Hire mercenaries',mercLv()?'+1 soldier, stronger volley, +'+R((8+castleLv()*2)*0.5)+' defense':'+'+(8+castleLv()*2)+' defense, a volley each wave',mercLv()>=4?null:R(prepCost('merc')*Math.pow(1.6,mercLv())),()=>{p.merc=true;p.mercLv=mercLv()+1;}]];
