...
function update(dt,ui=true){G.t+=dt;if(G.shake>0)G.shake-=dt;
  ...
  if(G.mode==='enter'){let all2=true;for(const e of G.enemies){if(e.x>e.slot){e.x=Math.max(e.slot,e.x-70*dt);all2=false;}else if(e.anim==='walk'&&!e.fly)setAnim(e,'idle');}if(all2){G.mode='battle';if(G.hordeFight&&G.hordeFight.volley){G.hordeFight.volley=false;const fr=Math.min(0.4,0.08+0.02*castleLv());for(const e of G.enemies)if(!e.dead){const d=R(e.maxhp*fr);e.hp=Math.max(1,e.hp-d);float(e.x+e.dx,GROUND+(e.yoff||0)-40,fmtNum(d),'#9ad0ff');}float(W/2,GROUND-70,'Mercenary volley!','#9ad0ff',true);sfx('crit');}G.enemies.forEach(e=>setAnim(e,'idle'));}}
...
function drawScene(){const Z=ZONES[G.zone];const key=G.hordeFight?'door':G.delve?(G.delve.floor>=12?'door':'cave'):((G.bossFight&&Z.bossScene)?Z.bossScene:zoneScene(Z,G.prog[G.zone]));const layers=SCENES[key];
  ...
  if(G.hordeFight){ctx.globalAlpha=0.8;frame(4,SCENE_Y+4,150,36,'rgba(44,22,26,0.8)');ctx.globalAlpha=1;text(9,SCENE_Y+7,'Horde at the gate','sb','#ff9696');text(9,SCENE_Y+17,'Wave '+G.hordeFight.wave+' of 3 · '+hordeTier(),'xs','#c88080');const hf=G.hordeFight;if(hf.mercs>0){const A=ATLAS.soldier;for(let i=0;i<hf.mercs;i++){const mx=6+i*14,fr=Math.floor(RT*4+i)%A.anims.idle.n;ctx.save();ctx.globalAlpha=0.95;drawSprite({uid:'soldier'},mx,GROUND+6-i*6,fr,'idle',false,1.2);ctx.restore();}}if(hf.gateMax>0){const gx=176,gy=SCENE_Y+34;px(gx-2,gy-4,90,18,'rgba(0,0,0,0.55)');bar(gx,gy,86,6,hf.gate/hf.gateMax,hf.gate>0?'#9ad0ff':'#553');text(gx+43,gy+7,hf.gate>0?'Gate '+Math.round(100*hf.gate/hf.gateMax)+'%':'Gate breached','xs',hf.gate>0?'#cfe8ff':'#ff8a80','center');}}else if(G.delve){drawDelveOverlay();}else{
  // zone plate
...
function serialize(){return JSON.stringify({v:1,t:now(),zone:G.zone,prog:G.prog,cleared:G.cleared,gold:G.gold,ore:G.ore,dust:G.dust,wins:G.wins,campfireDone:G.campfireDone,tree:G.tree,autoCast:G.autoCast,music:G.music,sfx:G.sfx!==false,autoUntil:G.autoUntil,reforges:G.reforges||0,surge:G.surge||0,chestAt:G.chestAt||0,warpAt:G.warpAt||0,far:G.far||[],run:G.run,stats:G.stats,simT:G.simT||0,zoneT:G.zoneT||[],treeTab:G.treeTab,delveKeys:G.delveKeys,keyAt:G.keyAt,delveBest:G.delveBest||0,speedUntil:G.speedUntil||0,autoSalvage:G.autoSalvage||0,musicVol:G.musicVol,sfxVol:G.sfxVol,tutDone:!!G.tutDone,vaelSeen:!!G.vaelSeen,treeSeen:!!G.treeSeen,keysSeen:!!G.keysSeen,dustFrac:G.dustFrac||0,heroesSeen:G.heroesSeen||[],cpHinted:!!G.cpHinted,endlessSeen:!!G.endlessSeen,endMilestone:G.endMilestone||0,oath:G.oath||null,omens:G.omens||[],omenOffer:G.omenOffer||null,omenMarks:G.omenMarks||[],renown:G.renown||0,boostSeen:!!G.boostSeen,vaelNew:!!G.vaelNew,itemSeq,
  roster:G.roster.map(h=>({id:h.id,lvl:h.lvl,xp:h.xp,hp:h.hp,abLvl:h.abLvl,eq:h.eq,tier:h.tier||0,tierMax:h.tierMax||0,talents:h.talents||{}})),active:G.active.map(h=>h.id),pack:G.pack,
...
function loadGame(){const raw=storeGet();if(!raw)return false;let d;try{d=JSON.parse(raw);}catch(e){return false;}if(!d||d.v!==1)return false;
  ...
  G.roster=[];for(const r of d.roster){const def=HEROES.find(x=>x.id===r.id);if(!def)continue;const h=mkHero(def);h.lvl=r.lvl;h.xp=r.xp;h.abLvl=r.abLvl||1;h.eq=r.eq||{};h.tier=r.tier||0;h.tierMax=Math.max(r.tierMax||0,h.tier);h.uid=heroUid(h);h.talents=r.talents||{};refreshStats(h);h.hp=Math.min(h.maxhp,r.hp>0?r.hp:h.maxhp);G.roster.push(h);}
...
function drawSheet(){const s=G.sheet;if(!s)return;tx.clearRect(0,0,tc.width,tc.height);dimRect(0,0,W,H,0.75);hit(0,0,W,H,()=>{if(s.kind!=='welcome')closeSheet();});
  ...
  if(s.kind==='swap'){text(16,y0+8,'Party',"h",C.goldL);text(16,y0+20,'Tap a marcher, then a hero at camp to trade places.','xs',C.muted);text(16,y0+29,'Two marchers to reorder. Left slot is the front.','xs',C.muted);
    ...
    let y=y0+36;const rows=[['Shore up the walls','+25% wall defense; a stronger gate if you fight',p.walls?null:prepCost('walls'),()=>{p.walls=true;}],['Hire mercenaries','+'+(8+castleLv()*2)+' defense; two soldiers volley each wave if you fight',p.merc?null:prepCost('merc'),()=>{p.merc=true;}]];
