...
function doReforge(){const gain=reforgeGain();G.dust+=gain;G.reforges=(G.reforges||0)+1;const startLv=1+5*treeLv('b_start');
  ...
  G.run={start:now(),fights:0,bosses:0,gold:0,bestFloor:0,endless:0};G.endMilestone=0;G.oath=G.oathPick||null;G.oathPick=null;G.castle.currentHorde=null;G.omens=[];G.omenOffer=null;G.omenMarks=[];if(G.active.length>partyMax())G.active=G.active.slice(0,partyMax());G.gold=0;G.ore=R(refund);G.prog=ZONES.map(()=>0);G.far=ZONES.map(()=>0);G.cleared=ZONES.map(()=>false);G.zone=0;G.enemies=[];G.projs=[];G.action=null;G.mode='walk';G.enc=4;G.wins=Math.max(G.wins,3);G.newZone=false;
...
function drawMap(){drawPanelScreen();ctx.strokeStyle=C.goldL;ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(16,33,9,0,Math.PI*2);ctx.stroke();icon('back',10,27,4);text(32,26,'The Crystal Road','title',C.goldL);hit(0,16,110,34,()=>{G.screen='road';});const rb=BIG?26:18;const ok0=canReforge();button(170,22,92,rb,ok0);icon('crystal',174,22+rb/2-6,2);text(220,22+rb/2-4,ok0?'Shatter +'+fmtNum(reforgeGain()):'Beat the Warlord','xsb',ok0?C.goldL:C.dimt,'center');if(ok0)hit(170,22,92,rb,()=>openSheet('shatter'));
  ...
  const ey=52+ZONES.length*72;text(W/2,ey,'Shatter keeps your heroes, talents, gear, Vael,','xs',C.dimt,'center');text(W/2,ey+10,'the tree and blessings, and pays out crystal dust.','xs',C.dimt,'center');endScroll('map',ey+30);}
...
function drawSheet(){const s=G.sheet;if(!s)return;tx.clearRect(0,0,tc.width,tc.height);dimRect(0,0,W,H,0.75);hit(0,0,W,H,()=>{if(s.kind!=='welcome')closeSheet();});
  ...
  if(s.kind==='swap'){text(16,y0+8,'Party',"h",C.goldL);text(16,y0+20,'Tap a marcher, then a hero at camp to trade places.','xs',C.muted);text(16,y0+29,'Two marchers to reorder. Left slot is the front.','xs',C.muted);
    ...
    text(16,ty,'Shatter the Crystal for +'+reforgeGain()+' dust. Its shards scatter along the road','xs','#bbdefb');text(16,ty+9,'again, and you walk it from Greenhollow. You keep your heroes, talents,','xs','#bbdefb');text(16,ty+18,'gear, Vael, the tree and blessings. Gear upgrades pay back half their ore.','xs','#bbdefb');
    button(16,ty+33,110,22,true);text(71,ty+40,'Begin run '+((G.reforges||0)+2),'sb',C.goldL,'center');hit(16,ty+33,110,22,doReforge);button(140,ty+33,110,22,false);text(195,ty+40,'Not yet','xs',C.cream,'center');hit(140,ty+33,110,22,closeSheet);}
