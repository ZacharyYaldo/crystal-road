...
function update(dt,ui=true){G.t+=dt;if(G.shake>0)G.shake-=dt;
  ...
  else if(G.mode==='defeat'){G.timer-=dt;if(G.timer<=0){G.enemies=[];G.projs=[];G.action=null;for(const h of G.active){h.dead=false;h.hp=h.maxhp;h.dx=0;h.status={};setAnim(h,'walk');}layout();if(!G.cleared[G.zone]){const Z9=ZONES[G.zone];if(Z9.endless){G.prog[G.zone]=Math.max(0,Math.floor((G.prog[G.zone]-1)/25)*25);if((G.omens||[]).length){G.omens=[];G.omenOffer=null;toast('The omens scatter');}}else if(G.bossFight){const cs=Z9.fights/3;G.prog[G.zone]=Math.max(Math.floor((Z9.fights-1)/cs)*cs-cs,Z9.fights-1-bossRetry(G.zone));}else{const cs=Z9.fights/3;G.prog[G.zone]=Math.floor(G.prog[G.zone]/cs)*cs;}}G.mode='walk';G.enc=3;noteFight(false,!!G.bossFight);}}
