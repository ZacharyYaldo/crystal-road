...
function mercVolley(lv){return lv?Math.min(0.6,(0.08+0.02*castleLv())*(1+0.15*(lv-1))):0;}
...
function drawVael(){drawPanelScreen();castleTick();const C2=G.castle,D=defense(),S=hordeStrength();
  ...
  const safe=D.total>=S;const pw=80,px1=98,px2=184,py=18,ph=35,sy=py+20,sh=12;const plate=(x,fill,line)=>{px(x,py,pw,ph,C.out);px(x+1,py+1,pw-2,ph-2,line);px(x+2,py+2,pw-4,ph-4,fill);};plate(px1,safe?'#14321e':'#3c1e1e',safe?'#5ab46e':'#c85050');text(px1+pw/2,py+3,'Defense '+fmtNum(D.total),'sb',safe?'#96e6aa':'#ff9696','center');{const wa=fmtNum(D.walls),ga=fmtNum(D.garrison);const cands=['walls '+wa+' · garrison '+ga,'walls '+wa+' · gar '+ga,'W '+wa+' · G '+ga];const t2=cands.find(t=>textW(t,'xs')<=pw-6)||cands[2];text(px1+pw/2,py+12,t2,'xs',safe?'#78aa82':'#c88080','center');}button(px1+4,sy,pw-8,sh,false);text(px1+pw/2,sy+sh/2-4,'Garrison','xs',C.cream,'center');hit(px1,py,pw,ph,()=>openSheet('garrison'));
