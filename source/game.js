...
const BATTLE_SPEED=1.15; // combat runs this much faster than real time (animations, gauges, actions); walking, timers and travel are unaffectedfunction tapRateMult(n){return n<=1?1:n===2?0.7:n===3?0.45:0.25;}
...
function update(dt,ui=true){if(G.mode==='battle')dt*=BATTLE_SPEED;G.t+=dt;if(G.shake>0)G.shake-=dt;
  ...
  if(G.gearHint&&G.tut==null&&G.mode==='walk'&&G.screen==='road'&&!G.sheet){G.gearHint=false;G.tut=TUT.length;TUT.push({t:'Gear up before pressing on',s:'Open Gear and equip a weapon, cape and charm on every hero. Salvage spares for ore and upgrade what you wear.',s2:'Then spend gold in the Tree for permanent HP and attack ranks'+(G.wins<3?' - it opens after three wins.':'.'),tabs:['gear','tree'],box:[0,TABY,W,42],once:true});}
