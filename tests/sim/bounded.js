...
//   2. Shatter 10 is not reached within 96 h           6. no boss softlocks (uncleared boss with >= 3 attempts, or a fail streak >= 8) and a usable ore economy
//   3. gear reaches its level cap (median item level     (at least one ascension per run and ore spent > 0 in the last 24 h)
//      at 96 h within 2 levels of the rank's cap)      7. engaged is 15-30% ahead of boosted idle on the hour Endless is entered (needs both profiles in the batch)
...
for(const p of P){const rs=R[p];if(!rs.length){console.log('\n== '+p+': no runs');fails++;continue;}
  ...
  const capOf=r=>{const g=r.roster.flatMap(h=>Object.values(h.gear).filter(Boolean));const capped=g.filter(it=>it.lvl>=20*(it.rank+1)-2).length;return g.length?capped/g.length:0;};const capShare=med(rs.map(capOf));say(capShare>=0.5,'3. gear at its cap: median share of equipped items within 2 levels of the cap '+f(100*capShare,0)+'% (item level '+f(med(rs.map(r=>at(r,96).itemLvl)),0)+', rank '+f(med(rs.map(r=>at(r,96).itemRank)),2)+')');
