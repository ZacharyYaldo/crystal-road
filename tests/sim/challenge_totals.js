...
const rd=f=>JSON.parse(fs.readFileSync(path.join(__dirname,'bench',f),'utf8'));
(async()=>{const S=H.load({seed:1,startMs:1700000000000});await S.ready;const C=S.CHALLENGE,Z=S.ZONES;
  ...
  const tot={gold:0,ore:0,dust:0};const line=(name,kind,key,target,rt,zi,prov)=>{const parts=[];for(let t=0;t<4;t++){const b=S.challengeBundle(kind,t,zi);parts.push(C.tiers[t]+' '+fmt(target*C.tierPct[t])+': '+fmt(b.gold*rt.gold)+' gold'+(b.ore?' + '+fmt(b.ore*rt.ore)+' ore':'')+(b.dust?' + '+b.dust+' dust':''));}
    const c=S.challengeBundle(kind,3,zi);if(!prov){tot.gold+=c.gold*rt.gold;tot.ore+=c.ore*rt.ore;tot.dust+=c.dust;}
    console.log((name+(prov?' (provisional, no reward)':'')).padEnd(34)+' Crystal '+fmt(target).padStart(8)+'  rate '+fmt(rt.gold)+' gold/min, '+fmt(rt.ore)+' ore/min\n    '+parts.join('\n    ')+'\n    Overdrive '+fmt(target*C.overdrive)+': badge');};
  console.log('== zone brackets (target = P90 of the production Challenge on 40 states; bundles cumulative)');
  keys.forEach((k,zi)=>{const R=zones.filter(r=>r.key===k);line(Z[zi].name,'zone',String(zi),C.targets.zone[zi],rate(R),zi,false);});
  console.log('\n== Shatter brackets (target = P90 on 40 peak-of-run states; bracket n uses the states saved before Shatter n+1)');
  for(let n=3;n<=10;n++){const t=C.targets.shatter[String(n)];if(t==null)continue;let R=shat.filter(r=>r.key==='shatter'+(n+1));if(!R.length)R=extra.filter(r=>r.bracket==='shatter:'+n);const rt=R.length?rate(R):rate(shat.filter(r=>r.key==='shatter7'));line('Shatters '+n,'shatter',String(n),t,rt,null,!!C.provisional[String(n)]);}
