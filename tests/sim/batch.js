...
function next(){if(idx>=jobs.length)return;const j=jobs[idx++];const a=[path.join(__dirname,'bot.js'),'--hours',String(HOURS),'--seed',String(j.s),'--profile',j.p,'--quiet','--json',j.file,'--snapshots',path.join(__dirname,args.tag?'snapshots_'+args.tag:'snapshots')];if(SHATTERS)a.push('--shatters',String(SHATTERS));if(args.replace)a.push('--replace',String(args.replace));if(args.replaceFile)a.push('--replaceFile',String(args.replaceFile));if(args.untilZone)a.push('--untilZone',String(args.untilZone));if(args.noRetreat)a.push('--noRetreat');if(args.retreatAfter!=null)a.push('--retreatAfter',String(args.retreatAfter));if(args.trainUntil)a.push('--trainUntil',String(args.trainUntil));if(args.retreatOn)a.push('--retreatOn',String(args.retreatOn));if(args.noAutoTrain)a.push('--noAutoTrain');if(args.stallRule)a.push('--stallRule',String(args.stallRule));if(args.stallHours!=null)a.push('--stallHours',String(args.stallHours));const c=spawn(process.execPath,a,{stdio:['ignore','ignore','inherit']});c.on('exit',()=>{done++;process.stdout.write('\r'+done+'/'+jobs.length+' done ('+((Date.now()-t0)/60000).toFixed(1)+' min)   ');if(done===jobs.length)report();else next();});}
