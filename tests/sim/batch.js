...
function next(){if(idx>=jobs.length)return;const j=jobs[idx++];const a=[path.join(__dirname,'bot.js'),'--hours',String(HOURS),'--seed',String(j.s),'--profile',j.p,'--quiet','--json',j.file,'--snapshots',path.join(__dirname,args.tag?'snapshots_'+args.tag:'snapshots')];if(SHATTERS)a.push('--shatters',String(SHATTERS));if(args.replace)a.push('--replace',String(args.replace));if(args.replaceFile)a.push('--replaceFile',String(args.replaceFile));if(args.speed)a.push('--speed',String(args.speed));if(args.dt)a.push('--dt',String(args.dt));if(args.allowCoarseDt)a.push('--allowCoarseDt');if(args.untilZone)a.push('--untilZone',String(args.untilZone));if(args.noRetreat)a.push('--noRetreat');if(args.retreatAfter!=null)a.push('--retreatAfter',String(args.retreatAfter));if(args.trainUntil)a.push('--trainUntil',String(args.trainUntil));if(args.retreatOn)a.push('--retreatOn',String(args.retreatOn));if(args.noAutoTrain)a.push('--noAutoTrain');if(args.stallRule)a.push('--stallRule',String(args.stallRule));if(args.stallHours!=null)a.push('--stallHours',String(args.stallHours));if(args.schedule)a.push('--schedule',String(args.schedule));for(const k of ['drills','double','boosts','clericBack'])if(args[k])a.push('--'+k);const c=spawn(process.execPath,a,{stdio:['ignore','ignore','inherit']});c.on('exit',()=>{done++;process.stdout.write('\r'+done+'/'+jobs.length+' done ('+((Date.now()-t0)/60000).toFixed(1)+' min)   ');if(done===jobs.length)report();else next();});}
...
function report(){console.log('\n');const rows=[];const all={};for(const j of jobs){try{(all[j.p]=all[j.p]||[]).push(JSON.parse(fs.readFileSync(j.file,'utf8')));}catch(e){}}
  ...
  const outFile=args.out||path.join(dir,'summary.json');fs.writeFileSync(outFile,JSON.stringify({config:{SEEDS,HOURS,PROFILES,SHATTERS},rows},null,1));
  {const crypto=require('crypto');const files={},provs={};let cfg=null;for(const j of jobs){if(!fs.existsSync(j.file))continue;const name=path.basename(j.file);files[name]=crypto.createHash('sha256').update(fs.readFileSync(j.file)).digest('hex');try{const c=JSON.parse(fs.readFileSync(j.file,'utf8')).config||{};cfg=cfg||c;const k=(c.commit||'?')+'|'+(c.gameHash||'?')+'|'+(c.harnessHash||'?');provs[k]=(provs[k]||0)+1;}catch(e){}}
   const pk=Object.keys(provs);const [commit,gameHash,harnessHash]=(pk.length===1?pk[0]:'mixed|mixed|mixed').split('|');
   const manifest={tag:args.tag||null,createdAt:new Date().toISOString(),commit,gameHash,harnessHash,provenanceSets:provs,config:{seedStart:SEED0,seeds:SEEDS,hours:HOURS,profiles:PROFILES,shatters:SHATTERS,dt:cfg&&cfg.dt,speed:cfg&&cfg.speed,frameCap:cfg&&cfg.frameCap,schedule:args.schedule||null,model:{drills:!!args.drills,double:!!args.double,boosts:!!args.boosts,clericBack:!!args.clericBack}},files};
   fs.writeFileSync(path.join(dir,'manifest.json'),JSON.stringify(manifest,null,1));const mdir=path.join(__dirname,'manifests');fs.mkdirSync(mdir,{recursive:true});if(args.tag)fs.writeFileSync(path.join(mdir,args.tag+'.json'),JSON.stringify(manifest,null,1));console.log('manifest written ('+Object.keys(files).length+' files, provenance '+pk.join(' ; ')+')');}console.log('\nwritten '+outFile);}
