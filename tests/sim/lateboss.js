...
(async()=>{
  ...
  for(const B of BOSSES){
    ...
    for(const p of P){
      let snaps=fs.readdirSync(SNAP).filter(x=>x.startsWith(B.prefix+'_'+p+'_')||(B.alt&&x.startsWith(B.alt+'_'+p+'_'))).map(x=>JSON.parse(fs.readFileSync(path.join(SNAP,x),'utf8'))).filter(s=>s.zoneName===B.zone);
