// Late-boss damage-per-action comparison (observation only): Warlord (locked reference), Sand Tyrant, Hunter King, Grave Knight.
// Combines existing arrival snapshots (hero HP/DEF at natural arrival) with existing batch telemetry (attempt logs).
// Usage: node tests/sim/lateboss.js [--snapDir snapshots] [--dirs batch_out_p15,batch_out_p14x] [--profiles idle,light,casual,engaged,stress]
'use strict';
...
const BOSSES=[{zone:'Ironvein Caverns',id:'warlord',prefix:'ironvein',charge:'melee x2.5'},{zone:'Emberwaste',id:'sandtyrant',prefix:'emberwaste',charge:'melee x2.5'},{zone:'Amberfall Woods',id:'hunterking',prefix:'amberfall',charge:'volley x2.2'},{zone:'Ashen Keep',id:'graveknight',prefix:'ashen',charge:'melee x2.5'}];
...
(async()=>{
  ...
  const runs={};for(const p of P){for(const d of DIRS){const dir=path.join(__dirname,d);if(!fs.existsSync(dir))continue;const rs=fs.readdirSync(dir).filter(x=>x.startsWith(p+'_')&&x.endsWith('.json')).map(x=>JSON.parse(fs.readFileSync(path.join(dir,x),'utf8'))).filter(r=>r.bossFailRates);if(rs.length){runs[p]={dir:d,rs};break;}}}
  for(const B of BOSSES){
    ...
    for(const p of P){
      const snaps=fs.readdirSync(SNAP).filter(x=>x.startsWith(B.prefix+'_'+p+'_')).map(x=>JSON.parse(fs.readFileSync(path.join(SNAP,x),'utf8'))).filter(s=>s.zoneName===B.zone);
      ...
      const ordF=bossAtk!=null&&mDF!=null?Math.max(1,bossAtk-mDF*0.5*1.15):null,ordB=bossAtk!=null&&mDB!=null?Math.max(1,bossAtk-mDB*0.5):null;const cm=B.charge.startsWith('volley')?2.2:2.5;
