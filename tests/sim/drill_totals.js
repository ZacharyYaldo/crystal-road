...
(async()=>{const S=H.load({seed:1,startMs:1700000000000});await S.ready;const D=S.DRILL,Z=S.ZONES;
  ...
  console.log('\n== zone brackets (target = P90 of the production drill at the first boss attempt of the zone, best roster hero, two taps a second; bundles cumulative)');
  const ZK=['greenhollow','stillwater','thornwood','ironvein','emberwaste','amberfall','ashen_approach','ashen_keep','the_foundry'];for(let zi=0;zi<Z.length-1;zi++){const R=all.filter(r=>r.key===ZK[zi]&&r.bracket==='zone:'+zi);line(Z[zi].name,'zone',String(zi),D.targets.zone[zi],R,zi,false);} /* zone brackets: the state at the zone's first boss attempt, the same rule the targets use */
