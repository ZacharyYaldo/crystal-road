...
(async()=>{const S=H.load({seed:1,startMs:1700000000000});await S.ready;const D=S.DRILL,Z=S.ZONES;const tot={gold:0,ore:0,dust:0};
  const line=(name,kind,key,zi)=>{const br=kind==='zone'?{kind,zi,key}:{kind,key,n:Number(key)};const t=S.drillTarget(br);const sm=summaries[kind+':'+key];const prov=S.drillProvisional(br);const rt=sm?{gold:sm.goldMin,ore:sm.oreMin}:{gold:0,ore:0};
    if(t==null){console.log(name.padEnd(20)+' not calibrated'+(sm?' (bench has '+sm.n+' states)':''));return;}
    ...
    console.log((name+(prov?' (provisional, no reward)':'')).padEnd(34)+' sample '+(sm?sm.n+' runs ('+Object.entries(sm.profiles||{}).map(([p,v])=>p+' '+v.n+': median '+fmt(v.median)).join(', ')+')':'none')+'; defense '+(sm?sm.def:'-')+'; rate '+fmt(rt.gold)+' gold/min, '+fmt(rt.ore)+' ore/min\n    '+parts.join('\n    ')+'\n    Overdrive '+fmt(t[3]*D.overdrive)+': badge');};
  console.log('\n== zone brackets (thresholds = P25 / P50 / P75 / P90 of the production drill at the first boss attempt of the zone, best roster hero, two taps a second; bundles cumulative)');
