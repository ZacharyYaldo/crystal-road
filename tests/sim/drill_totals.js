...
(async()=>{const S=H.load({seed:1,startMs:1700000000000});await S.ready;const D=S.DRILL,Z=S.ZONES;const tot={gold:0,ore:0,dust:0};
  const line=(name,kind,key,zi)=>{const br=kind==='zone'?{kind,zi,key}:{kind,key,wave:Number(key)};const t=S.drillTarget(br);const sm=summaries[kind+':'+key];const prov=S.drillProvisional(br)||!S.drillRewardsOn(br);const rt=sm?{gold:sm.goldMin,ore:sm.oreMin}:{gold:0,ore:0};
