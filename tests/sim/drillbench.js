// Drill bench: what a hero deals in the 30-second Training Dummy drill at each stage of the game, measured on saved game states
// (the bot's snapshots: one at every zone's first boss attempt, one just before every Shatter). Each save boots a fresh production game,
// every roster hero drills twice (Auto-Cast on with 2 taps/s, and Auto-Cast on without taps), and the best hero's total is the stage value.
// Usage: node tests/sim/drillbench.js --dir snapshots_p45x2 [--profiles idleboost,engaged] [--seeds 81-85] [--taps 2]
'use strict';
...
const ZONE_ORDER=['greenhollow','stillwater','thornwood','ironvein','emberwaste','amberfall','ashen_approach','ashen_keep','the_foundry','the_endless','shatter'];
...
async function bench(file){const rec=JSON.parse(fs.readFileSync(path.join(DIR,file),'utf8'));const save=rec.save;const t=JSON.parse(save).t||1700000000000;
  ...
  let best={tap:0,idle:0,hero:null};if(MODE==='challenge'){S.CHALLENGE.claims=false;if(!S.startChallenge())throw new Error('no party');while(G.drill.running){S.clock.frame();S.setRT(S.getRT()+DT);S.update(DT,true);S.drillTick(DT);}G.sheet=null;const st=G.drill.stats;const br=G.drill.bracket,zi=br.kind==='zone'?br.zi:S.ZONES.length-1,rate=S.incomeRate(zi,br.kind==='zone'?S.ZONES[zi].fights-1:0);best={tap:st.total,idle:st.hero,hero:br.kind+' '+br.key+' def '+G.drill.g.enemies[0].def+' party '+G.active.map(h=>h.cls[0]+h.lvl).join('/'),goldMin:rate.gold,oreMin:rate.ore,bracket:br.kind+':'+br.key};}else{for(const h of G.roster){const a=drill(h,TAPS),b=drill(h,0);if(a>best.tap){best.tap=a;best.hero=h.name+' '+S.CLASSES[h.cls].name+' Lv'+h.lvl;}if(b>best.idle)best.idle=b;}}
  const m=file.match(/^(.*)_([a-z]+)_(\d+)\.json$/);const key=m[1].startsWith('shatter')?m[1]:m[1];return{key,profile:m[2],seed:Number(m[3]),lv:rec.lv||null,hours:rec.hours,zone:rec.zoneName||rec.zone,tap:best.tap,idle:best.idle,hero:best.hero,reforges:G.reforges||0,goldMin:best.goldMin||null,oreMin:best.oreMin||null,bracket:best.bracket||null};}
