...
function main(){
  ...
  while(simSec<target){
    ...
    if(G.mode==='defeat'&&G.timer>2.5){if(bossWas){const b=M.boss[bossZoneName];if(args.verbose){const e=G.enemies.find(x=>x.boss)||G.enemies[0];ev('bossLOSS',bossZoneName+' after '+(simSec-bossStart).toFixed(0)+'s bossHP='+(e&&bossHpMax?Math.round(100*e.hp/bossHpMax):'?')+'% enemies='+G.enemies.filter(x=>!x.dead).map(x=>x.name).join('/'));}b.fails++;b.streak++;b.maxStreak=Math.max(b.maxStreak,b.streak);const e=G.enemies.find(x=>x.boss)||G.enemies[0];b.losses.push({dur:+(simSec-bossStart).toFixed(1),hpLeft:e&&bossHpMax?+(e.hp/bossHpMax).toFixed(2):null,wipe:G.active.every(h=>h.dead)});b.combatStall=(b.combatStall||0)+(simSec-bossStart);b.lossAt=simSec;bossWas=false;}zoneDefeats++;}
