...
function main(){
  ...
  while(simSec<target){
    ...
    const cl=G.cleared.filter(Boolean).length;if(cl>lastCleared){lastCleared=cl;const zi=G.cleared.lastIndexOf(true);M.zones[Z[zi].name]=M.zones[Z[zi].name]||+M.hours.toFixed(2);lastProgressH=M.hours;{const rk=String(G.reforges||0);M.runClears=M.runClears||{};M.runClears[rk]=M.runClears[rk]||{};M.runClears[rk][Z[zi].name]=M.runClears[rk][Z[zi].name]||+M.hours.toFixed(2);}const zr=M.zoneRec[Z[zi].name];if(zr&&!zr.clearH){zr.clearH=+M.hours.toFixed(2);zr.lvAtClear=avgLv();zr.hours=+(zr.clearH-zr.enterH).toFixed(2);const b=M.boss[Z[zi].name];if(b){b.streak=0;b.stallH=+((simSec-b.firstTry)/3600).toFixed(2);b.lvClear=avgLv();b.attemptsToClear=b.attempts;if(b.attemptLog&&b.attemptLog.length){const L=b.attemptLog[b.attemptLog.length-1],fsx=G.fs||{};L.win=true;Object.assign(L,{dur:+(simSec-b.firstTry-(b.retryStall||0)-(b.combatStall||0)).toFixed(1),survivors:G.active.filter(h=>!h.dead).length,partyHp:+(G.active.reduce((q,h)=>q+Math.max(0,h.hp),0)/Math.max(1,G.active.reduce((q,h)=>q+h.maxhp,0))).toFixed(2),telegraphs:fsx.chargeTelegraphs||0,chargeHits:fsx.chargeHits||0,chargeKills:fsx.chargeKills||0,parried:fsx.chargeParried||0,interrupted:fsx.chargeInterrupted||0,bossHits:fsx.bossHits||0,raised:fsx.raised||0,run:G.reforges||0});}b.fightsBetween=G.wins-b.winsAtFirst-b.attempts+1;b.lvGain=b.lvClear-b.lvFirst;b.upsBetween=M.gearUps-b.upsAtFirst;b.combatStallH=+((b.combatStall||0)/3600).toFixed(2);b.retryStallH=+((b.retryStall||0)/3600).toFixed(2);}}ev('cleared',Z[zi].name+' (party Lv '+avgLv()+')');}
    if(G.zone!==lastZone){lastZone=G.zone;zoneEnter=simSec;zoneDefeats=0;const zn=Z[G.zone].name;if(!M.zoneRec[zn])M.zoneRec[zn]={enterH:+M.hours.toFixed(2),lvAtEntry:avgLv()};const rk=String(G.reforges||0);M.runZones=M.runZones||{};M.runZones[rk]=M.runZones[rk]||{};if(!M.runZones[rk][zn])M.runZones[rk][zn]={enterH:+M.hours.toFixed(2),lvAtEntry:+avgLv().toFixed(1),power:S.partyPower()};}
  ...
  function act(){
    ...
    if(S.canReforge()&&SHATTERS&&(G.reforges||0)<SHATTERS){const gain=S.reforgeGain();const stalled=(M.hours-lastProgressH)>=(args.stallHours||3);if(gain>=15&&(stalled||(Z[G.zone].endless&&(G.prog[G.zone]||0)>=100))){const rec={h:+M.hours.toFixed(2),run:G.reforges||0,zone:Z[G.zone].name,prog:G.prog[G.zone]||0,cleared:G.cleared.filter(Boolean).length,lv:+avgLv().toFixed(1),power:S.partyPower(),gain,dustBefore:G.dust,stallH:+(M.hours-lastProgressH).toFixed(2),reason:stalled?'stalled':'endless',farMark:farMark[G.zone]||0,lastProgressH:+lastProgressH.toFixed(2),before:S.partyState(),defeatsAt:G.stats.defeats||0,winsAt:G.wins,trainSecAt:(G.stats.autoTrain&&G.stats.autoTrain.timeSec)||0,byZoneAt:JSON.parse(JSON.stringify((G.stats&&G.stats.byZone)||{})),zoneTimeAt:JSON.parse(JSON.stringify(M.zoneTime||{})),bossAt:Object.fromEntries(Object.entries(M.boss).map(([k,v])=>[k,(v.attemptLog||[]).length]))};G.oathPick=null;S.doReforge();lastProgressH=M.hours;lastCleared=0;farMark={};rec.dustAfterGain=G.dust;rec.purchases=dustSpend();rec.dustAfter=G.dust;rec.treeAfter={b_start:S.treeLv('b_start'),b_hp:S.treeLv('b_hp'),b_dmg:S.treeLv('b_dmg')};rec.startLvNextRun=1+5*S.treeLv('b_start');rec.lvAfter=+avgLv().toFixed(1);rec.powerAfter=S.partyPower();rec.dust=G.dust;M.shatters.push(rec);M.firsts['shatter '+G.reforges]=+M.hours.toFixed(2);ev('shatter','#'+G.reforges+' for +'+gain+' dust (total '+G.dust+')');zoneDefeats=0;retreatUntil=0;}}
  }
  ...
  function dustSpend(){const DUST_PRIORITY=['b_start','b_hp','b_dmg'];const bought=[];for(;;){let did=false;for(const id of DUST_PRIORITY){const n=S.TREE.find(x=>x.id===id);if(!n)continue;const lv=S.treeLv(id);if(lv>=n.max)continue;const c=Math.round(n.base*Math.pow(1.5,lv));if(G.dust>=c){G.dust-=c;G.tree[id]=lv+1;for(const h of G.roster)S.refreshStats(h);bought.push({id,cost:c,rank:lv+1,dustLeft:G.dust,h:+M.hours.toFixed(2)});did=true;break;}}if(!did)break;}if(bought.length){M.dustBuys=(M.dustBuys||[]).concat(bought);}return bought;}
