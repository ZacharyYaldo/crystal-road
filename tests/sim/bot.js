...
const TAPS=args.taps==null?2:Number(args.taps),ACTIVE=!args.idle;
...
const M={hours:0,events:[],recruits:{},talents:{},promotions:{},zones:{},shatters:[],boss:{},drops:{},gold:[],delve:{best:0,runs:0,deaths:0},endless:{best:0,milestones:0},stuck:[]};
...
function main(){
  ...
  let retreatUntil=0,farmTarget=0,tapAcc=0,lastProgressH=0;
  const t0=Date.now();
  ...
  while(simSec<target){
    ...
    if(G.bossFight&&!bossWas){bossWas=true;const b=(M.boss[Z[G.zone].name]=M.boss[Z[G.zone].name]||{attempts:0,fails:0});b.attempts++;}
    if(!G.bossFight&&bossWas){bossWas=false;}
    if(G.mode==='defeat'&&G.timer>2.5){if(bossWas){M.boss[Z[G.zone].name].fails++;bossWas=false;}zoneDefeats++;}
    if(G.pack.length>lastPack){for(let i=lastPack;i<G.pack.length;i++){const it=G.pack[i];const k=Z[G.zone].name;(M.drops[k]=M.drops[k]||[0,0,0,0,0,0])[Math.min(5,it.rank||0)]++;}}
    ...
    const cl=G.cleared.filter(Boolean).length;if(cl>lastCleared){lastCleared=cl;const zi=G.cleared.lastIndexOf(true);M.zones[Z[zi].name]=M.zones[Z[zi].name]||+M.hours.toFixed(2);lastProgressH=M.hours;ev('cleared',Z[zi].name+' (party Lv '+avgLv()+')');}
    if(G.zone!==lastZone){lastZone=G.zone;zoneEnter=simSec;zoneDefeats=0;}
  ...
  function play(){if(G.mode!=='battle')return;for(const h of G.active){if(!h.dead&&h.charge>=100&&!h.tapCast&&!S.autoOn()){if(S.reactTo(h))continue;h.tapCast=true;h.charge=0;}}if(G.surge>=100&&!G.delve&&!G.hordeFight){try{S.castSurge();}catch(e){}}tapAcc+=TAPS*DT;while(tapAcc>=1){tapAcc-=1;const foes=G.enemies.filter(e=>!e.dead);if(!foes.length)break;const t=(G.focus&&G.focus.e&&!G.focus.e.dead)?G.focus.e:foes.sort((a,b)=>a.hp-b.hp)[0];try{S.tapEnemy(t);}catch(e){}}}
  ...
  function act(){
    ...
    for(const h of G.roster){const q=S.promoteReq(h);if((h.tier||0)<3&&h.lvl>=q.lvl&&G.gold>=q.gold&&G.dust>=q.dust){const t=h.tier||0;S.promoteHero(h);if((h.tier||0)>t){M.promotions[h.name+' T'+h.tier]=+M.hours.toFixed(2);ev('promote',h.name+' -> tier '+h.tier);}}
      const TL=S.TALENTS[h.cls]||[];TL.forEach((t,i)=>{if(h.talents&&h.talents[t.id])return;const q=S.talentReq(i);if(h.lvl>=q.lvl&&G.gold>=q.gold*1.2){S.buyTalent(h,i);if(h.talents[t.id]){M.talents[h.name+': '+t.name]=+M.hours.toFixed(2);ev('talent',h.name+' learns '+t.name);}}});}
    // road decisions
    const zi=G.zone;
    ...
    if(S.canReforge()&&SHATTERS&&(G.reforges||0)<SHATTERS){const gain=S.reforgeGain();const stalled=(M.hours-lastProgressH)>=3&&retreatUntil>0;if(gain>=15&&(stalled||(Z[G.zone].endless&&(G.prog[G.zone]||0)>=100))){G.oathPick=null;S.doReforge();lastProgressH=M.hours;M.shatters.push({h:+M.hours.toFixed(2),gain,dust:G.dust});ev('shatter','#'+G.reforges+' for +'+gain+' dust (total '+G.dust+')');zoneDefeats=0;retreatUntil=0;}}
  }
  ...
  function report(){
    const out={config:{hours:HOURS,shatters:SHATTERS,seed:SEED,dt:DT,taps:ACTIVE?TAPS:0},simHours:+M.hours.toFixed(2),final:{zone:Z[G.zone].name,prog:G.prog[G.zone],partyLv:avgLv(),gold:Math.round(G.gold),ore:Math.round(G.ore),dust:G.dust,shatters:G.reforges||0,wins:G.wins,defeats:G.stats.defeats||0,castle:G.castle.b,renown:G.renown||0},
      recruits:M.recruits,zonesCleared:M.zones,promotions:M.promotions,talents:M.talents,shatters:M.shatters,bossFailRates:Object.fromEntries(Object.entries(M.boss).map(([k,v])=>[k,{attempts:v.attempts,fails:v.fails,rate:v.attempts?+(v.fails/v.attempts).toFixed(2):0}])),
