...
// Shatter depth rule, attention bonus, Auto-Cast versus manual cast power, item level cap by rank, flat ascension cost, tree caps of 50.
...
(async()=>{
  // ---- Shatter depth rule: Shatters 1-3 need only the Warlord; the n-th Shatter from the fourth needs Endless wave 100 x (n - 3) this run
  {const S=await game(),G=S.G;G.cleared[3]=false;G.run={endless:1e9};ok(!S.canReforge(),'no Shatter before the Warlord even at any depth');G.cleared[3]=true;
    for(let done=0;done<=9;done++){G.reforges=done;const n=done+1,need=n<=3?0:100*(n-3);ok(S.reforgeNeedWave()===need,'Shatter '+n+' needs wave '+need);G.run={endless:Math.max(0,need-1)};ok(S.canReforge()===(need===0),'Shatter '+n+' refused one wave short ('+(need-1)+')');G.run={endless:need};ok(S.canReforge(),'Shatter '+n+' allowed at wave '+need);}
    G.reforges=3;G.run={};ok(!S.canReforge(),'fourth Shatter refused with no Endless depth recorded');}
  ...
  // ---- gear: item level cap 30 x (rank + 1); upgrades stop at the cap; ascension cost is flat per rank and independent of item level
  {const S=await game(),G=S.G;for(let r=0;r<=4;r++)ok(S.itemLvlCap({rank:r})===30*(r+1),'level cap at rank '+r+' is '+(30*(r+1)));
    const it=S.makeItem(0);it.rank=0;it.lvl=28;G.ore=1e15;G.mult='max';S.upgradeItem(it);ok(it.lvl===30,'bulk upgrade stops exactly at the cap (got '+it.lvl+')');const o=G.ore;S.upgradeItem(it);ok(it.lvl===30&&G.ore===o,'no upgrade and no ore spent at the cap');it.rank=1;S.upgradeItem(it);ok(it.lvl===60,'after ascending to rank 1 the cap is 60 (got '+it.lvl+')');
    ok(JSON.stringify(S.ASCEND_COST)===JSON.stringify([1500,150000,15000000,200000000]),'ascension table 1.5K / 150K / 15M / 200M');for(let r=0;r<4;r++){ok(S.ascendCost({rank:r,lvl:0})===S.ASCEND_COST[r]&&S.ascendCost({rank:r,lvl:200})===S.ASCEND_COST[r],'ascension from rank '+r+' costs '+S.ASCEND_COST[r]+' at any item level');}
    const it2=S.makeItem(0);it2.rank=0;it2.lvl=30;G.reforges=0;G.ore=1500;S.ascendItem(it2);ok(it2.rank===1&&G.ore===0,'ascending rank 0 to 1 costs exactly 1500 ore and needs no Shatter');G.ore=150000;S.ascendItem(it2);ok(it2.rank===1&&G.ore===150000,'rank 2 refused without a Shatter');G.reforges=1;S.ascendItem(it2);ok(it2.rank===2&&G.ore===0,'rank 2 after one Shatter costs exactly 150K');}
