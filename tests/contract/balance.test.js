...
(async()=>{
  ...
  // ---- gear: item level cap by rank (human 2026-09-15): 50 / 50 / 50 / 75 / 100, Mythic unlimited, +50 per devoured item; upgrades stop at the cap; ascension cost is flat per rank and independent of item level
  {const S=await game(),G=S.G;const caps=[50,50,50,75,100];for(let r=0;r<=4;r++)ok(S.itemLvlCap({rank:r})===caps[r],'level cap at rank '+r+' is '+caps[r]);ok(S.itemLvlCap({rank:5})===Infinity,'Mythic has no cap');ok(S.itemLvlCap({rank:0,devoured:2})===150,'two devoured items raise a Common cap to 150');
    const it=S.makeItem(0);it.rank=0;it.lvl=48;G.ore=1e15;G.mult='max';S.upgradeItem(it);ok(it.lvl===50,'bulk upgrade stops exactly at the cap (got '+it.lvl+')');const o=G.ore;S.upgradeItem(it);ok(it.lvl===50&&G.ore===o,'no upgrade and no ore spent at the cap');it.rank=3;S.upgradeItem(it);ok(it.lvl===75,'after ascending to Epic the cap is 75 (got '+it.lvl+')');
    ...
    const it2=S.makeItem(0);it2.rank=0;it2.lvl=20;G.reforges=0;G.ore=1500;S.ascendItem(it2);ok(it2.rank===1&&G.ore===0,'ascending rank 0 to 1 costs exactly 1500 ore and needs no Shatter');G.ore=150000;S.ascendItem(it2);ok(it2.rank===1&&G.ore===150000,'rank 2 refused without a Shatter');G.reforges=1;S.ascendItem(it2);ok(it2.rank===2&&G.ore===0,'rank 2 after one Shatter costs exactly 150K');}
