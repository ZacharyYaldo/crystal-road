...
(async()=>{
  ...
  {const S=await game(),G=S.G;S.setRT(100);ok(S.attnMult()===1,'attention off on a fresh game');const g0=S.goldMult(),x0=S.xpMult();S.noteAttention();near(S.attnMult(),1.25,1e-12,'attention on = 1.25');near(S.goldMult()/g0,1.25,1e-12,'goldMult x1.25 while attentive');near(S.xpMult()/x0,1.25,1e-12,'xpMult x1.25 while attentive');
    ...
    S.setRT(300);const h=G.active.find(x=>!x.dead);h.charge=100;h.tapCast=false;ok(S.tapCastHero(h)===true&&h.tapCast===true&&h.charge===0&&!S.attnOn(),'a manual cast through tapCastHero arms the cast and does not start attention (only enemy taps do, human 2026-09-15)');{const en=G.enemies.find(e=>!e.dead);S.tapEnemy(en);ok(S.attnOn(),'an enemy tap starts attention');}ok(S.tapCastHero(h)===false,'a second tap while the cast is armed does nothing');}
