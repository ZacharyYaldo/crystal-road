...
(async()=>{
  ...
  // ---- gear: item level cap 20 x (rank + 1); upgrades stop at the cap; ascension cost is flat per rank and independent of item level
  {const S=await game(),G=S.G;for(let r=0;r<=4;r++)ok(S.itemLvlCap({rank:r})===20*(r+1),'level cap at rank '+r+' is '+(20*(r+1)));
    const it=S.makeItem(0);it.rank=0;it.lvl=18;G.ore=1e15;G.mult='max';S.upgradeItem(it);ok(it.lvl===20,'bulk upgrade stops exactly at the cap (got '+it.lvl+')');const o=G.ore;S.upgradeItem(it);ok(it.lvl===20&&G.ore===o,'no upgrade and no ore spent at the cap');it.rank=1;S.upgradeItem(it);ok(it.lvl===40,'after ascending to rank 1 the cap is 40 (got '+it.lvl+')');
