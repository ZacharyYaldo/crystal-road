...
(async()=>{
  ...
  {const S=await game();const dust=(zi,t)=>S.challengeBundle('zone',t,zi).dust;ok([0,1,2].every(zi=>dust(zi,3)===0)&&[3,4,5].every(zi=>dust(zi,2)===0&&dust(zi,3)===1)&&[6,7,8].every(zi=>dust(zi,2)===1&&dust(zi,3)===2),'zone dust: none before Ironvein, 1 at Crystal for Ironvein-Amberfall, 1 at Gold and 2 at Crystal for Ashen Approach-Foundry');
    let total=0;for(let zi=0;zi<9;zi++)total+=dust(zi,3);ok(total===9,'clearing every zone at Crystal awards 9 dust in total');ok(S.challengeBundle('shatter',3,3).dust===6&&S.challengeBundle('shatter',1,3).dust===1,'Shatter bracket dust 1 / 3 / 6 at Silver / Gold / Crystal');}
  ...
  // ---- offline parity: the refactored offlineGains equals the previous formula whenever no temporary modifier is active
  {const S=await game(),G=S.G;const old=(hours,eff)=>{const fights=Math.floor(hours*3600/22*eff);const Z=S.ZONES[G.zone],L=Z.endless?S.endlessLv(Z,G.prog[G.zone]):S.roadLv(Z,Z.lv+Math.min(G.prog[G.zone],Z.fights-1)*0.45);const per=1.8;return{fights,gold:Math.round(fights*per*(3+2*L)*Math.pow(1.05,L)*S.goldMult()*S.zoneBonus()),xp:Math.round(fights*per*(6+3*L)*Math.pow(1.04,L)*S.bless('xp')),ore:Math.round(fights*S.ORE_DROP_CHANCE*1.5*(1+Math.floor(L/6))*S.oreScale(L)*S.oreMult()*S.zoneBonus())};};
    let worst=0,cases=0;for(const zone of [0,3,6,8,9])for(const prog of [0,10,30])for(const rf of [0,2,5])for(const b of [0,4]){G.zone=zone;G.prog[zone]=prog;G.reforges=rf;G.tree.b_gold=b;G.tree.b_ore=b;G.tree.ore=b;G.renown=rf*1e5;for(const [hours,eff] of [[2,0.5],[8,0.65],[0.5,0.5]]){const a=old(hours,eff),n=S.offlineGains(hours,eff);cases++;for(const k of ['fights','gold','xp','ore']){const rel=Math.abs(a[k]-n[k])/Math.max(1,Math.abs(a[k]));worst=Math.max(worst,rel);}}}
    ok(worst<0.002,'offline parity over '+cases+' states: worst relative difference '+worst.toExponential(2)+' (rounding only)');
    G.zone=9;G.prog[9]=200;G.reforges=3;G.omens=['swarm'];const withOmen=old(2,0.5),now=S.offlineGains(2,0.5);G.omens=[];const without=old(2,0.5);ok(now.gold===without.gold,'with an Endless omen active the new estimate ignores it (with omen '+withOmen.gold+', new '+now.gold+', permanent-only '+without.gold+')');G.zone=3;G.reforges=0;}
