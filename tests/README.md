...
Running the suites: `node tests/run_tests.js` runs every file in tests/contract in its own process and exits 1 if any suite fails or crashes. Never pipe a suite into tail or grep to read its verdict: a pipe returns the exit code of the last command, so a failing suite behind `| tail` looks green (that hid a failing speed test once, 2026-09-15). Read the runner's verdict lines instead.
