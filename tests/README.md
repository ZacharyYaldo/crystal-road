...
Timestep: the bot advances the game in `--dt` second steps, default 0.05, which is production's per-frame cap (a phone at 60fps steps 1/60). Fight timing is step-sensitive, so a coarser step (the old 0.1 default) makes fights slower and every timing later; the bot asserts `dt <= 0.05` unless `--allowCoarseDt`, and `--dt 0.0167` is available for full frame-rate fidelity at about three times the cost. Every tick also asserts that the game's clock equals the simulated clock.
